...
QMap < QString, QString > UTILITY_JSON::split( QString qs )
{
   ...
   for ( unsigned i = 0; i < (unsigned int) qs.length(); i++ )
   {
      QChar qc = qs.at( i );
      if ( qc == '{' && !in_json )
      {
...
QString UTILITY_JSON::compose( QMap < QString, QString > &mqq )
{
         ...
         it++ )
   {
      ...
      result += 
         ( rx_no_quotes.indexIn( it.key() ) != -1  ?
           it.key() :
           "\"" + it.key() + "\"" )
         + ":" +
         QString( rx_no_quotes.indexIn( it.value() ) != -1  ?
                  it.value() :
                  "\"" + it.value() + "\"" 
                  ).replace( "\n", "\\r\\n" ) 
         + "," 
...
QVector < plotData > * UTILITY_JSON::array_string_to_qvv( const QString & s )
{
   QVector < plotData > * results = new QVector < plotData >;
...
QString UTILITY::unique_directory( const QString & basedir )
{
   ...
   {
      // try to make parent
      if ( !qd.mkdir( QFileInfo( qd.path() ).path() ) ) 
      {
