...
text:"Internal error: no url defined for cache clear"})};ga.trytilltrue=function(a,b,c,d){if(eval(a))return eval(b);c--;if(0>c)console.warn("ga.trytilltrue failed ... to many tries");else return setTimeout(ga.trytilltrue,d,a,b,c,d)};
ga.loginverify=function(a){var b={text:a.text||"You must verify your email address."};a.useroptions&&(a.useroptions.resend&&(b.buttons=b.buttons||[],b.buttons.push({id:"resend",label:"Resend the verification email",cb:ga.loginverify.resend})),a.useroptions.resend&&(b.buttons=b.buttons||[],b.buttons.push({id:"changeaddress",label:"Change your email address and resend the verification",cb:ga.loginverify.change})),a.useroptions.cancel&&(b.buttons=b.buttons||[],b.buttons.push({id:"cancelregistration",
label:"Cancel your registration"})));messagebox(b)};ga.loginverify.resend=function(){var a=$("#sys_login");if(!a)return messagebox({icon:"toast.png",text:"Internal error: form missing"});a.append('<input type="hidden" name="_resendverify" class="toclear">');do_sys_login_submit(a)};ga.loginverify.change=function(){var a=$("#sys_login");if(!a)return messagebox({icon:"toast.png",text:"Internal error: form missing"});a.append('<input type="hidden" name="_resendverify" class="toclear">');do_sys_login_submit(a)};
ga.loginverify.cancel=function(){var a=$("#sys_login");if(!a)return messagebox({icon:"toast.png",text:"Internal error: form missing"});a.append('<input type="hidden" name="_cancel" class="toclear">');do_sys_login_submit(a)};ga.colors=function(a){ga.colors.background=ga.colors.makeRGB(a.background);ga.colors.text=ga.colors.makeRGB(a.text)};ga.colors.makeRGB=function(a){if(/\d{1,3},\s*\d{1,3},\s*\d{1,3}$/.test(a))return a;a=ga.color.toRGB(a);return a.r+","+a.g+","+a.b};
...
h+'">'+e.resources[h][n]+"</label></td></tr>";messagebox({icon:"question.png",text:f+"</table>",buttons:g});return"deferred";default:return messagebox({icon:"toast.png",text:"ga.airavata.select, unknown selectoin type '"+a+"'"}),"abort"}};ga.value={};ga.value.settings={};
...
ga.value.processInputfromFiles=function(a,b,c){var d=a.trim().split(/[\r\n]+/g),e=[],f=0;c=ga.repeat.map.convert(c);for(var g=0;g<d.length;g++){var h=d[g].trim().split(/\s+/),f=f+h.length;e.push(f)}a=a.trim().split(/\s+/);d=0;switch(b){case "whitespaceseparated":case "whitespaceseparated_reverselogic":if(a.length==c.length)for(g=0;g<a.length;g++){for(f=0;f<e.length;f++)if(g+1<=e[f]){d=f+1;break}switch($("#"+c[g]).attr("type")){case "checkbox":f=/^(0|1|n|y|true|false|t|f|yes|no)$/;if(!f.test(a[g].toLowerCase())){messagebox({icon:"warning.png",
text:"Wrong format of the input file! Checkbox input value on the line #"+d+" is not valid. Options are: [1 | 0 | yes | no | true | false | t | f | T | F | y | n | Y | N ]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;case "number":f=[a[g]];if(ga.value.checkFloatIntOK("#"+c[g],f))a[g]=f[0];else{messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+d+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",
buttons:[{id:"ok",label:"OK"}]});return}break;case "text":f=new RegExp($("#"+c[g]).attr("pattern"));if(!f.test(a[g])){messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+d+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;default:messagebox({icon:"warning.png",text:"Selected input type is currently not supported. Contact the developer",buttons:[{id:"ok",
label:"OK"}]});return}}else{messagebox({icon:"warning.png",text:"Wrong format of the input file! Number of parameters is inconsistent with the model chosen. Check your parameter file",buttons:[{id:"ok",label:"OK"}]});return}break;default:messagebox({icon:"warning.png",text:"Selected file parsing mode is currently not supported. Contact the developer",buttons:[{id:"ok",label:"OK"}]});return}for(g=0;g<a.length;g++)switch($("#"+c[g]).attr("type")){case "checkbox":if(0<=b.indexOf("reverselogic"))switch(a[g].toLowerCase()){case "0":case "false":case "f":case "n":case "no":$("#"+
c[g]).prop("checked",!0);break;case "1":case "true":case "t":case "yes":case "y":$("#"+c[g]).prop("checked",!1);break}else switch(a[g].toLowerCase()){case "0":case "false":case "f":case "n":case "no":$("#"+c[g]).prop("checked",!1);break;case "1":case "true":case "t":case "yes":case "y":$("#"+c[g]).prop("checked",!0);break}default:$("#"+c[g]).val(a[g]),$("#"+c[g]).prop("defaultValue",a[g])}};ga.value.input={};
ga.value.setInputForRFile=function(a,b,c,d,e){ga.value.input[a]=ga.value.input[a]||{};ga.value.input[a][c]={};ga.value.input[a][c].id=c;ga.value.input[a][c].tag=b;ga.value.input[a][c].mode=d;ga.value.input[a][c].ids=e};ga.value.types={};ga.value.registerid=function(a,b,c,d){ga.value.types[a]=ga.value.types[a]||{};ga.value.types[a][b]={};ga.value.types[a][b].id=b;ga.value.types[a][b].label=c;ga.value.types[a][b].req=d||0};
ga.value.setInputfromRFile=function(a,b,c){var d=c.split(",");a="results/users/"+$("#_state").data("_logon")+"/"+a;$.get(a,function(a){ga.value.processInputfromFiles(a,b,d)},"text")};ga.value.setInputfromFile=function(a,b,c){$(a).hide();var d=c.split(",");$(a).change(function(c){c=$(a)[0].files[0];var f=new FileReader;f.onload=function(a){ga.value.processInputfromFiles(a.target.result,b,d)};f.readAsText(c)})};
...
b+'_tr"'),'id="%%id%%_tr"').replace(RegExp('id="'+b+'_button"'),'id="%%id%%_button"').replace(RegExp('="'+b+'_altval"',"g"),'="%%id%%_altval"').replace(RegExp('name="_selaltval_'+b+'"'),'name="_selaltval_%%id%%"').replace(RegExp('id="'+b+'-repeater"'),'id="%%id%%-repeater"');ga.repeat.data[a].repeat[b].evalr=d.replace(RegExp('"#'+b+'"',"g"),'"#%%id%%"').replace(RegExp('"#'+b+" option","g"),'"#%%id%% option').replace(RegExp(":"+b+":","g"),":%%id%%:").replace(RegExp('"#'+b+'_msg"',"g"),'"#%%id%%_msg"').replace(RegExp('"'+
b+'"',"g"),'"%%id%%"').replace(RegExp('"#'+b+'_button"',"g"),'"#%%id%%_button"').replace(RegExp('"'+b+'_altval"',"g"),'"%%id%%_altval"').replace(RegExp('"#'+b+'_altval"',"g"),'"#%%id%%_altval"');return'<tr><td></td><td><span id="'+b+'-span"></span></td></tr>'};
...
ga.repeat.children(a,b);switch(ga.repeat.data[a].repeater[b].type){case "checkbox":if(d)for(k in g)l=b+"-"+k,ga.repeat.map[k]=l,h+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,l).replace("%%label%%",""),n+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,l),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[l]||(ga.repeat.data[a].repeater[l]=jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(l),ga.repeat.data[a].repeater[l].value&&delete ga.repeat.data[a].repeater[l].value);
break;case "integer":for(p=1;p<=d;++p)for(k in g)l=b+"-"+k+"-"+(p-1),ga.repeat.map[k]=l,h+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,l).replace("%%label%%","["+p+"]"),n+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,l),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[l]||(ga.repeat.data[a].repeater[l]=jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(l),ga.repeat.data[a].repeater[l].value&&delete ga.repeat.data[a].repeater[l].value);break;case "listbox":g=b.replace(/-[0-9]+$/,
"").replace(/^(.*)-([A-ZA-z0-9_]*)$/,"$2")+"-"+d;p=b+"-"+d;g=ga.repeat.children(a,g);for(k in g)l=p+"-"+k,ga.repeat.map[k]=l,h+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,l).replace("%%label%%",""),n+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,l),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[l]||(ga.repeat.data[a].repeater[l]=jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(l),ga.repeat.data[a].repeater[l].value&&delete ga.repeat.data[a].repeater[l].value);break;
default:return console.warn("ga.repeat.change( "+a+" , "+b+" ) type "+ga.repeat.data[a].repeater[b].type+" not supported"),!1}!/^<tr>/.test(h)&&/<\/tr>$/.test(h)&&(h="<tr>"+h);$(f+"-repeater").html(h);eval(n);ga.repeat.data[a].repeater[b].value=d;for(k=0;k<e.length;++k)ga.repeat.change(a,e[k],c);$("#global_data").data("_pull_json#"+b)&&ga.pull.doPull("#"+b);resetHoverHelp()};ga.repeat.map.convert=function(a){var b,c=[];for(b=0;b<a.length;++b)c[b]=ga.repeat.map[a[b]]||a[b];return c};
...
ga.data.nofcrefresh={};
ga.data.update=function(a,b,c,d){var e=0,f=0,g=0,h=0,n=0,k=a+"_output",p="#"+k,l=$(p),A={},x=p+"_msgs",w=$(x),y,t,u,v,q,z,r;c&&($("#"+a+"_progress").html(""),w.text(""));$.each(b,function(b,m){r=l.find("#"+b);if(r.length)switch(e||(w.text(""),e=1),r.attr("type")){case "plot2d":y="#"+b;m.data?(ga.value.set.plot2d(y,m.options),$.plot(r,m.data,ga.value.get.plot2d.plot_options(y,m.options))):$.plot(r,m,ga.value.get.plot2d.plot_options(y));u=k+":#"+b+":last_value";$("#global_data").data(u,m);break;case "atomicstructure":u=
k+":#"+b+":last_value";z=m.file?m.file:m;_jmol_info[b].script="set background ["+ga.colors.background+"]; set zoomlarge false;set echo top center;echo loading "+z.split("/").pop()+';refresh;load "'+z+'";';ga.set(a+":jsmoladd")&&(_jmol_info[b].script+=ga.set(a+":jsmoladd"));m.script&&(_jmol_info[b].script+=";"+m.script);$("#global_data").data(u,_jmol_info[b].script);$("#"+b).html(Jmol.getAppletHtml("jmolApplet"+b,_jmol_info[b]));break;case "checkbox":case "radio":r.prop("checked",!0);break;case "div":r.html(m);
break;case "video":t=$("#"+b);q="<video ";t.attr("data-width")&&(q+=' width="'+t.attr("data-width")+'"');t.attr("data-height")&&(q+=' height="'+t.attr("data-height")+'"');q+=" controls>";q+='<source src="'+m+'.mp4" type="video/mp4" /><source src="'+m+'.webm" type="video/webm" />';q+="</video>";t.html(q);break;case "image":t=$("#"+b);q="<img ";t.attr("data-width")&&(q+=' width="'+t.attr("data-width")+'"');t.attr("data-height")&&(q+=' height="'+t.attr("data-height")+'"');q+=' src="'+m+'">';t.html(q);
break;case "filelink":v='<a href="'+m+'" target="_blank">'+m.split("/").pop()+"</a>";u=k+":#"+b+":last_value";$("#global_data").data(u,v);$("#"+b+"_filelink").html(v);break;case "filelinkm":u=k+":#"+b+":last_value";v="";$.each(m,function(a,b){v+='<a href="'+b+'" target="_blank">'+b.split("/").pop()+"</a> "});$("#global_data").data(u,v);$("#"+b+"_filelink").html(v);break;default:$("#global_data").data("_append:"+k+"_"+b)?(r.val(r.val()+"\n"+m),r.height(parseFloat(r.prop("scrollHeight")+parseFloat(r.css("borderTopWidth"))+
parseFloat(r.css("borderBottomWidth"))))):r.val(m)}else c?"_"==b.charAt(0)?/^_fs_/.test(b)&&ga.data.nofcrefresh[a]||("_message"==b&&messagebox(m),/^_getinput/.test(b)&&"_getinput"==b&&ga.valuen.input(a,m),"_textarea"==b&&ga.data.textarea(p,m),"_airavata"==b&&ga.data.airavata(p,m),"_status"==b&&"complete"==m&&c(d,0,0)):(f||(w.append("<p>Unexpected results:</p>"),f=1),w.append("<p>"+b+" => "+m+"</p>")):"_"==b.charAt(0)?/^_fs_/.test(b)&&ga.data.nofcrefresh[a]||($("#_state").data(b,m),g=1,"_status"==
b&&(A.job_status=m),/^_getinput/.test(b)&&"_getinput"==b&&ga.valuen.input(a,m),"_textarea"==b&&ga.data.textarea(p,m),"_airavata"==b&&ga.data.airavata(p,m),"_loginverify"==b&&ga.loginverify(m)):"-close"==b?h=1:"-close2"==b?n=1:(f||(w.text(""),w.append("<p>Unexpected results:</p>"),e=f=1),w.append("<p>"+b+" => "+m+"</p>"))});ga.value.saveLastValues(k);ga.value.saveLastValue(k,x);$(p+"_progress").html("");g&&syncState();h&&closeModal();n&&closeModal2();return A};
ga.data.textarea=function(a,b){var c=a+"_textarea",d=$(c);d.is(":hidden")&&(d.show(),$(c+"_label").show());"__reset__\n"==b.substr(0,10)?d.val(b.substr(10)):d.val(d.val()+b);ga.set("textarea:rows")?ga.data.textarea.h[a]?d.height(ga.data.textarea.h[a]):ga.data.textarea.h[a]=parseFloat(d.prop("clientHeight"))+parseFloat(d.css("borderTopWidth"))+parseFloat(d.css("borderBottomWidth")):d.height(parseFloat(d.prop("scrollHeight"))+parseFloat(d.css("borderTopWidth"))+parseFloat(d.css("borderBottomWidth")))};
ga.data.textarea.h={};ga.data.airavata=function(a,b){var c=$(a+"_airavata");c.is(":hidden")&&c.show();c.html(b)};
...
ga.valuen.restore=function(a,b,c){var d="#"+a,e=$(d),f=e.find(":input").get(),g={},h,n;b=b||ga.valuen.data[a];c=c||ga.valuen.html[a];if(b){$(d+" .field_msg").html("");do h=!1,$.each(f,function(){var c,d,l=$(this),n,x;if(l.attr("data-repeater")&&!g[this.name]){g[this.name]=!0;if(this.name&&(b[this.name]||/checkbox|radio/i.test(this.type))){d=b[this.name];if(/checkbox|radio/i.test(this.type)){n=l.val();x=!1;if(d)for(c=0;c<d.length;c++)if(d[c]==n){x=!0;break}l.prop("checked",x)}else l.val(d[0]);h=!0;
