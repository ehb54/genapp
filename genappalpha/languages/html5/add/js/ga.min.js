...
ga.data.update=function(a,b,c,d){var e=0,f=0,h=0,g=0,p=0,k=a+"_output",q="#"+k,m=$(q),z={},r=q+"_msgs",w=$(r),n,u,x,A,v,B,y;c&&($("#"+a+"_progress").html(""),w.text(""));$.each(b,function(b,l){y=m.find("#"+b);if(y.length)switch(e||(w.text(""),e=1),y.attr("type")){case "plot2d":n="#"+b;var r,C;ga.value.plot2d.zstack.reset(n);l.data?(ga.value.set.plot2d(n,l.options),ga.plotglobal=l.options,ga.dataplotglobal=l.data,$.plot(n,l.data,ga.value.get.plot2d.plot_options(n,l.options))):$.plot(n,l,ga.value.get.plot2d.plot_options(n));
$(n+"_changescale").length&&$(n+"_changescale").removeClass("hidden");if($(n+"_savetofile").length){var D=document.getElementById(b+"_savetofile");html2canvas(y.get(0),{background:"#ffffff",onrendered:function(a){r=a.toDataURL("image/png").replace("image/png","image/octet-stream");C=dataURLtoFile(r,"plot.png");D.href=URL.createObjectURL(C);$(n+"_savetofile").removeClass("hidden")}})}if(ga.value.settings[n].selzoom||l.options&&l.options.selection&&l.options.selection.mode&&"xy"==l.options.selection.mode)$(n).on("plotselected",
...
"</a>";x=k+":#"+b+":last_value";$("#global_data").data(x,A);$("#"+b+"_filelink").html(A);break;case "filelinkm":x=k+":#"+b+":last_value";A="";$.each(l,function(a,b){A+='<a href="'+b+'" target="_blank">'+b.split("/").pop()+"</a> "});$("#global_data").data(x,A);$("#"+b+"_filelink").html(A);break;default:$("#global_data").data("_append:"+k+"_"+b)?(y.val(y.val()+"\n"+l),y.height(parseFloat(y.prop("scrollHeight")+parseFloat(y.css("borderTopWidth"))+parseFloat(y.css("borderBottomWidth"))))):y.val(l)}else c?
"_"==b.charAt(0)?/^_fs_/.test(b)&&ga.data.nofcrefresh[a]||("_message"==b&&messagebox(l),/^_getinput/.test(b)&&"_getinput"==b&&ga.valuen.input(a,l),"_textarea"==b&&ga.data.textarea(q,l),"_airavata"==b&&ga.data.airavata(q,l),"_status"==b&&"complete"==l&&c(d,0,0)):(f||(w.append("<p>Unexpected results:</p>"),f=1),w.append("<p>"+b+" => "+l+"</p>")):"_"==b.charAt(0)?/^_fs_/.test(b)&&ga.data.nofcrefresh[a]||($("#_state").data(b,l),h=1,"_status"==b&&(z.job_status=l),/^_getinput/.test(b)&&"_getinput"==b&&
ga.valuen.input(a,l),"_textarea"==b&&ga.data.textarea(q,l),"_airavata"==b&&ga.data.airavata(q,l),"_loginverify"==b&&ga.login.verify(l),"_loginapprove"==b&&ga.login.approve(l)):"-close"==b?g=1:"-close2"==b?p=1:(f||(w.text(""),w.append("<p>Unexpected results:</p>"),e=f=1),w.append("<p>"+b+" => "+l+"</p>"))});ga.value.saveLastValues(k);ga.value.saveLastValue(k,r);$(q+"_progress").html("");h&&syncState();g&&closeModal();p&&closeModal2();return z};
