var ga={tmp:{},multistage:{},stages:{},stagesdefault:{},currentstage:0,fielderrors:0};
...
ga.cache.doclear=function(a,b){ga.cache.url_clear?$.getJSON(ga.cache.url_clear,{tagmode:"any",format:"json",_window:window.name,_logon:$("#_state").data("_logon"),_uuid:a,_cachedelete:b}).done(function(a,b,e){"true"==a.success?ga.msg.box({icon:"information.png",text:"Previously computed results cleared"}):ga.msg.box({icon:"toast.png",text:a.error})}).fail(function(a,b,e){console.warn("could not clear cache data");ga.msg.box({icon:"toast.png",text:"Error: server failed to clear cached entry"})}):ga.msg.box({icon:"toast.png",
...
ga.admin.ajax.group=function(a,b,c,d,e){ga.msg.box({icon:"admin.png",text:"Enter the group for user "+b+'<form id="sys_musergrp"><input id="sys_musergrp_text" class="help_link" type="text" size="25" value="'+e+'"><span class="help">Enter a group for this user, then press ok or cancel</span></form>',eval:"resetHoverHelp();$('#sys_musergrp').on('keyup keypress', function(e) { var code = e.keyCode || e.which;  if (code  == 13) { e.preventDefault(); return false; }});",buttons:[{id:"ok",label:"Ok",cb:ga.admin.ajax.group.cb,
adata:[a,b,c,d]},{id:"cancel",label:"Cancel"}]})};
ga.admin.ajax.group.cb=function(a,b,c,d,e){$.get(ga.admin.ajax.url,{tagmode:"any",format:"json",_window:window.name,_logon:$("#_state").data("_logon"),_cmd:a,_name:b,_id:c,_manageid:d,_group:$("#sys_musergrp_text").val()}).done(function(a,b,c){a=JSON.parse(a.replace(/^\s*[\r\n]/gm,"").split(/\r?\n/)[1]);"true"!=a.success&&ga.msg.box({icon:"toast.png",text:a.error?a.error:"unknown error"});a._submitid&&$("#"+a._submitid).trigger("click")}).fail(function(a,b,c){ga.msg.box({icon:"toast.png",text:"Error: system user management backend command failed to run: "+
...
color:ga.color.data[".help"].color});"#"===ga.color.data.body.color.slice(0,1)?(c=parseInt(ga.color.data.body.color.slice(1),16),b=parseInt(c/256),c-=256*b,a=parseInt(b/256),b-=256*a,d=a,e=b,f=c,a+=128<a?-12:12,b+=128<b?-12:12,c+=128<c?-12:12,$(".header1").css({color:"rgb("+a+","+b+","+c+")"}),a+=128<a?-12:12,b+=128<b?-12:12,c+=128<c?-12:12,$(".header2").css({color:"rgb("+a+","+b+","+c+")"}),a+=128<a?-12:12,b+=128<b?-12:12,c+=128<c?-12:12,$(".header3").css({color:"rgb("+a+","+b+","+c+")"}),a+=128<
a?-12:12,b+=128<b?-12:12,c+=128<c?-12:12,$(".header4").css({color:"rgb("+a+","+b+","+c+")"}),a+=128<a?-12:12,b+=128<b?-12:12,c+=128<c?-12:12,$("hr").css({color:"rgb("+a+","+b+","+c+")"}),a=d,b=e,c=f,b+=128<b?-75:75,ga.cssrule.kill("a:link"),g=ga.cssrule.add("a:link"),g.style.color="rgb("+a+","+b+","+c+")",b=e,c+=128<c?-75:75,a+=128<a?-75:75,ga.cssrule.kill("a:visited"),g=ga.cssrule.add("a:visited"),g.style.color="rgb("+a+","+b+","+c+")",c=f,a=d,a+=128<a?-75:75,ga.cssrule.kill("a:active"),g=ga.cssrule.add("a:active"),
...
ga.altfile.button.lrfile=function(a,b,c){var d=[],e="#"+b,f="#"+c,g="",h="_hidden_lrfile_sels_"+c;$(e+" ."+h).remove();$.each($(a).jstree("get_checked",!0),function(){this.children.length||(g+='<input type="hidden" name="'+c+'_altval[]" value="'+this.id+'" class="'+h+'">',d.push($.base64.decode(this.id).substr(2)))});d.length&&($(f+"_altval").html("<i>Server</i>: "+d),$(f+"_msg").html(""),$(f).val(""),$(e).append(g),ga.value.input[b]&&ga.value.input[b][c]&&ga.value.setInputfromRFile(d,ga.value.input[b][c].mode,
ga.value.input[b][c].ids))};ga.altfile.button.rpath=function(a,b,c){var d=[];b="#"+b;var e="#"+c,f="",g="_hidden_rpath_sels_"+c,h=$(a).jstree(!0);$(b+" ."+g).remove();$.each(h.get_top_checked(!0),function(){h.is_leaf(this)||(f+='<input type="hidden" name="'+c+'[]" value="'+this.id+'" class="'+g+'"><input type="hidden" name="_decodepath_'+c+'" class="'+g+'">',d.push($.base64.decode(this.id).substr(2)))});d.length&&($(e+"_altval").html("<i>Server</i>: "+d),$(e+"_msg").html(""),$(e).val(""),$(b).append(f))};
ga.altfile.button.rfile=function(a,b,c){var d=[],e="#"+b,f="#"+c,g="",h="_hidden_rfile_sels_"+c;$(a).jstree(!0);$(e+" ."+h).remove();$.each($(a).jstree("get_checked",!0),function(){this.children.length||(g+='<input type="hidden" name="'+c+'_altval[]" value="'+this.id+'" class="'+h+'">',d.push($.base64.decode(this.id).substr(2)))});d.length&&($(f+"_altval").html("<i>Server</i>: "+d),$(f+"_msg").html(""),$(f).val(""),$(e).append(g),ga.value.input[b]&&ga.value.input[b][c]&&ga.value.setInputfromRFile(d,
ga.value.input[b][c].mode,ga.value.input[b][c].ids))};ga.altfile.button.job=function(a,b,c){var d="";b="#"+b;var e="#"+c,f="",g="_hidden_job_sels_"+c;$(a).jstree(!0);$(b+" ."+g).remove();$.each($(a).jstree("get_checked",!0),function(){this.children.length||(f+='<input type="hidden" name="'+c+'_altval[]" value="'+this.id+'" class="'+g+'">',d+="<tr><td>"+this.parent+":"+this.text+"</td></tr>")});d.length&&($(e+"_altval").html("<table>"+d+"</table>"),$(e+"_msg").html(""),$(e).val(""),$(b).append(f))};
...
ga.airavata.select=function(a,b,c,d){var e=ga.airavata.data,f="",g=[],h,q,k;if("__resource__"==a&&!e.defaultresource||"airavata"!=a&&"__resource__"!=a)return"notused";if(!e.resources||!e.resources.length)return ga.msg.box({icon:"warning.png",text:"No resources currently enabled for Airavata submission"}),"abort";if(1==e.resources.length)return Object.keys(e.resources[k])[0];a="__airavataselect__"!=b?b:e.select.length?e.select:"random";switch(a){case "random":return k=Math.floor(e.resources.length*
Math.random()),Object.keys(e.resources[k])[0];case "choose":g.push({id:"submit_module",label:"Submit",data:[c,d,e.resources],cb:function(a){a[0](a[1],Object.keys(a[2][$("#airavata input[name=selectresource]:checked").val()])[0])}});f='<h3>Select a compute resource and press submit</h3><form id="airavata"><table>';for(h in e.resources)for(q in e.resources[h])f+='<tr><td><input type="radio" name="selectresource" id="airavata_'+h+'" value="'+h+'"'+(0==h?'checked="checked"':"")+'></td><td class="hoverhighlight" style="text-align:left"><label for="airavata_'+
h+'">'+e.resources[h][q]+"</label></td></tr>";ga.msg.box({icon:"question.png",text:f+"</table>",buttons:g});return"deferred";default:return ga.msg.box({icon:"toast.png",text:"ga.airavata.select, unknown selection type '"+a+"'"}),"abort"}};ga.resource={};ga.xsede={};ga.resource.xsedeprojecttokeys=function(){var a;ga.resource.xsedepkeys={};for(a in ga.resource.xsedeproject)ga.resource.xsedepkeys[ga.resource.xsedeproject[a]]=1};
ga.xsede.select=function(a,b,c){var d=ga.xsede.data;a="__resource__"==a?ga.resource.defaultval:a;var e="",f=[],g;delete ga.xsede.useproject;if(!(ga.resource.xsedeproject&&a in ga.resource.xsedepkeys))return b(c);if(!d||!d.length)return ga.msg.box({icon:"warning.png",text:"No XSEDE projects currently defined.  Create one under the user configuration button at the top right."}),"abort";if(1==d.length)return ga.xsede.useproject=d[0],b(c);f.push({id:"submit_xsedeproject",label:"Submit",data:[b,c,d],cb:function(a){ga.xsede.useproject=
a[2][$("#xsedeproject input[name=selectxsedeproject]:checked").val()];a[0](a[1])}});e='<h3>Select an XSEDE project and press submit</h3><form id="xsedeproject"><table>';for(g in d)e+='<tr><td><input type="radio" name="selectxsedeproject" id="xsedeproject_'+g+'" value="'+g+'"'+(0==g?'checked="checked"':"")+'></td><td class="hoverhighlight" style="text-align:left"><label for="xsedeproject_'+g+'">'+d[g]+"</label></td></tr>";ga.msg.box({icon:"question.png",text:e+"</table>",buttons:f});return"deferred"};
...
ga.value.showfields=function(a,b,c,d){var e=[];$.each(a[b],function(a,b){console.log(a);a==c[d]&&$.each(b,function(a,b){console.log(b);e.push(b)})});for(a=0;a<e.length;a++)$("#"+e[a]).data("repeater")&&$("#"+e[a]+"-repeater").show(),$("#"+e[a]+", label[for="+e[a]+"]").show()};
...
ga.value.setInputfromFile=function(a,b,c,d){$(a).hide();var e=c.split(",");$(a).change(function(c){c=$(a)[0].files[0];var g=new FileReader;g.onload=function(a){ga.value.processInputfromFiles(a.target.result,b,e,d)};g.readAsText(c)})};
ga.value.setLastValue=function(a,b,c){var d=a+":"+b+":last_value",e=a+":"+b+":default_value",f=$(b);if(!/_output$/.test(a))return!1;if(void 0==$("#global_data").data(d))switch(f.attr("type")){case "checkbox":case "radio":$("#global_data").data(d,f.is(":checked"));$("#global_data").data(e,f.is(":checked"));break;case "div":case "msgs":$("#global_data").data(d,f.html());$("#global_data").data(e,f.html());break;case "plot2d":break;case "bokeh":ga.bokeh.renderdata(a,b.replace(/^#/,""));break;case "filelink":case "filelinkm":$("#global_data").data(d,
$(b+"_filelink").html());break;default:c&&f.val(c),$("#global_data").data(d,f.val()),$("#global_data").data(e,f.val())}else switch(f.attr("type")){case "checkbox":case "radio":f.prop("checked",$("#global_data").data(d));break;case "div":case "msgs":f.html($("#global_data").data(d));break;case "atomicstructure":b=b.replace(/^#/,"");$("#global_data").data(d)?(_jmol_info[b].script=$("#global_data").data(d),f.html(Jmol.getAppletHtml("jmolApplet"+b,_jmol_info[b]))):f.html("");break;case "plot2d":d=gd.data(d);
d.data?(ga.value.set.plot2d(b,d.options),f.plot(d.data,ga.value.get.plot2d.plot_options(b,d.options))):f.plot(d,ga.value.get.plot2d.plot_options(b));break;case "bokeh":break;case "filelink":case "filelinkm":$(b+"_filelink").html($("#global_data").data(d));break;default:f.val($("#global_data").data(d))}};
...
ga.pull.toPull=function(a,b,c,d,e){e&&"string"==typeof e&&0!=e.length&&"#__fields:repeat__"!=e||(e="");a=$("#global_data");var f=a.data("_pull_json"+e)||{},g=a.data("_pull_update"+e)||{};f[d]=0;"object"!=typeof g[d]&&(g[d]={});g[d][b]=c;a.data("_pull_json"+e,f);a.data("_pull_update"+e,g)};
...
jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(n),ga.repeat.data[a].repeater[n].value&&delete ga.repeat.data[a].repeater[n].value);break;case "listbox":g=b.replace(/-[0-9]+$/,"").replace(/^(.*)-([A-ZA-z0-9_]*)$/,"$2")+"-"+d;r=b+"-"+d;g=ga.repeat.children(a,g);for(k in g)n=r+"-"+k,ga.repeat.map[k]=n,h+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,n).replace("%%label%%",""),q+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,n),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[n]||
(ga.repeat.data[a].repeater[n]=jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(n),ga.repeat.data[a].repeater[n].value&&delete ga.repeat.data[a].repeater[n].value);break;default:return console.warn("ga.repeat.change( "+a+" , "+b+" ) type "+ga.repeat.data[a].repeater[b].type+" not supported"),!1}!/^<tr>/.test(h)&&/<\/tr>$/.test(h)&&(h="<tr>"+h);$(f+"-repeater").html(h);eval(q);ga.repeat.data[a].repeater[b].value=d;for(k=0;k<e.length;++k)ga.repeat.change(a,e[k],c);$("#global_data").data("_pull_json#"+
...
ga.data.nofcrefresh={};ga.data.dataURLtoFile=function(a,b){for(var c=a.split(","),d=c[0].match(/:(.*?);/)[1],c=atob(c[1]),e=c.length,f=new Uint8Array(e);e--;)f[e]=c.charCodeAt(e);return new File([f],b,{type:d})};
...
parseFloat(A.css("borderTopWidth"))+parseFloat(A.css("borderBottomWidth"))))):A.val(l)}else c?"_"==p.charAt(0)?/^_fs_/.test(p)&&ga.data.nofcrefresh[a]||("_message"==p&&ga.msg.box(l),"_question"==p&&ga.qr.question(a,b),"_question_answered"==p&&ga.qr.answered(a,b),"_question_timeout"==p&&ga.qr.timeout(a,b),/^_getinput/.test(p)&&"_getinput"==p&&ga.valuen.input(a,l),"_textarea"==p&&ga.data.textarea(r,l),"_airavata"==p&&ga.data.airavata(r,l),"_status"==p&&"complete"==l&&c(d,0,0)):(f||(x.append("<p>Unexpected results:</p>"),
f=1),x.append("<p>"+p+" => "+l+"</p>")):"_"==p.charAt(0)?/^_fs_/.test(p)&&ga.data.nofcrefresh[a]||($("#_state").data(p,l),g=1,"_status"==p&&(z.job_status=l),/^_getinput/.test(p)&&"_getinput"==p&&ga.valuen.input(a,l),"_textarea"==p&&ga.data.textarea(r,l),"_airavata"==p&&ga.data.airavata(r,l),"_loginverify"==p&&ga.login.verify(l),"_loginapprove"==p&&ga.login.approve(l)):"-close"==p?h=1:"-close2"==p?q=1:(f||(x.text(""),x.append("<p>Unexpected results:</p>"),e=f=1),x.append("<p>"+p+" => "+l+"</p>"))});
...
a[e]+"</em> has been <strong>denied</strong>.</p>";b.push(a[e]);break;case "pending":c+="<p>Your license request for <em>"+a[e]+"</em> is pending approval.</p>";b.push(a[e]);break;default:console.warn("ga.license() "+a[e]+" unknown status "+ga.license.data[a[e]].status),b.push(a[e])}else b.push(a[e]);return b.length?(ga.msg.box({icon:"warning.png",text:c,buttons:d}),!1):!0};ga.license.data={};
...
a[e]+"</em> has been <strong>denied</strong>.</p>";b.push(a[e]);break;case "pending":c+="<p>Your register request for <em>"+a[e]+"</em> is pending approval.</p>";b.push(a[e]);break;default:console.warn("ga.register() "+a[e]+" unknown status "+ga.register.data[a[e]].status),b.push(a[e])}else b.push(a[e]);return b.length?(ga.msg.box({icon:"warning.png",text:c,buttons:d}),!1):!0};ga.register.data={};
...
ga.calc.depthofdeps=function(a,b,c){var d,e,f=0;c=c||0;ga.calc.data[a].calc[b].dependents&&c++;if(99<c)return c;for(d=0;d<ga.calc.data[a].calc[b].dependents.length;++d)ga.calc.data[a].calc[ga.calc.data[a].calc[b].dependents[d]]&&(e=ga.calc.depthofdeps(a,ga.calc.data[a].calc[b].dependents[d],c),f<e&&(f=e));return c+f};
...
ga.calc.mktree=function(a,b){for(var c=[],d=null,e,f,g=0;g<a.length;++g){token=a[g];if(ga.calc.is_function_paren.test(token)){e=ga.calc.parensub(a.slice(g));if(e._error)return e;g+=e.newofs;token={op:token,args:[e=ga.calc.mktree(e.a)]};f=1;if(e._error)return e}else if(ga.calc.is_open_paren.test(token)){e=ga.calc.parensub(a.slice(g));if(e._error)return e;g+=e.newofs;token=ga.calc.mktree(e.a);if(token._error)return token;f=1}else f=0;f||ga.calc.is_atom.test(token)?(c.push(token),d&&(b?ga.calc.precedence[d]<
...
ga.bokeh.getdata=function(a,b){var c={},d=b.split(/\r?\n/),e=d.length,f,g=0;for(f=0;f<e;++f)if(g||-1!==d[f].indexOf("var docs_json ="))if(!g)g=1,c.docs_json=$.parseJSON(d[f].replace(/^\s*var\s+docs_json\s+=\s+/,"").replace(/(^'|('|);$)/g,""));else if(1==g&&-1!==d[f].indexOf("var render_items =")){c.render_items=$.parseJSON(d[f].replace(/^\s*var\s+render_items\s+=\s+/,"").replace(/;$/,""));break}return c};
...
ga.qr.question=function(a,b){console.log("ga.qr.question( "+a+", q )");var c="",d,e,f,g="",h,q=[],k={};b._uuid||(g+="no _uuid in received data. ");b._msgid||(g+="no _msgid in received data. ");b._question||(g+="no _question in received data. ");b._question.fields||(g+="no _question:fields in received data. ");if(g.length)return ga.qr.rerror(b,g);b._question.title&&(c+="<h3>"+b._question.title+"</h3>");b._question.text&&(c+=b._question.text);h=b._uuid+"-"+b._msgid;c+='<form id="'+h+'"><table>';for(d=
0;d<b._question.fields.length;++d)if(f=b._question.fields[d],f.id||(g+="No id in field "+d+". "),f.type||(g+="No type in field "+d+". "),f.id&&f.type)switch(k[f.id]&&(g+="Duplicate id in _question fields:"+f.id+". "),k[f.id]=1,f.type){case "label":c+="<tr><td colspan=2>";f.label&&(c+=f.label);c+="</td></tr>";break;case "text":c+="<tr><td>";f.label&&(c+='<label for="'+f.id+'">'+f.label+"</label>");c+='</td><td><input type="text" id="'+f.id+'"';f.required&&(c+=" required");f.readonly&&(c+=" readonly");
f["default"]&&(c+=' value="'+f["default"]+'"');f.pattern&&(c+=' pattern="'+f.pattern+'"');f.maxlength&&(c+=' maxlength="'+f.maxlength+'"');f.size&&(c+=' size="'+f.size+'"');c+="></td></tr>";break;case "checkbox":c+="<tr><td>";f.label&&(c+='<label for="'+f.id+'" class="highlight">'+f.label+"</label>");c+='</td><td><input type="checkbox" id="'+f.id+'"';f.checked&&(c+=" checked");f.readonly&&(c+=" readonly");c+="></td></tr>";break;case "listbox":c+="<tr><td>";f.label&&(c+='<label for="'+f.id+'" class="highlight">'+
f.label+"</label>");c+="</td>";c=f.size&&1<f.size?c+"</tr><tr><td colspan=2>":c+"<td>";c+='<select id="'+f.id+'"';f.fontfamily&&(c+=' style="font-family: '+f.fontfamily+';"');f.size&&(c+=" size="+f.size);c+=">";if(!f.values){g+="No values for listbox "+f.id+". ";break}for(e=0;e<f.values.length;++e)c+='<option value="'+e+'">'+f.values[e].replace(/ /g,"&nbsp")+"</option>";c+="</select></td></tr>";break;default:g+="Unknown or currently unsupported field:type "+f.type+". "}c+="</table></form>";if(b._question.buttons&&
b._question.buttons.length)for(d=0;d<b._question.buttons.length;++d)switch(e=b._question.buttons[d],typeof e){case "string":bid=e.replace(/\W/g,"").toLowerCase();k[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". ");k[bid]=1;q.push({id:bid,label:e,cb:ga.qr.cb,adata:[b,bid]});break;case "object":e.id?(k[e.id]&&(g+="Duplicate id in _question fields & buttons:"+e.id+". "),k[e.id]=1,e.id!=e.id.replace(/\W/g,"")&&(g+="Buttons array object entry "+(d+1)+" id is not pure alphanumeric. ")):
g+="Buttons array object entry "+(d+1)+" does not have an id. ";e.label||(g+="Buttons array object entry "+(d+1)+" does not have a label. ");if(!g.length){q.push({id:e.id,label:e.label,cb:ga.qr.cb,adata:[b,e.id]});break}break;default:g+="Buttons array entry "+(d+1)+" unknown type: "+typeof e+". "}else bid="ok",k[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". "),k[bid]=1,bid="cancel",k[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". "),k[bid]=1,q=[{id:"ok",label:"OK",
cb:ga.qr.cb,adata:[b,"ok"]},{id:"cancel",label:"Cancel",cb:ga.qr.cb,adata:[b,"cancel"]}];if(g.length)return ga.qr.rerror(b,g);console.log("qtext:"+c);ga.qr.openq[h]="open";ga.msg.box({icon:"question.png",text:c,eval:'$("#'+h+'").on("keyup keypress", function(e) { var code = e.keyCode || e.which;  if (code  == 13) { e.preventDefault(); return false; }});',buttons:q})};
ga.qr.cb=function(a,b){console.log("ga.qr.cb( data )");console.dir(a);console.dir(data);var c=a._uuid+"-"+a._msgid;if(ga.qr.openq[c]){switch(ga.qr.openq[c]){case "open":break;case "answered":ga.msg.box({icon:"information.png",text:"Question has already been answered in another session"});delete ga.qr.openq[c];return;case "timeout":ga.msg.box({icon:"information.png",text:"The time for answering a question has expired"});delete ga.qr.openq[c];return;default:ga.msg.box({icon:"toast.png",text:"Internal error, unknown message state"});
delete ga.qr.openq[c];return}delete ga.qr.openq[c];var d={};d._uuid=a._uuid;d._msgid=parseFloat(a._msgid);d._response={};d._response.button=b;a._question&&a._question.id&&(d._response.id=a._question.id);$("#"+c+" *").filter(":input").each(function(){console.dir(this);switch(this.type){case "text":case "select-one":d._response[this.id]=this.value;break;case "checkbox":this.checked&&(d._response[this.id]=!0)}});ga.qr.post(d)}};
ga.qr.answered=function(a,b){console.log("ga.qr.answered( moq, q )");console.dir(b);var c;b._uuid&&b._msgid&&(c=b._uuid+"-"+b._msgid,ga.qr.openq[c]&&(ga.qr.openq[c]="answered"))};ga.qr.timeout=function(a,b){console.log("ga.qr.timeout( moq, q )");console.dir(b);var c;b._uuid&&b._msgid&&(c=b._uuid+"-"+b._msgid,ga.qr.openq[c]&&(ga.qr.openq[c]="timeout"))};
ga.qr.rerror=function(a,b){console.log("ga.qr.rerror( q, "+b+" )");console.dir(a);console.dir(data);var c={};c._uuid=a._uuid;c._msgid=parseFloat(a._msgid);c._response={};c._response.error=b;a._question&&a._question.id&&(c._response.id=a._question.id);ga.qr.post(c);ga.msg.box({icon:"toast.png",text:b})};
ga.qr.post=function(a){console.log("ga.qr.post( r )");console.dir(a);$.ajax({url:ga.qr.url,data:{_window:window.name,_data:a},dataType:"json",method:"POST"}).success(function(a){console.log("ajax delete done");a.error&&a.error.length&&ga.msg.box({icon:"toast.png",text:"ajax data error: "+a.error,buttons:[{id:"ok",label:"OK"}]})}).error(function(a){console.log("ajax error");console.dir(a);ga.msg.box({icon:"toast.png",text:"ajax error: "+a.statusText,buttons:[{id:"ok",label:"OK"}]})})};ga.msg={};
ga.msg.box=function(a,b,c){c=c||3;if(!b&&$(".modalDialog"+c).hasClass("modalDialog"+c+"_on")){b=$("#_state").data("__msgs");if(!b||b&&"[object Array]"!=Object.prototype.toString.call(b))b=[];b.push(a);$("#_state").data("__msgs",b)}else{a.icon?$("#configbody"+c).html("<table style='width:95%;vertical-align:middle'><tr><td style='width:10%'><img src='pngs/"+a.icon+"' width=40px></td><td style='text-align:center'>"+a.text+"</td></tr></table>"):$("#configbody"+c).html("<center>"+a.text+"</center>");if(a.buttons){tmp=
"<center><table><tr>";for(i=0;i<a.buttons.length;i++)tmp=tmp+'<td><button id="_mbb_'+a.buttons[i].id+'">'+a.buttons[i].label+"</button></td>";tmp+="</tr></table><center>";$("#configbody"+c).append(tmp);for(i=0;i<a.buttons.length;i++)if(a.buttons[i].cb)if(a.buttons[i].adata)switch(a.buttons[i].adata.length){case 2:$("#_mbb_"+a.buttons[i].id).off().one("click",a.buttons[i],function(a){a.data.cb(a.data.adata[0],a.data.adata[1]);ga.msg.close(c)});break;case 3:$("#_mbb_"+a.buttons[i].id).off().one("click",
a.buttons[i],function(a){a.data.cb(a.data.adata[0],a.data.adata[1],a.data.adata[2]);ga.msg.close(c)});break;case 4:$("#_mbb_"+a.buttons[i].id).off().one("click",a.buttons[i],function(a){a.data.cb(a.data.adata[0],a.data.adata[1],a.data.adata[2],a.data.adata[3]);ga.msg.close(c)});break;case 5:$("#_mbb_"+a.buttons[i].id).off().one("click",a.buttons[i],function(a){a.data.cb(a.data.adata[0],a.data.adata[1],a.data.adata[2],a.data.adata[3],a.data.adata[4]);ga.msg.close(c)});break;case 6:$("#_mbb_"+a.buttons[i].id).off().one("click",
a.buttons[i],function(a){a.data.cb(a.data.adata[0],a.data.adata[1],a.data.adata[2],a.data.adata[3],a.data.adata[4],a.data.adata[5]);ga.msg.close(c)});break;default:console.warn("in ga.msg.box unsupported number of adata arguments "+a.buttons[i].adata.length)}else $("#_mbb_"+a.buttons[i].id).off().one("click",a.buttons[i],function(a){a.data.cb(a.data.data);ga.msg.close(c)});else $("#_mbb_"+a.buttons[i].id).off().one("click",a.buttons[i],function(a){ga.msg.close(c)})}a.ptext&&$("#configbody"+c).append(a.ptext);
a.eval&&eval(a.eval);ga.repeats.save();$(".modalDialog"+c).addClass("modalDialog"+c+"_on")}};ga.msg.close1=function(){ga.repeats.restore();$(".modalDialog").removeClass("modalDialog_on");setTimeout(function(){$("#configbody").html("")},400);ga.usesplash&&setTimeout(function(){splashlogin()},500)};ga.msg.close2=function(){ga.repeats.restore();$(".modalDialog2").removeClass("modalDialog2_on");setTimeout(function(){$("#configbody2").html("")},400);ga.usesplash&&setTimeout(function(){splashlogin()},500)};
