...
ga.data.update=function(a,b,c,d){var e=0,f=0,g=0,h=0,q=0,k=a+"_output",r="#"+k,n=$(r),z={},u=r+"_msgs",x=$(u),m,v,y,B,w,C,A;c&&($("#"+a+"_progress").html(""),x.text(""));$.each(b,function(p,l){A=n.find("#"+p);if(A.length)switch(e||(x.text(""),e=1),A.attr("type")){case "plot2d":m="#"+p;ga.value.plot2d.zstack.reset(m);if(l.data){ga.value.set.plot2d(m,l.options);ga.plotglobal=l.options;ga.dataplotglobal=l.data;$.plot(m,l.data,ga.value.get.plot2d.plot_options(m,l.options));var u=null;$(m).bind("plothover",
function(a,b,c){c?u!=c.dataIndex&&(u=c.dataIndex,$("#rtooltip").remove(),c.datapoint[0].toFixed(2),c.datapoint[1].toFixed(2),c.series.rdata.length&&showTooltip(c.pageX,c.pageY,"Residue: "+c.series.rdata[c.dataIndex],c.series.color)):($("#rtooltip").remove(),u=null)})}else $.plot(m,l,ga.value.get.plot2d.plot_options(m));$(m+"_savetofile").length&&$(m+"_savetofile").removeClass("hidden");$(m+"_changescalex").length&&($(m+"_changescalex").removeClass("hidden"),"log"==l.options.xscale?$(m+"_changescalex_message").html("X-log"):
$(m+"_changescalex_message").html("X-lin"));$(m+"_changescaley").length&&($(m+"_changescaley").removeClass("hidden"),"log"==l.options.yscale?$(m+"_changescaley_message").html("Y-log"):$(m+"_changescaley_message").html("Y-lin"));if(ga.value.settings[m].selzoom||l.options&&l.options.selection&&l.options.selection.mode&&"xy"==l.options.selection.mode)$(m).on("plotselected",{htag:m,data:l.data?l.data:l,options:l.data?ga.value.get.plot2d.plot_options(m,l.options):ga.value.get.plot2d.plot_options(m)},function(a,
b){1E-5>b.xaxis.to-b.xaxis.from&&(b.xaxis.to=b.xaxis.from+1E-5);1E-5>b.yaxis.to-b.yaxis.from&&(b.yaxis.to=b.yaxis.from+1E-5);ga.value.plot2d.zstack.dopush(a.data.htag,b);$.plot(a.data.htag,a.data.data,$.extend(!0,{},a.data.options,{xaxis:{min:b.xaxis.from,max:b.xaxis.to},yaxis:{min:b.yaxis.from,max:b.yaxis.to}}))}).on("contextmenu",{htag:m,data:l.data?l.data:l,options:l.data?ga.value.get.plot2d.plot_options(m,l.options):ga.value.get.plot2d.plot_options(m)},function(a){a.preventDefault();var b=ga.value.plot2d.zstack.dopop(a.data.htag);
b?$.plot(a.data.htag,a.data.data,$.extend(!0,{},a.data.options,{xaxis:{min:b.xaxis.from,max:b.xaxis.to},yaxis:{min:b.yaxis.from,max:b.yaxis.to}})):$.plot(a.data.htag,a.data.data,a.data.options)});y=k+":#"+p+":last_value";$("#global_data").data(y,l);break;case "bokeh":ga.bokeh.render(a,p,l);break;case "atomicstructure":y=k+":#"+p+":last_value";C=l.file?l.file:l;_jmol_info[p].script="set background ["+ga.colors.background+"]; set zoomlarge false;set echo top center;echo loading "+C.split("/").pop()+
';refresh;load "'+C+'";';ga.set(a+":jsmoladd")&&(_jmol_info[p].script+=ga.set(a+":jsmoladd"));l.script&&(_jmol_info[p].script+=";"+l.script);$("#global_data").data(y,_jmol_info[p].script);$("#"+p).html(Jmol.getAppletHtml("jmolApplet"+p,_jmol_info[p]));break;case "checkbox":case "radio":A.prop("checked",!0);break;case "div":A.html(l);break;case "video":v=$("#"+p);w="<video ";v.attr("data-width")&&(w+=' width="'+v.attr("data-width")+'"');v.attr("data-height")&&(w+=' height="'+v.attr("data-height")+
...
"_question"==p&&ga.qr.question(a,b),"_question_answered"==p&&ga.qr.answered(a,b),"_question_timeout"==p&&ga.qr.timeout(a,b),/^_getinput/.test(p)&&"_getinput"==p&&ga.valuen.input(a,l),"_textarea"==p&&ga.data.textarea(r,l),"_airavata"==p&&ga.data.airavata(r,l),"_status"==p&&"complete"==l&&c(d,0,0)):(f||(x.append("<p>Unexpected results:</p>"),f=1),x.append("<p>"+p+" => "+l+"</p>")):"_"==p.charAt(0)?/^_fs_/.test(p)&&ga.data.nofcrefresh[a]||($("#_state").data(p,l),g=1,"_status"==p&&(z.job_status=l),/^_getinput/.test(p)&&
"_getinput"==p&&ga.valuen.input(a,l),"_textarea"==p&&ga.data.textarea(r,l),"_airavata"==p&&ga.data.airavata(r,l),"_loginverify"==p&&ga.login.verify(l),"_loginapprove"==p&&ga.login.approve(l)):"-close"==p?h=1:"-close2"==p?q=1:(f||(x.text(""),x.append("<p>Unexpected results:</p>"),e=f=1),x.append("<p>"+p+" => "+l+"</p>"))});ga.value.saveLastValues(k);ga.value.saveLastValue(k,u);$(r+"_progress").html("");g&&syncState();h&&ga.msg.close(1);q&&ga.msg.close(2);return z};
...
'"');f.pattern&&(c+=' pattern="'+f.pattern+'"');f.maxlength&&(c+=' maxlength="'+f.maxlength+'"');f.size&&(c+=' size="'+f.size+'"');c+="></td></tr>";break;case "checkbox":c+="<tr><td>";f.label&&(c+='<label for="'+f.id+'" class="highlight">'+f.label+"</label>");c+='</td><td><input type="checkbox" id="'+f.id+'"';f.checked&&(c+=" checked");f.readonly&&(c+=" readonly");c+="></td></tr>";break;case "listbox":c+="<tr><td>";f.label&&(c+='<label for="'+f.id+'" class="highlight">'+f.label+"</label>");c+="</td>";
c=f.size&&1<f.size?c+"</tr><tr><td colspan=2>":c+"<td>";c+='<select id="'+f.id+'"';f.fontfamily&&(c+=' style="font-family: '+f.fontfamily+';"');f.size&&(c+=" size="+f.size);c+=">";if(!f.values){g+="No values for listbox "+f.id+". ";break}for(e=0;e<f.values.length;++e)c+='<option value="'+e+'">'+f.values[e].replace(/ /g,"&nbsp")+"</option>";c+="</select></td></tr>";break;default:g+="Unknown or currently unsupported field:type "+f.type+". "}c+="</table></form>";if(b._question.buttons&&b._question.buttons.length)for(d=
0;d<b._question.buttons.length;++d)switch(e=b._question.buttons[d],typeof e){case "string":bid=e.replace(/\W/g,"").toLowerCase();k[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". ");k[bid]=1;q.push({id:bid,label:e,cb:ga.qr.cb,adata:[b,bid]});break;case "object":e.id?(k[e.id]&&(g+="Duplicate id in _question fields & buttons:"+e.id+". "),k[e.id]=1,e.id!=e.id.replace(/\W/g,"")&&(g+="Buttons array object entry "+(d+1)+" id is not pure alphanumeric. ")):g+="Buttons array object entry "+
(d+1)+" does not have an id. ";e.label||(g+="Buttons array object entry "+(d+1)+" does not have a label. ");if(!g.length){q.push({id:e.id,label:e.label,cb:ga.qr.cb,adata:[b,e.id]});break}break;default:g+="Buttons array entry "+(d+1)+" unknown type: "+typeof e+". "}else bid="ok",k[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". "),k[bid]=1,bid="cancel",k[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". "),k[bid]=1,q=[{id:"ok",label:"OK",cb:ga.qr.cb,adata:[b,"ok"]},{id:"cancel",
label:"Cancel",cb:ga.qr.cb,adata:[b,"cancel"]}];if(g.length)return ga.qr.rerror(b,g);ga.qr.openq[h]="open";ga.msg.box({icon:"question.png",text:c,eval:'$("#'+h+'").on("keyup keypress", function(e) { var code = e.keyCode || e.which;  if (code  == 13) { e.preventDefault(); return false; }});',buttons:q})};
