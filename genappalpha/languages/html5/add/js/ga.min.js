...
$("#global_data").data(d)?(_jmol_info[b].script=$("#global_data").data(d),e.html(Jmol.getAppletHtml("jmolApplet"+b,_jmol_info[b]))):e.empty();break;case "plot2d":d=gd.data(d);d.data?(ga.value.set.plot2d(b,d.options),e.plot(d.data,ga.value.get.plot2d.plot_options(b,d.options))):e.plot(d,ga.value.get.plot2d.plot_options(b));break;case "bokeh":break;case "filelink":case "filelinkm":$(b+"_filelink").html($("#global_data").data(d));break;default:e.val($("#global_data").data(d))}};
...
break;case "bokeh":ga.bokeh.reset(a,b.replace(/^#/,""));break;case "plot3d":console.log("reset default value for plot3d: "+b);Plotly.purge(b.replace(/^#/,""));$(b+"_showcollapse").length&&$(b+"_showcollapse").addClass("hidden");break;case "image":c.empty();break;case "video":c.empty();break;case "atomicstructure":b.replace(/^#/,"");$("#global_data").data(a+":"+b+":last_value","");$(b).empty();break;default:c.val(c.attr("value"))}ga.value.saveLastValue(a,b);$(b+"_msg").empty()};
...
"_getinput"==p&&ga.valuen.input(a,l),"_textarea"==p&&ga.data.textarea(r,l),"_airavata"==p&&ga.data.airavata(r,l),"_loginverify"==p&&ga.login.verify(l),"_loginapprove"==p&&ga.login.approve(l)):"-close"==p?h=1:"-close2"==p?q=1:(e||(x.text(""),x.append("<p>Unexpected results:</p>"),f=e=1),x.append("<p>"+p+" => "+l+"</p>"))});ga.value.saveLastValues(k);ga.value.saveLastValue(k,u);$(r+"_progress").empty();g&&syncState();h&&ga.msg.close(1);q&&ga.msg.close(2);return z};
