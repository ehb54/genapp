...
ga.value.processInputfromFiles=function(a,b,c){for(var d=a.trim().split(/[\r\n]+/g),e=[],f=0,g=0;g<d.length;g++){var h=d[g].trim().split(/\s+/),f=f+h.length;e.push(f)}a=a.trim().split(/\s+/);d=0;switch(b){case "whitespaceseparated":case "whitespaceseparated_reverselogic":if(a.length==c.length)for(g=0;g<a.length;g++){for(f=0;f<e.length;f++)if(g+1<=e[f]){d=f+1;break}switch($("#"+c[g]).attr("type")){case "checkbox":f=/^(0|1|n|y|true|false|t|f|yes|no)$/;if(!f.test(a[g].toLowerCase())){messagebox({icon:"warning.png",
...
ga.repeats.save=function(){ga.repeats.cache._jmol_info=_jmol_info||{};ga.repeats.cache.specproj=ga.specproj.data||[]};ga.repeats.restore=function(){_jmol_info=ga.repeats.cache._jmol_info;ga.specproj.data=ga.repeats.cache.specproj};ga.repeat={};ga.repeat.data={};
...
h+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,l).replace("%%label%%",""),n+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,l),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[l]||(ga.repeat.data[a].repeater[l]=jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(l),ga.repeat.data[a].repeater[l].value&&delete ga.repeat.data[a].repeater[l].value);break;default:return console.warn("ga.repeat.change( "+a+" , "+b+" ) type "+ga.repeat.data[a].repeater[b].type+" not supported"),
!1}!/^<tr>/.test(h)&&/<\/tr>$/.test(h)&&(h="<tr>"+h);$(f+"-repeater").html(h);eval(n);ga.repeat.data[a].repeater[b].value=d;for(k=0;k<e.length;++k)ga.repeat.change(a,e[k],c);$("#global_data").data("_pull_json#"+b)&&ga.pull.doPull("#"+b);resetHoverHelp()};ga.hide=function(a,b){ga.hide.data=ga.hide.data||{};ga.hide.data[a]=ga.hide.data[a]||{};ga.hide.data[a][b]=ga.hide.data[a][b]||{};ga.hide.data[a][b].active=1};ga.hide.data={};
