...
ga.airavata.select=function(a,b,c,d){var e=ga.airavata.data,f="",h=[],g,q,k;if("__resource__"==a&&!e.defaultresource||"airavata"!=a&&"__resource__"!=a)return"notused";if(!e.resources||!e.resources.length)return messagebox({icon:"warning.png",text:"No resources currently enabled for Airavata submission"}),"abort";if(1==e.resources.length)return Object.keys(e.resources[k])[0];a="__airavataselect__"!=b?b:e.select.length?e.select:"random";switch(a){case "random":return k=Math.floor(e.resources.length*
Math.random()),Object.keys(e.resources[k])[0];case "choose":h.push({id:"submit_module",label:"Submit",data:[c,d,e.resources],cb:function(a){a[0](a[1],Object.keys(a[2][$("#airavata input[name=selectresource]:checked").val()])[0])}});f='<h3>Select a compute resource and press submit</h3><form id="airavata"><table>';for(g in e.resources)for(q in e.resources[g])f+='<tr><td><input type="radio" name="selectresource" id="airavata_'+g+'" value="'+g+'"'+(0==g?'checked="checked"':"")+'></td><td class="hoverhighlight" style="text-align:left"><label for="airavata_'+
g+'">'+e.resources[g][q]+"</label></td></tr>";messagebox({icon:"question.png",text:f+"</table>",buttons:h});return"deferred";default:return messagebox({icon:"toast.png",text:"ga.airavata.select, unknown selection type '"+a+"'"}),"abort"}};ga.resource={};ga.xsede={};ga.resource.xsedeprojecttokeys=function(){var a;ga.resource.xsedepkeys={};for(a in ga.resource.xsedeproject)ga.resource.xsedepkeys[ga.resource.xsedeproject[a]]=1};
...
ga.value.processInputfromFiles=function(a,b,c,d){var e=a.trim().split(/[\r\n]+/g),f=[],h=0;c=ga.repeat.map.convert(c);for(var g=0;g<e.length;g++){var q=e[g].trim().split(/\s+/),h=h+q.length;f.push(h)}var h=[],k=[],r=[],n=[],z=[],u=[],q=[],x=0;switch(b){case "whitespace_formulchcompost":a=[];for(g=0;g<e.length;g++)f=e[g].split("#")[0],f=f.replace(/\s{2,}/g," "),f=f.replace(/\t/g," "),f=f.toString().trim().replace(/(\r\n|\n|\r)/g,""),a.push(f);for(var e=parseInt(a[1]),m,g=0;g<a.length;g++)if(2<g&&g<
3+e)for(m=a[g].trim().split(/\s+/),f=0;f<m.length;f++)k.push(m[f]);else h.push(a[g]);q.push(k);for(g=0;g<c.length;g++)switch($("#"+c[g]).attr("type")){case "text":k=new RegExp($("#"+c[g]).attr("pattern"));if(!k.test(h[g])){messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+x+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;case "number":k=[h[g]];
...
[];for(g=0;g<e.length;g++)f=e[g].split("#")[0],f=f.replace(/\s{2,}/g," "),f=f.replace(/\t/g," "),f=f.toString().trim().replace(/(\r\n|\n|\r)/g,""),a.push(f);for(var e=parseInt(a[1]),v=parseInt(a[2+e]),y=parseInt(a[5+e+v]),B=parseInt(a[9+e+v+y]),g=0;g<a.length;g++)if(1<g&&g<2+e)for(m=a[g].trim().split(/\s+/),f=0;f<m.length;f++)k.push(m[f]);else g>2+e&&g<3+e+v?(m=a[g].trim().split(/\s+/),r.push(m[1]),r.push(m[2]),r.push(m[0]),r.push(m[3])):g>5+e+v&&g<6+e+v+y?(m=a[g].trim().split(/\s+/),n.push(m[1]),
n.push(m[2]),n.push(m[0]),n.push(m[3])):g>8+e+v+y&&g<9+e+v+y+B?(m=a[g].trim().split(/\s+/),z.push(m[1]),z.push(m[2]),z.push(m[0]),z.push(m[3])):h.push(a[g]);u.push(k);u.push(r);u.push(n);u.push(z);for(g=f=0;g<c.length;g++)if($("#"+c[g]).length)switch($("#"+c[g]).data("repeater")&&(q.push(u[f]),f++),$("#"+c[g]).attr("type")){case "text":k=new RegExp($("#"+c[g]).attr("pattern"));if(!k.test(h[g])){messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+x+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",
...
jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(n),ga.repeat.data[a].repeater[n].value&&delete ga.repeat.data[a].repeater[n].value);break;case "listbox":h=b.replace(/-[0-9]+$/,"").replace(/^(.*)-([A-ZA-z0-9_]*)$/,"$2")+"-"+d;r=b+"-"+d;h=ga.repeat.children(a,h);for(k in h)n=r+"-"+k,ga.repeat.map[k]=n,g+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,n).replace("%%label%%",""),q+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,n),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[n]||
(ga.repeat.data[a].repeater[n]=jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(n),ga.repeat.data[a].repeater[n].value&&delete ga.repeat.data[a].repeater[n].value);break;default:return console.warn("ga.repeat.change( "+a+" , "+b+" ) type "+ga.repeat.data[a].repeater[b].type+" not supported"),!1}!/^<tr>/.test(g)&&/<\/tr>$/.test(g)&&(g="<tr>"+g);$(f+"-repeater").html(g);eval(q);ga.repeat.data[a].repeater[b].value=d;for(k=0;k<e.length;++k)ga.repeat.change(a,e[k],c);$("#global_data").data("_pull_json#"+
...
f=1),x.append("<p>"+p+" => "+l+"</p>")):"_"==p.charAt(0)?/^_fs_/.test(p)&&ga.data.nofcrefresh[a]||($("#_state").data(p,l),h=1,"_status"==p&&(z.job_status=l),/^_getinput/.test(p)&&"_getinput"==p&&ga.valuen.input(a,l),"_textarea"==p&&ga.data.textarea(r,l),"_airavata"==p&&ga.data.airavata(r,l),"_loginverify"==p&&ga.login.verify(l),"_loginapprove"==p&&ga.login.approve(l)):"-close"==p?g=1:"-close2"==p?q=1:(f||(x.text(""),x.append("<p>Unexpected results:</p>"),e=f=1),x.append("<p>"+p+" => "+l+"</p>"))});
ga.value.saveLastValues(k);ga.value.saveLastValue(k,u);$(r+"_progress").html("");h&&syncState();g&&closeModal();q&&closeModal2();return z};
...
ga.valuen.input=function(a,b){var c=$("#"+a),d=c.find(":input").get(),e={},f;if(b){do f=!1,$.each(d,function(){var h,g,q=$(this),k,r;if(q.attr("data-repeater")&&!e[this.name]){e[this.name]=!0;if(this.name&&(b[this.name]||/checkbox|radio/i.test(this.type))){g=b[this.name];"[object Array]"!==Object.prototype.toString.call(g)&&(g=[g]);if(/checkbox|radio/i.test(this.type)){k=q.val();r=!1;if(g)for(h=0;h<g.length;h++)if(g[h]==k){r=!0;break}q.prop("checked",r)}else q.val(g[0]);f=!0;ga.repeat.change(a,this.name,
...
ga.qr.question=function(a,b){console.log("ga.qr.question( "+a+", q )");var c="",d,e,f="",h;b._uuid||(f+="no _uuid in received data. ");b._msgid||(f+="no _msgid in received data. ");b._question||(f+="no _question in received data. ");b._question.fields||(f+="no _question:fields in received data. ");if(f.length)return ga.qr.rerror(b,f);b._question.title&&(c+="<h3>"+b._question.title+"</h3>");b._question.text&&(c+=b._question.text);h=b._uuid+"-"+b._msgid;c+='<form id="'+h+'"><table>';for(d=0;d<b._question.fields.length;++d)if(e=
b._question.fields[d],e.id||(f+="No id in field "+d+". "),e.type||(f+="No type in field "+d+". "),e.id&&e.type)switch(e.type){case "label":c+="<tr><td colspan=2>";e.label&&(c+=e.label);c+="</td></tr>";break;case "text":c+="<tr><td>";e.label&&(c+='<label for="'+e.id+'">'+e.label+"</label>");c+='</td><td><input type="text" id="'+e.id+'"';e.required&&(c+=" required");e.readonly&&(c+=" readonly");e["default"]&&(c+=' value="'+e["default"]+'"');e.pattern&&(c+=' pattern="'+e.pattern+'"');e.maxlength&&(c+=
' maxlength="'+e.maxlength+'"');e.size&&(c+=' size="'+e.size+'"');c+="></td></tr>";break;case "checkbox":c+="<tr><td>";e.label&&(c+='<label for="'+e.id+'" class="highlight">'+e.label+"</label>");c+='</td><td><input type="checkbox" id="'+e.id+'"';e.checked&&(c+=" checked");e.readonly&&(c+=" readonly");c+="></td></tr>";break;default:f+="Unknown or currently unsupported field:type "+e.fype+". "}c+="</table></form>";if(f.length)return ga.qr.rerror(b,f);console.log("qtext:"+c);ga.qr.openq[h]="open";messagebox({icon:"question.png",
text:c,eval:'$("#'+h+'").on("keyup keypress", function(e) { var code = e.keyCode || e.which;  if (code  == 13) { e.preventDefault(); return false; }});',buttons:[{id:"ok",label:"OK",cb:ga.qr.cb,adata:[b,"ok"]},{id:"cancel",label:"Cancel",cb:ga.qr.cb,adata:[b,"cancel"]}]})};
