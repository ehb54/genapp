...
ga.data.nofcrefresh={};function dataURLtoFile(a,b){for(var c=a.split(","),d=c[0].match(/:(.*?);/)[1],c=atob(c[1]),e=c.length,f=new Uint8Array(e);e--;)f[e]=c.charCodeAt(e);return new File([f],b,{type:d})}
function create_image_htmltocanvas(a){if($("#"+a+"_savetofile").length){var b=document.getElementById(a+"_savetofile"),c=$("#"+a+"_div");html2canvas(c.get(0),{background:"#ffffff",onrendered:function(c){image=c.toDataURL("image/png").replace("image/png","image/octet-stream");b.href=image;$("#"+a+"_savetofile").removeClass("hidden")}})}}
function create_image(a,b){if($("#"+a+"_savetofile").length){var c=document.getElementById(a+"_savetofile"),d=b.getCanvas().toDataURL("image/png").replace("image/png","image/octet-stream"),d=dataURLtoFile(d,"plot.png");c.href=URL.createObjectURL(d);$("#"+a+"_savetofile").removeClass("hidden")}}
...
$(n+"_changescale").length&&$(n+"_changescale").removeClass("hidden");create_image_htmltocanvas(b);if(ga.value.settings[n].selzoom||l.options&&l.options.selection&&l.options.selection.mode&&"xy"==l.options.selection.mode)$(n).on("plotselected",{htag:n,data:l.data?l.data:l,options:l.data?ga.value.get.plot2d.plot_options(n,l.options):ga.value.get.plot2d.plot_options(n)},function(a,b){1E-5>b.xaxis.to-b.xaxis.from&&(b.xaxis.to=b.xaxis.from+1E-5);1E-5>b.yaxis.to-b.yaxis.from&&(b.yaxis.to=b.yaxis.from+
