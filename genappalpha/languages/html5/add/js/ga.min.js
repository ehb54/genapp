...
ga.admin.ajax.group.cb=function(a,b,c,d,e){$.get(ga.admin.ajax.url,{tagmode:"any",format:"json",_window:window.name,_logon:$("#_state").data("_logon"),_cmd:a,_name:b,_id:c,_manageid:d,_group:$("#sys_musergrp_text").val()}).done(function(a,b,c){a=JSON.parse(a.split(/\r?\n/)[1]);"true"!=a.success&&messagebox({icon:"toast.png",text:a.error?a.error:"unknown error"});a._submitid&&$("#"+a._submitid).trigger("click")}).fail(function(a,b,c){messagebox({icon:"toast.png",text:"Error: system user management backend command failed to run: "+
c})})};ga.admin.ajax.jobview=function(a,b,c,d){$("#configbody").load("etc/userjob.html",function(){$("#jobtext_label").html("Jobs for "+b);$.ajax({url:ga.jc.url,data:{_window:window.name,_asuser:b}}).success(function(a){$("#seluserjobs").html(a)}).error(function(a){$("#seluserjobs").html("error:"+data)})});$(".modalDialog").addClass("modalDialog_on")};
