...
f.label+"</label>");c+="</td>";f.size&&1<f.size&&(c+="</tr><tr>");c+='<td><select id="'+f.id+'"';f.size&&(c+=" size="+f.size);c+=">";if(!f.values){g+="No values for listbox "+f.id+". ";break}for(e=0;e<f.values.length;++e)c+='<option value="'+e+'">'+f.values[e]+"</option>";c+="</select></td></tr>";break;default:g+="Unknown or currently unsupported field:type "+f.type+". "}c+="</table></form>";if(b._question.buttons&&b._question.buttons.length)for(d=0;d<b._question.buttons.length;++d)switch(e=b._question.buttons[d],
typeof e){case "string":bid=e.replace(/\W/g,"").toLowerCase();k[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". ");k[bid]=1;q.push({id:bid,label:e,cb:ga.qr.cb,adata:[b,bid]});break;case "object":e.id?(k[e.id]&&(g+="Duplicate id in _question fields & buttons:"+e.id+". "),k[e.id]=1,e.id!=e.id.replace(/\W/g,"")&&(g+="Buttons array object entry "+(d+1)+" id is not pure alphanumeric. ")):g+="Buttons array object entry "+(d+1)+" does not have an id. ";e.label||(g+="Buttons array object entry "+
(d+1)+" does not have a label. ");if(!g.length){q.push({id:e.id,label:e.label,cb:ga.qr.cb,adata:[b,e.id]});break}break;default:g+="Buttons array entry "+(d+1)+" unknown type: "+typeof e+". "}else bid="ok",k[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". "),k[bid]=1,bid="cancel",k[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". "),k[bid]=1,q=[{id:"ok",label:"OK",cb:ga.qr.cb,adata:[b,"ok"]},{id:"cancel",label:"Cancel",cb:ga.qr.cb,adata:[b,"cancel"]}];if(g.length)return ga.qr.rerror(b,
g);console.log("qtext:"+c);ga.qr.openq[h]="open";ga.msg.box({icon:"question.png",text:c,eval:'$("#'+h+'").on("keyup keypress", function(e) { var code = e.keyCode || e.which;  if (code  == 13) { e.preventDefault(); return false; }});',buttons:q})};
