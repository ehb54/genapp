...
ga.value.processInputfromFiles=function(a,b,c,d){var e=a.trim().split(/[\r\n]+/g),g=[],h=0;c=ga.repeat.map.convert(c);for(var f=0;f<e.length;f++){var m=e[f].trim().split(/\s+/),h=h+m.length;g.push(h)}var h=[],k=[],p=[],l=[],z=[],r=[],m=[],w=0;switch(b){case "whitespace_formulchcompost":a=[];for(f=0;f<e.length;f++)g=e[f].split("#")[0],g=g.replace(/\s{2,}/g," "),g=g.replace(/\t/g," "),g=g.toString().trim().replace(/(\r\n|\n|\r)/g,""),a.push(g);for(var e=parseInt(a[1]),q,f=0;f<a.length;f++)if(2<f&&f<
3+e)for(q=a[f].trim().split(/\s+/),g=0;g<q.length;g++)k.push(q[g]);else h.push(a[f]);m.push(k);for(f=0;f<c.length;f++)switch($("#"+c[f]).attr("type")){case "text":k=new RegExp($("#"+c[f]).attr("pattern"));if(!k.test(h[f])){messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+w+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;case "number":k=[h[f]];
if(ga.value.checkFloatIntOK("#"+c[f],k))h[f]=k[0],console.log("Number is: "+h[f]);else{messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+w+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;default:messagebox({icon:"warning.png",text:"Selected input type is currently not supported. Contact the developer",buttons:[{id:"ok",label:"OK"}]});return}break;
case "whitespace_formulchrg":case "whitespace_formulchcontrast":a=[];for(f=0;f<e.length;f++)g=e[f].split("#")[0],g=g.replace(/\s{2,}/g," "),g=g.replace(/\t/g," "),g=g.toString().trim().replace(/(\r\n|\n|\r)/g,""),a.push(g);for(var e=parseInt(a[1]),u=parseInt(a[2+e]),y=parseInt(a[5+e+u]),A=parseInt(a[9+e+u+y]),f=0;f<a.length;f++)if(1<f&&f<2+e)for(q=a[f].trim().split(/\s+/),g=0;g<q.length;g++)k.push(q[g]);else f>2+e&&f<3+e+u?(q=a[f].trim().split(/\s+/),p.push(q[1]),p.push(q[2]),p.push(q[0]),p.push(q[3])):
f>5+e+u&&f<6+e+u+y?(q=a[f].trim().split(/\s+/),l.push(q[1]),l.push(q[2]),l.push(q[0]),l.push(q[3])):f>8+e+u+y&&f<9+e+u+y+A?(q=a[f].trim().split(/\s+/),z.push(q[1]),z.push(q[2]),z.push(q[0]),z.push(q[3])):h.push(a[f]);r.push(k);r.push(p);r.push(l);r.push(z);for(f=g=0;f<c.length;f++)if($("#"+c[f]).length)switch($("#"+c[f]).data("repeater")&&(m.push(r[g]),g++),$("#"+c[f]).attr("type")){case "text":k=new RegExp($("#"+c[f]).attr("pattern"));if(!k.test(h[f])){messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+
w+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;case "number":k=[h[f]];if(ga.value.checkFloatIntOK("#"+c[f],k))h[f]=k[0],console.log("Number is: "+h[f]);else{messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+w+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",
label:"OK"}]});return}break;default:messagebox({icon:"warning.png",text:"Selected input type is currently not supported. Contact the developer",buttons:[{id:"ok",label:"OK"}]});return}else g++;break;case "whitespaceseparated":case "whitespaceseparated_reverselogic":h=a.trim().split(/\s+/);if(h.length==c.length)for(f=0;f<h.length;f++){for(r=0;r<g.length;r++)if(f+1<=g[r]){w=r+1;break}switch($("#"+c[f]).attr("type")){case "checkbox":k=/^(0|1|n|y|true|false|t|f|yes|no)$/;if(!k.test(h[f].toLowerCase())){messagebox({icon:"warning.png",
text:"Wrong format of the input file! Checkbox input value on the line #"+w+" is not valid. Options are: [1 | 0 | yes | no | true | false | t | f | T | F | y | n | Y | N ]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;case "number":k=[h[f]];if(ga.value.checkFloatIntOK("#"+c[f],k))h[f]=k[0];else{messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+w+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",
buttons:[{id:"ok",label:"OK"}]});return}break;case "text":k=new RegExp($("#"+c[f]).attr("pattern"));if(!k.test(h[f])){messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+w+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;default:messagebox({icon:"warning.png",text:"Selected input type is currently not supported. Contact the developer",buttons:[{id:"ok",
...
c[f]).prop("checked",!0);break;case "1":case "true":case "t":case "yes":case "y":case "r":$("#"+c[f]).prop("checked",!1);break}else switch(h[f].toLowerCase()){case "0":case "false":case "f":case "n":case "no":$("#"+c[f]).prop("checked",!1);break;case "1":case "true":case "t":case "yes":case "y":case "r":$("#"+c[f]).prop("checked",!0);break}default:$("#"+c[f]).val(h[f]),$("#"+c[f]).prop("defaultValue",h[f])}if($("#"+c[f]).data("repeater")){ga.repeat.change(d,c[f]);children=ga.repeat.children(d,c[f]);
k=$("#"+c[f]).val();l=p=0;for(r=1;r<=k;++r)for(t in l=0,children)if(z=m[w][p],g=c[f]+"-"+t+"-"+(r-1),++l,"whitespace_formulchrg"!=b&&"whitespace_formulchcompost"!=b||1!=l)"whitespace_formulchcompost"==b&&2==l?("r"==z.toLowerCase()&&$("#"+g).prop("checked",!0),"f"==z.toLowerCase()&&$("#"+g).prop("checked",!1)):"whitespace_formulchcompost"==b&&5==l||$("#"+g).val(z),p++;ga.repeat.change(d,c[f]);w++}}};ga.value.input={};
...
ga.value.setInputfromRFile=function(a,b,c,d){var e=c.split(",");a="results/users/"+$("#_state").data("_logon")+"/"+a;$.get(a,function(a){ga.value.processInputfromFiles(a,b,e,d)},"text")};ga.value.setInputfromFile=function(a,b,c,d){$(a).hide();var e=c.split(",");$(a).change(function(c){c=$(a)[0].files[0];var h=new FileReader;h.onload=function(a){ga.value.processInputfromFiles(a.target.result,b,e,d)};h.readAsText(c)})};
...
ga.repeat.change=function(a,b,c){var d,e=[],g="#"+b;d=$(g);var h,f="",m="",k,p,l;if(!(ga.repeat.data[a]&&ga.repeat.data[a].repeater&&ga.repeat.data[a].repeater[b]&&d.length))return!1;switch(ga.repeat.data[a].repeater[b].type){case "checkbox":d=d.prop("checked")?1:0;break;case "integer":case "listbox":d=d.val();break;default:return console.warn("ga.repeat.change( "+a+" , "+b+" ) type "+ga.repeat.data[a].repeater[b].type+" not supported"),!1}if(!c&&ga.repeat.data[a].repeater[b].value===d)return!1;h=
ga.repeat.children(a,b);switch(ga.repeat.data[a].repeater[b].type){case "checkbox":if(d)for(k in h)l=b+"-"+k,ga.repeat.map[k]=l,f+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,l).replace("%%label%%",""),m+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,l),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[l]||(ga.repeat.data[a].repeater[l]=jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(l),ga.repeat.data[a].repeater[l].value&&delete ga.repeat.data[a].repeater[l].value);
break;case "integer":if(ga.repeat.data[a].repeater[b].tableize&&0<d)for(k in h)f+=ga.repeat.data[a].repeat[k].htmls;for(p=1;p<=d;++p)for(k in h)l=b+"-"+k+"-"+(p-1),ga.repeat.map[k]=l,f+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,l).replace("%%label%%","["+p+"]").replace(ga.repeat.data[a].repeater[b].tableize?/<td.*?><label.*?>.*?<\/label><\/td>/:"",""),m+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,l),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[l]||(ga.repeat.data[a].repeater[l]=
jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(l),ga.repeat.data[a].repeater[l].value&&delete ga.repeat.data[a].repeater[l].value);break;case "listbox":h=b.replace(/-[0-9]+$/,"").replace(/^(.*)-([A-ZA-z0-9_]*)$/,"$2")+"-"+d;p=b+"-"+d;h=ga.repeat.children(a,h);for(k in h)l=p+"-"+k,ga.repeat.map[k]=l,f+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,l).replace("%%label%%",""),m+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,l),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[l]||
(ga.repeat.data[a].repeater[l]=jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(l),ga.repeat.data[a].repeater[l].value&&delete ga.repeat.data[a].repeater[l].value);break;default:return console.warn("ga.repeat.change( "+a+" , "+b+" ) type "+ga.repeat.data[a].repeater[b].type+" not supported"),!1}!/^<tr>/.test(f)&&/<\/tr>$/.test(f)&&(f="<tr>"+f);$(g+"-repeater").html(f);eval(m);ga.repeat.data[a].repeater[b].value=d;for(k=0;k<e.length;++k)ga.repeat.change(a,e[k],c);$("#global_data").data("_pull_json#"+
...
ga.data.nofcrefresh={};
ga.data.update=function(a,b,c,d){var e=0,g=0,h=0,f=0,m=0,k=a+"_output",p="#"+k,l=$(p),z={},r=p+"_msgs",w=$(r),q,u,y,A,v,B,x;c&&($("#"+a+"_progress").html(""),w.text(""));$.each(b,function(b,n){x=l.find("#"+b);if(x.length)switch(e||(w.text(""),e=1),x.attr("type")){case "plot2d":q="#"+b;n.data?(ga.value.set.plot2d(q,n.options),$.plot(x,n.data,ga.value.get.plot2d.plot_options(q,n.options))):$.plot(x,n,ga.value.get.plot2d.plot_options(q));y=k+":#"+b+":last_value";$("#global_data").data(y,n);break;case "atomicstructure":y=
k+":#"+b+":last_value";B=n.file?n.file:n;_jmol_info[b].script="set background ["+ga.colors.background+"]; set zoomlarge false;set echo top center;echo loading "+B.split("/").pop()+';refresh;load "'+B+'";';ga.set(a+":jsmoladd")&&(_jmol_info[b].script+=ga.set(a+":jsmoladd"));n.script&&(_jmol_info[b].script+=";"+n.script);$("#global_data").data(y,_jmol_info[b].script);$("#"+b).html(Jmol.getAppletHtml("jmolApplet"+b,_jmol_info[b]));break;case "checkbox":case "radio":x.prop("checked",!0);break;case "div":x.html(n);
break;case "video":u=$("#"+b);v="<video ";u.attr("data-width")&&(v+=' width="'+u.attr("data-width")+'"');u.attr("data-height")&&(v+=' height="'+u.attr("data-height")+'"');v+=" controls>";v+='<source src="'+n+'.mp4" type="video/mp4" /><source src="'+n+'.webm" type="video/webm" />';v+="</video>";u.html(v);break;case "image":u=$("#"+b);v="<img ";u.attr("data-width")&&(v+=' width="'+u.attr("data-width")+'"');u.attr("data-height")&&(v+=' height="'+u.attr("data-height")+'"');v+=' src="'+n+'">';u.html(v);
break;case "filelink":A='<a href="'+n+'" target="_blank">'+n.split("/").pop()+"</a>";y=k+":#"+b+":last_value";$("#global_data").data(y,A);$("#"+b+"_filelink").html(A);break;case "filelinkm":y=k+":#"+b+":last_value";A="";$.each(n,function(a,b){A+='<a href="'+b+'" target="_blank">'+b.split("/").pop()+"</a> "});$("#global_data").data(y,A);$("#"+b+"_filelink").html(A);break;default:$("#global_data").data("_append:"+k+"_"+b)?(x.val(x.val()+"\n"+n),x.height(parseFloat(x.prop("scrollHeight")+parseFloat(x.css("borderTopWidth"))+
parseFloat(x.css("borderBottomWidth"))))):x.val(n)}else c?"_"==b.charAt(0)?/^_fs_/.test(b)&&ga.data.nofcrefresh[a]||("_message"==b&&messagebox(n),/^_getinput/.test(b)&&"_getinput"==b&&ga.valuen.input(a,n),"_textarea"==b&&ga.data.textarea(p,n),"_airavata"==b&&ga.data.airavata(p,n),"_status"==b&&"complete"==n&&c(d,0,0)):(g||(w.append("<p>Unexpected results:</p>"),g=1),w.append("<p>"+b+" => "+n+"</p>")):"_"==b.charAt(0)?/^_fs_/.test(b)&&ga.data.nofcrefresh[a]||($("#_state").data(b,n),h=1,"_status"==
b&&(z.job_status=n),/^_getinput/.test(b)&&"_getinput"==b&&ga.valuen.input(a,n),"_textarea"==b&&ga.data.textarea(p,n),"_airavata"==b&&ga.data.airavata(p,n),"_loginverify"==b&&ga.login.verify(n),"_loginapprove"==b&&ga.login.approve(n)):"-close"==b?f=1:"-close2"==b?m=1:(g||(w.text(""),w.append("<p>Unexpected results:</p>"),e=g=1),w.append("<p>"+b+" => "+n+"</p>"))});ga.value.saveLastValues(k);ga.value.saveLastValue(k,r);$(p+"_progress").html("");h&&syncState();f&&closeModal();m&&closeModal2();return z};
...
ga.valuen.restore=function(a,b,c){var d="#"+a,e=$(d),g=e.find(":input").get(),h={},f,m;b=b||ga.valuen.data[a];c=c||ga.valuen.html[a];if(b){$(d+" .field_msg").html("");do f=!1,$.each(g,function(){var c,d,l=$(this),m,r;if(l.attr("data-repeater")&&!h[this.name]){h[this.name]=!0;if(this.name&&(b[this.name]||/checkbox|radio/i.test(this.type))){d=b[this.name];if(/checkbox|radio/i.test(this.type)){m=l.val();r=!1;if(d)for(c=0;c<d.length;c++)if(d[c]==m){r=!0;break}l.prop("checked",r)}else l.val(d[0]);f=!0;
