...
ga.calc.register=function(a,c,b){ga.calc.data[a]=ga.calc.data[a]||{};ga.calc.data[a].calc=ga.calc.data[a].calc||{};ga.calc.data[a].calc[c]={};ga.calc.data[a].calc[c].calc=b;ga.calc.data[a].calc[c].tokens=ga.calc.tokens(b);ga.calc.data[a].calc[c].tokens._error?ga.msg.box({icon:"toast.png",text:"Module field calc internal error (tokens): "+ga.calc.data[a].calc[c].tokens._error+" in calc field id "+c}):(ga.calc.repeatertokens(a,c),ga.calc.data[a].calc[c].dependents=ga.calc.dependents(a,c),99<ga.calc.depthofdeps(a,
c)?ga.msg.box({icon:"toast.png",text:"Module field calc internal error: maximum recursion depth found in calc field id "+c}):(ga.calc.data[a].calc[c].tree=ga.calc.mktree(ga.calc.data[a].calc[c].tokens),ga.calc.data[a].calc[c].tree._error?ga.msg.box({icon:"toast.png",text:"Module field calc internal error (tree): "+ga.calc.data[a].calc[c].tree._error+" in calc field id "+c}):ga.calc.install(a,c)))};
...
ga.calc.process=function(a,c){var b=ga.calc.evaltree(jQuery.extend(!0,{},ga.calc.data[a].calc[c].tree));b._error?ga.msg.box({icon:"toast.png",text:"Module field calc internal error: "+b._error+" in calc field id "+c}):$("#"+c).val(b).trigger("change")};
ga.calc.tokens=function(a){var c=[],b,e=[],f=RegExp("^("+ga.calc.str_function_paren+"|"+ga.calc.str_atom_id+"|"+ga.calc.str_paren+"|"+ga.calc.str_atom_numeric+"|"+ga.calc.str_function_no_paren+")"),d=RegExp("^("+ga.calc.str_binary+"|"+ga.calc.str_close_paren+")"),g=500;a=a.replace(/\s+/g,"");e.push(0);do{if(0<e.length&&e[e.length-1]){b=d.exec(a);if(!b)return{_error:"Invalid token found "+a};if(ga.calc.is_close_paren.test(b[0])){if(!e.length)return{_error:"Invalid closing parenthesis "+a};e.pop();
e.pop();e.push(1)}else e[e.length-1]=0}else{b=f.exec(a);if(!b)return{_error:"Invalid token found "+a};if(ga.calc.is_atom.test(b[0]))e[e.length-1]=1;else if(ga.calc.is_open_paren.test(b[0]))e.push(0);else if(ga.calc.is_close_paren.test(b[0])){if(!e.length)return{_error:"Invalid closing parenthesis "+a};e.pop();e.pop();e.push(1)}else ga.calc.is_function_paren.test(b[0])&&e.push(0)}a=a.substring(b[0].length);c.push(b[0])}while(b&&b.length&&a.length&&0<--g);return 0>=g?{_error:"maximum token limit of 500 reached"}:
c};ga.calc.parensub=function(a){for(var c=1,b={a:[]},e=1;e<a.length;++e){if(")"==a[e]&&(c--,0==c))return b.newofs=e,b;b.a.push(a[e]);/\($/.test(a[e])&&c++}return{_error:"Closing parenthesis error"}};
