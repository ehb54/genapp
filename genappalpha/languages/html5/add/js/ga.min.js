...
"_getinput"==p&&ga.valuen.input(a,l),"_textarea"==p&&ga.data.textarea(r,l),"_airavata"==p&&ga.data.airavata(r,l),"_loginverify"==p&&ga.login.verify(l),"_loginapprove"==p&&ga.login.approve(l)):"-close"==p?h=1:"-close2"==p?q=1:(e||(x.text(""),x.append("<p>Unexpected results:</p>"),f=e=1),x.append("<p>"+p+" => "+l+"</p>"))});ga.value.saveLastValues(k);ga.value.saveLastValue(k,u);$(r+"_progress").html("");g&&syncState();h&&ga.msg.close(1);q&&ga.msg.close(2);return z};
...
h+='ga.altfile("'+q+'","'+e.id+'","'+e.id+'_altval" );$( "#'+q+'" ).change( function(){ $( "#'+e.id+'_altval" ).html( "<i>Local</i>: " + $( "#'+e.id+'" ).val().replace(/^C:.fakepath./,""));});ga.altfile.button( "'+q+'","'+e.id+'","'+e.label+'","rfile",function(v){ga.altfile.button.lrfile(v,"'+q+'","'+e.id+'")});';e.required&&(h+=',"lrfile"');h+='$("#'+e.id+'_button").on("click",function(){return ga.altfile.button.call("'+q+'","'+e.id+'");});';break;case "text":c+="<tr><td>";e.label&&(c+='<label for="'+
...
e.id+'"'+f;e.required&&(c+=" required");c+=" readonly";e.fontfamily&&(c+=' style="font-family: '+e.fontfamily+';"');e.cols&&(c+=" cols="+e.cols);e.rows&&(c+=" rows="+e.rows);c+=">";e["default"]&&(c+=e["default"]);c+="</textarea>"+help_span+"</td></tr>";break;case "checkbox":c+="<tr><td>";e.label&&(c+='<label for="'+e.id+'"'+n+">"+e.label+"</label>");c+='</td><td><input type="checkbox" id="'+e.id+'" name="'+e.id+'"'+f;e.checked&&(c+=" checked");e.readonly&&(c+=" readonly");c+=">"+help_span+"</td></tr>";
break;case "listbox":if(e.header){c+="<tr><td colspan=2";if(e.fontfamily||e.fontsize)c+=' style="',e.fontfamily&&(c+="font-family:"+e.fontfamily+";"),e.fontsize&&(c+="font-size:"+e.fontsize+";"),c+='"';e.width&&(e.header=e.header.padEnd(e.width));e.header=e.header.replace(/ /g,"&nbsp;");console.log(e.header);c+=">"+e.header+"</td></tr>"}c+="<tr><td>";e.label&&(c+='<label for="'+e.id+'"'+n+">"+e.label+"</label>");c+="</td>";c=e.size&&1<e.size?c+"</tr><tr><td colspan=2>":c+"<td>";c+='<select id="'+
e.id+'" name="'+e.id+'"'+f;e.fontfamily&&(c+=' style="font-family: '+e.fontfamily+';"');e.size&&(c+=" size="+e.size);e.multiple&&(c+=" multiple");c+=">";if(!e.values){g+="No values for listbox "+e.id+". ";break}if(e.returns){if(e.returns&&e.returns.length!=e.values.length){g+="Listbox values length ("+e.values.length+") does not equal return length ("+e.returns.length+") for listbox "+e.id+". ";break}for(f=0;f<e.values.length;++f)c+='<option value="'+e.returns[f]+'">'+e.values[f].replace(/ /g,"&nbsp")+
"</option>"}else for(f=0;f<e.values.length;++f)c+='<option value="'+f+'">'+e.values[f].replace(/ /g,"&nbsp")+"</option>";e["default"]&&(h+='$("#'+e.id+" option[value='"+e["default"]+'\']").attr("selected", "true");',console.log("Listbox qeval: "+h));c+="</select>"+help_span+"</td></tr>";break;default:g+="Unknown or currently unsupported field:type "+e.type+". "}c+="</table></form>";if(b._question.buttons&&b._question.buttons.length)for(d=0;d<b._question.buttons.length;++d)switch(e=b._question.buttons[d],
typeof e){case "string":bid=e.replace(/\W/g,"").toLowerCase();r[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". ");r[bid]=1;k.push({id:bid,label:e,cb:ga.qr.cb,adata:[b,bid]});break;case "object":!e.id&&e.label&&(e.id=e.label.replace(/\W/g,"").toLowerCase());e.id?(r[e.id]&&(g+="Duplicate id in _question fields & buttons:"+e.id+". "),r[e.id]=1,e.id!=e.id.replace(/\W/g,"")&&(g+="Buttons array object entry "+(d+1)+" id is not pure alphanumeric. ")):g+="Buttons array object entry "+(d+1)+
" does not have an id. ";e.label||(g+="Buttons array object entry "+(d+1)+" does not have a label. ");if(!g.length){k.push({id:e.id,label:e.label,cb:ga.qr.cb,adata:[b,e.id]});e.help&&(k[k.length-1].help=e.help);break}break;default:g+="Buttons array entry "+(d+1)+" unknown type: "+typeof e+". "}else bid="ok",r[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". "),r[bid]=1,bid="cancel",r[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". "),r[bid]=1,k=[{id:"ok",label:"OK",cb:ga.qr.cb,
adata:[b,"ok"]},{id:"cancel",label:"Cancel",cb:ga.qr.cb,adata:[b,"cancel"]}];if(g.length)return ga.qr.rerror(b,g);ga.qr.openq[q]="open";ga.msg.box({icon:"question.png",noclose:1,text:c+"<p></p>",eval:'$("#'+q+'").on("keyup keypress", function(e) { var code = e.keyCode || e.which;  if (code  == 13) { e.preventDefault(); return false; }});'+h,buttons:k,ptext:"<p></p>"},0,2)};
