...
ga.admin.ajax.group=function(a,b,c,d,f){ga.msg.box({icon:"admin.png",text:"Enter the group for user "+b+'<form id="sys_musergrp"><input id="sys_musergrp_text" class="help_link" type="text" size="25" value="'+f+'"><span class="help">Enter a group for this user, then press ok or cancel</span></form>',eval:"resetHoverHelp();$('#sys_musergrp').on('keyup keypress', function(e) { var code = e.keyCode || e.which;  if (code  == 13) { e.preventDefault(); return false; }});",buttons:[{id:"ok",label:"Ok",cb:ga.admin.ajax.group.cb,
...
ga.altfile.button.lrfile=function(a,b,c){var d=[],f="#"+b,e="#"+c,g="",h="_hidden_lrfile_sels_"+c;$(f+" ."+h).remove();$.each($(a).jstree("get_checked",!0),function(){this.children.length||(g+='<input type="hidden" name="'+c+'_altval[]" value="'+this.id+'" class="'+h+'">',d.push($.base64.decode(this.id).substr(2)))});d.length&&($(e+"_altval").html("<i>Server</i>: "+d),$(e+"_msg").html(""),$(e).val(""),$(f).append(g),ga.value.input[b]&&ga.value.input[b][c]&&ga.value.setInputfromRFile(d,ga.value.input[b][c].mode,
...
ga.value.processInputfromFiles=function(a,b,c,d){var f=a.trim().split(/[\r\n]+/g),e=[],g=0;c=ga.repeat.map.convert(c);for(var h=0;h<f.length;h++){var q=f[h].trim().split(/\s+/),g=g+q.length;e.push(g)}var g=[],k=[],r=[],p=[],z=[],u=[],q=[],x=0;switch(b){case "whitespace_formulchcompost":a=[];for(h=0;h<f.length;h++)e=f[h].split("#")[0],e=e.replace(/\s{2,}/g," "),e=e.replace(/\t/g," "),e=e.toString().trim().replace(/(\r\n|\n|\r)/g,""),a.push(e);for(var f=parseInt(a[1]),m,h=0;h<a.length;h++)if(2<h&&h<
...
[];for(h=0;h<f.length;h++)e=f[h].split("#")[0],e=e.replace(/\s{2,}/g," "),e=e.replace(/\t/g," "),e=e.toString().trim().replace(/(\r\n|\n|\r)/g,""),a.push(e);for(var f=parseInt(a[1]),v=parseInt(a[2+f]),y=parseInt(a[5+f+v]),B=parseInt(a[9+f+v+y]),h=0;h<a.length;h++)if(1<h&&h<2+f)for(m=a[h].trim().split(/\s+/),e=0;e<m.length;e++)k.push(m[e]);else h>2+f&&h<3+f+v?(m=a[h].trim().split(/\s+/),r.push(m[1]),r.push(m[2]),r.push(m[0]),r.push(m[3])):h>5+f+v&&h<6+f+v+y?(m=a[h].trim().split(/\s+/),p.push(m[1]),
p.push(m[2]),p.push(m[0]),p.push(m[3])):h>8+f+v+y&&h<9+f+v+y+B?(m=a[h].trim().split(/\s+/),z.push(m[1]),z.push(m[2]),z.push(m[0]),z.push(m[3])):g.push(a[h]);u.push(k);u.push(r);u.push(p);u.push(z);for(h=e=0;h<c.length;h++)if($("#"+c[h]).length)switch($("#"+c[h]).data("repeater")&&(q.push(u[e]),e++),$("#"+c[h]).attr("type")){case "text":k=new RegExp($("#"+c[h]).attr("pattern"));if(!k.test(g[h])){ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+x+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",
...
jQuery.extend({},ga.repeat.data[a].repeater[k])),f.push(p),ga.repeat.data[a].repeater[p].value&&delete ga.repeat.data[a].repeater[p].value);break;case "listbox":g=b.replace(/-[0-9]+$/,"").replace(/^(.*)-([A-ZA-z0-9_]*)$/,"$2")+"-"+d;r=b+"-"+d;g=ga.repeat.children(a,g);for(k in g)p=r+"-"+k,ga.repeat.map[k]=p,h+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,p).replace("%%label%%",""),q+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,p),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[p]||
(ga.repeat.data[a].repeater[p]=jQuery.extend({},ga.repeat.data[a].repeater[k])),f.push(p),ga.repeat.data[a].repeater[p].value&&delete ga.repeat.data[a].repeater[p].value);break;default:return console.warn("ga.repeat.change( "+a+" , "+b+" ) type "+ga.repeat.data[a].repeater[b].type+" not supported"),!1}!/^<tr>/.test(h)&&/<\/tr>$/.test(h)&&(h="<tr>"+h);$(e+"-repeater").html(h);eval(q);ga.repeat.data[a].repeater[b].value=d;for(k=0;k<f.length;++k)ga.repeat.change(a,f[k],c);$("#global_data").data("_pull_json#"+
...
$("#global_data").data(y,B);$("#"+n+"_filelink").html(B);break;case "filelinkm":y=k+":#"+n+":last_value";B="";$.each(l,function(a,b){B+='<a href="'+b+'" target="_blank">'+b.split("/").pop()+"</a> "});$("#global_data").data(y,B);$("#"+n+"_filelink").html(B);break;default:$("#global_data").data("_append:"+k+"_"+n)?(A.val(A.val()+"\n"+l),A.height(parseFloat(A.prop("scrollHeight")+parseFloat(A.css("borderTopWidth"))+parseFloat(A.css("borderBottomWidth"))))):A.val(l)}else c?"_"==n.charAt(0)?/^_fs_/.test(n)&&
ga.data.nofcrefresh[a]||("_message"==n&&ga.msg.box(l),"_question"==n&&ga.qr.question(a,b),"_question_answered"==n&&ga.qr.answered(a,b),"_question_timeout"==n&&ga.qr.timeout(a,b),/^_getinput/.test(n)&&"_getinput"==n&&ga.valuen.input(a,l),"_textarea"==n&&ga.data.textarea(r,l),"_airavata"==n&&ga.data.airavata(r,l),"_status"==n&&"complete"==l&&c(d,0,0)):(e||(x.append("<p>Unexpected results:</p>"),e=1),x.append("<p>"+n+" => "+l+"</p>")):"_"==n.charAt(0)?/^_fs_/.test(n)&&ga.data.nofcrefresh[a]||($("#_state").data(n,
l),g=1,"_status"==n&&(z.job_status=l),/^_getinput/.test(n)&&"_getinput"==n&&ga.valuen.input(a,l),"_textarea"==n&&ga.data.textarea(r,l),"_airavata"==n&&ga.data.airavata(r,l),"_loginverify"==n&&ga.login.verify(l),"_loginapprove"==n&&ga.login.approve(l)):"-close"==n?h=1:"-close2"==n?q=1:(e||(x.text(""),x.append("<p>Unexpected results:</p>"),f=e=1),x.append("<p>"+n+" => "+l+"</p>"))});ga.value.saveLastValues(k);ga.value.saveLastValue(k,u);$(r+"_progress").html("");g&&syncState();h&&ga.msg.close(1);q&&
...
d.id+'"></form>',ga.msg.box({text:c,eval:"resetHoverHelp();$('#sys_captcha').on('keyup keypress', function(e) { var code = e.keyCode || e.which;  if (code  == 13) { e.preventDefault(); return false; }});",buttons:[{id:"submit",label:"Submit",cb:ga.captcha.verify,data:{cb:a,data:b}}]}))}).error(function(a){ga.msg.box({icon:"toast.png",text:"Error contacting server"})})};
...
ga.qr.question=function(a,b){var c="",d,f,e,g="",h,q=[],k={},r;b._uuid||(g+="no _uuid in received data. ");b._msgid||(g+="no _msgid in received data. ");b._question||(g+="no _question in received data. ");b._question.fields||(g+="no _question:fields in received data. ");if(g.length)return ga.qr.rerror(b,g);b._question.title&&(c+="<h3>"+b._question.title+"</h3>");b._question.text&&(c+=b._question.text);h=b._uuid+"-"+b._msgid;c+='<form id="'+h+'"><table>';for(d=0;d<b._question.fields.length;++d)if(e=
b._question.fields[d],e.id||(g+="No id in field "+d+". "),e.type||(g+="No type in field "+d+". "),e.id&&e.type)switch(k[e.id]&&(g+="Duplicate id in _question fields:"+e.id+". "),k[e.id]=1,e.help?(f=' class="help_link"',r=' class="highlight help_link"',help_span='<span class="help">'+e.help+"</span>"):(f="",r=' class="highlight"',help_span=""),e.type){case "label":c+="<tr><td colspan=2><label"+f+">";e.label&&(c+=e.label);c+="</label>"+help_span+"</td></tr>";break;case "text":c+="<tr><td>";e.label&&
(c+='<label for="'+e.id+'"'+f+">"+e.label+"</label>");c+='</td><td><input type="text" id="'+e.id+'"'+f;e.required&&(c+=" required");e.readonly&&(c+=" readonly");e["default"]&&(c+=' value="'+e["default"]+'"');e.pattern&&(c+=' pattern="'+e.pattern+'"');e.maxlength&&(c+=' maxlength="'+e.maxlength+'"');e.size&&(c+=' size="'+e.size+'"');c+=">"+help_span+"</td></tr>";break;case "textarea":c+="<tr><td>";e.label&&(c+='<label for="'+e.id+'"'+f+">"+e.label+"</label>");c+='</td><td><textarea id="'+e.id+'"'+
f;e.required&&(c+=" required");c+=" readonly";e.fontfamily&&(c+=' style="font-family: '+e.fontfamily+';"');e.cols&&(c+=" cols="+e.cols);e.rows&&(c+=" rows="+e.rows);c+=">";e["default"]&&(c+=e["default"]);c+="</textarea>"+help_span+"</td></tr>";break;case "checkbox":c+="<tr><td>";e.label&&(c+='<label for="'+e.id+'"'+r+">"+e.label+"</label>");c+='</td><td><input type="checkbox" id="'+e.id+'"'+f;e.checked&&(c+=" checked");e.readonly&&(c+=" readonly");c+=">"+help_span+"</td></tr>";break;case "listbox":c+=
"<tr><td>";e.label&&(c+='<label for="'+e.id+'"'+r+">"+e.label+"</label>");c+="</td>";c=e.size&&1<e.size?c+"</tr><tr><td colspan=2>":c+"<td>";c+='<select id="'+e.id+'"'+f;e.fontfamily&&(c+=' style="font-family: '+e.fontfamily+';"');e.size&&(c+=" size="+e.size);c+=">";if(!e.values){g+="No values for listbox "+e.id+". ";break}if(e.returns){if(e.returns&&e.returns.length!=e.values.length){g+="Listbox values length ("+e.values.length+") does not equal return length ("+e.returns.length+") for listbox "+
e.id+". ";break}for(f=0;f<e.values.length;++f)c+='<option value="'+e.returns[f]+'">'+e.values[f].replace(/ /g,"&nbsp")+"</option>"}else for(f=0;f<e.values.length;++f)c+='<option value="'+f+'">'+e.values[f].replace(/ /g,"&nbsp")+"</option>";c+="</select>"+help_span+"</td></tr>";break;default:g+="Unknown or currently unsupported field:type "+e.type+". "}c+="</table></form>";if(b._question.buttons&&b._question.buttons.length)for(d=0;d<b._question.buttons.length;++d)switch(e=b._question.buttons[d],typeof e){case "string":bid=
e.replace(/\W/g,"").toLowerCase();k[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". ");k[bid]=1;q.push({id:bid,label:e,cb:ga.qr.cb,adata:[b,bid]});break;case "object":!e.id&&e.label&&(e.id=e.label.replace(/\W/g,"").toLowerCase());e.id?(k[e.id]&&(g+="Duplicate id in _question fields & buttons:"+e.id+". "),k[e.id]=1,e.id!=e.id.replace(/\W/g,"")&&(g+="Buttons array object entry "+(d+1)+" id is not pure alphanumeric. ")):g+="Buttons array object entry "+(d+1)+" does not have an id. ";e.label||
(g+="Buttons array object entry "+(d+1)+" does not have a label. ");if(!g.length){q.push({id:e.id,label:e.label,cb:ga.qr.cb,adata:[b,e.id]});e.help&&(q[q.length-1].help=e.help);break}break;default:g+="Buttons array entry "+(d+1)+" unknown type: "+typeof e+". "}else bid="ok",k[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". "),k[bid]=1,bid="cancel",k[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". "),k[bid]=1,q=[{id:"ok",label:"OK",cb:ga.qr.cb,adata:[b,"ok"]},{id:"cancel",
label:"Cancel",cb:ga.qr.cb,adata:[b,"cancel"]}];if(g.length)return ga.qr.rerror(b,g);ga.qr.openq[h]="open";ga.msg.box({icon:"question.png",noclose:1,text:c,eval:'$("#'+h+'").on("keyup keypress", function(e) { var code = e.keyCode || e.which;  if (code  == 13) { e.preventDefault(); return false; }});',buttons:q})};
...
var d={};d._uuid=a._uuid;d._msgid=parseFloat(a._msgid);d._response={};d._response.button=b;a._question&&a._question.id&&(d._response.id=a._question.id);$("#"+c+" *").filter(":input").each(function(){switch(this.type){case "text":case "select-one":d._response[this.id]=this.value;break;case "checkbox":this.checked&&(d._response[this.id]=!0)}});ga.qr.post(d)}};ga.qr.answered=function(a,b){var c;b._uuid&&b._msgid&&(c=b._uuid+"-"+b._msgid,ga.qr.openq[c]&&(ga.qr.openq[c]="answered"))};
ga.qr.timeout=function(a,b){var c;b._uuid&&b._msgid&&(c=b._uuid+"-"+b._msgid,ga.qr.openq[c]&&(ga.qr.openq[c]="timeout"))};ga.qr.rerror=function(a,b){var c={};c._uuid=a._uuid;c._msgid=parseFloat(a._msgid);c._response={};c._response.error=b;a._question&&a._question.id&&(c._response.id=a._question.id);ga.qr.post(c);ga.msg.box({icon:"toast.png",text:b})};
...
ga.msg.close(c)});break;default:console.warn("in ga.msg.box unsupported number of adata arguments "+a.buttons[i].adata.length)}else $("#_mbb_"+a.buttons[i].id).off().one("click",a.buttons[i],function(a){a.data.cb(a.data.data);ga.msg.close(c)});else $("#_mbb_"+a.buttons[i].id).off().one("click",a.buttons[i],function(a){ga.msg.close(c)})}a.ptext&&$("#configbody"+c).append(a.ptext);a.eval&&eval(a.eval);ga.repeats.save();resetHoverHelp();$(".modalDialog"+c).addClass("modalDialog"+c+"_on")}};
...
ga.msg.clicks=function(){$("#closeModal").click(function(){ga.msg.close(1)});$("#closeModal2").click(function(){ga.msg.close(2)});$("#closeModal3").click(function(){ga.msg.close(3)});$("#closeModal4").click(function(){ga.msg.close(4)})};
