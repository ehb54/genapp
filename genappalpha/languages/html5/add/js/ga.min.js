...
ga.value.processInputfromFiles=function(a,b,c,d){var e=a.trim().split(/[\r\n]+/g),f=[],h=0;c=ga.repeat.map.convert(c);for(var g=0;g<e.length;g++){var p=e[g].trim().split(/\s+/),h=h+p.length;f.push(h)}var h=[],k=[],q=[],m=[],z=[],r=[],p=[],w=0;switch(b){case "whitespace_formulchcompost":a=[];for(g=0;g<e.length;g++)f=e[g].split("#")[0],f=f.replace(/\s{2,}/g," "),f=f.replace(/\t/g," "),f=f.toString().trim().replace(/(\r\n|\n|\r)/g,""),a.push(f);for(var e=parseInt(a[1]),n,g=0;g<a.length;g++)if(2<g&&g<
...
m.push(n[2]),m.push(n[0]),m.push(n[3])):g>8+e+u+x&&g<9+e+u+x+A?(n=a[g].trim().split(/\s+/),z.push(n[1]),z.push(n[2]),z.push(n[0]),z.push(n[3])):h.push(a[g]);r.push(k);r.push(q);r.push(m);r.push(z);for(g=f=0;g<c.length;g++)if($("#"+c[g]).length)switch($("#"+c[g]).data("repeater")&&(p.push(r[f]),f++),$("#"+c[g]).attr("type")){case "text":k=new RegExp($("#"+c[g]).attr("pattern"));if(!k.test(h[g])){messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+w+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",
...
ga.data.update=function(a,b,c,d){var e=0,f=0,h=0,g=0,p=0,k=a+"_output",q="#"+k,m=$(q),z={},r=q+"_msgs",w=$(r),n,u,x,A,v,B,y;c&&($("#"+a+"_progress").html(""),w.text(""));$.each(b,function(b,l){y=m.find("#"+b);if(y.length)switch(e||(w.text(""),e=1),y.attr("type")){case "plot2d":n="#"+b;var r,C;ga.value.plot2d.zstack.reset(n);l.data?(ga.value.set.plot2d(n,l.options),$.plot(n,l.data,ga.value.get.plot2d.plot_options(n,l.options))):$.plot(n,l,ga.value.get.plot2d.plot_options(n));if($(n+"_savetofile").length){var D=
document.getElementById(b+"_savetofile");html2canvas(y.get(0),{background:"#ffffff",onrendered:function(a){r=a.toDataURL("image/png").replace("image/png","image/octet-stream");C=dataURLtoFile(r,"plot.png");D.href=URL.createObjectURL(C);$(n+"_savetofile").removeClass("hidden")}})}if(ga.value.settings[n].selzoom||l.options&&l.options.selection&&l.options.selection.mode&&"xy"==l.options.selection.mode)$(n).on("plotselected",{htag:n,data:l.data?l.data:l,options:l.data?ga.value.get.plot2d.plot_options(n,
l.options):ga.value.get.plot2d.plot_options(n)},function(a,b){1E-5>b.xaxis.to-b.xaxis.from&&(b.xaxis.to=b.xaxis.from+1E-5);1E-5>b.yaxis.to-b.yaxis.from&&(b.yaxis.to=b.yaxis.from+1E-5);ga.value.plot2d.zstack.dopush(a.data.htag,b);$.plot(a.data.htag,a.data.data,$.extend(!0,{},a.data.options,{xaxis:{min:b.xaxis.from,max:b.xaxis.to},yaxis:{min:b.yaxis.from,max:b.yaxis.to}}))}).on("contextmenu",{htag:n,data:l.data?l.data:l,options:l.data?ga.value.get.plot2d.plot_options(n,l.options):ga.value.get.plot2d.plot_options(n)},
function(a){a.preventDefault();var b=ga.value.plot2d.zstack.dopop(a.data.htag);b?$.plot(a.data.htag,a.data.data,$.extend(!0,{},a.data.options,{xaxis:{min:b.xaxis.from,max:b.xaxis.to},yaxis:{min:b.yaxis.from,max:b.yaxis.to}})):$.plot(a.data.htag,a.data.data,a.data.options)});x=k+":#"+b+":last_value";$("#global_data").data(x,l);break;case "atomicstructure":x=k+":#"+b+":last_value";B=l.file?l.file:l;_jmol_info[b].script="set background ["+ga.colors.background+"]; set zoomlarge false;set echo top center;echo loading "+
B.split("/").pop()+';refresh;load "'+B+'";';ga.set(a+":jsmoladd")&&(_jmol_info[b].script+=ga.set(a+":jsmoladd"));l.script&&(_jmol_info[b].script+=";"+l.script);$("#global_data").data(x,_jmol_info[b].script);$("#"+b).html(Jmol.getAppletHtml("jmolApplet"+b,_jmol_info[b]));break;case "checkbox":case "radio":y.prop("checked",!0);break;case "div":y.html(l);break;case "video":u=$("#"+b);v="<video ";u.attr("data-width")&&(v+=' width="'+u.attr("data-width")+'"');u.attr("data-height")&&(v+=' height="'+u.attr("data-height")+
'"');v+=" controls>";v+='<source src="'+l+'.mp4" type="video/mp4" /><source src="'+l+'.webm" type="video/webm" />';v+="</video>";u.html(v);break;case "image":u=$("#"+b);v="<img ";u.attr("data-width")&&(v+=' width="'+u.attr("data-width")+'"');u.attr("data-height")&&(v+=' height="'+u.attr("data-height")+'"');v+=' src="'+l+'">';u.html(v);break;case "filelink":A='<a href="'+l+'" target="_blank">'+l.split("/").pop()+"</a>";x=k+":#"+b+":last_value";$("#global_data").data(x,A);$("#"+b+"_filelink").html(A);
break;case "filelinkm":x=k+":#"+b+":last_value";A="";$.each(l,function(a,b){A+='<a href="'+b+'" target="_blank">'+b.split("/").pop()+"</a> "});$("#global_data").data(x,A);$("#"+b+"_filelink").html(A);break;default:$("#global_data").data("_append:"+k+"_"+b)?(y.val(y.val()+"\n"+l),y.height(parseFloat(y.prop("scrollHeight")+parseFloat(y.css("borderTopWidth"))+parseFloat(y.css("borderBottomWidth"))))):y.val(l)}else c?"_"==b.charAt(0)?/^_fs_/.test(b)&&ga.data.nofcrefresh[a]||("_message"==b&&messagebox(l),
/^_getinput/.test(b)&&"_getinput"==b&&ga.valuen.input(a,l),"_textarea"==b&&ga.data.textarea(q,l),"_airavata"==b&&ga.data.airavata(q,l),"_status"==b&&"complete"==l&&c(d,0,0)):(f||(w.append("<p>Unexpected results:</p>"),f=1),w.append("<p>"+b+" => "+l+"</p>")):"_"==b.charAt(0)?/^_fs_/.test(b)&&ga.data.nofcrefresh[a]||($("#_state").data(b,l),h=1,"_status"==b&&(z.job_status=l),/^_getinput/.test(b)&&"_getinput"==b&&ga.valuen.input(a,l),"_textarea"==b&&ga.data.textarea(q,l),"_airavata"==b&&ga.data.airavata(q,
l),"_loginverify"==b&&ga.login.verify(l),"_loginapprove"==b&&ga.login.approve(l)):"-close"==b?g=1:"-close2"==b?p=1:(f||(w.text(""),w.append("<p>Unexpected results:</p>"),e=f=1),w.append("<p>"+b+" => "+l+"</p>"))});ga.value.saveLastValues(k);ga.value.saveLastValue(k,r);$(q+"_progress").html("");h&&syncState();g&&closeModal();p&&closeModal2();return z};
