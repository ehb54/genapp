...
b)+'">';break;default:console.warn("ga.altfile.button.getnames( "+b+" , "+c+" )")}$("#"+a).append(e)};ga.valid={};ga.valid.checkMatch=function(a,b){$(a).val()!=$(b).val()?$(a+"_msg").html(" does not match"):$(a+"_msg").html("")};ga.valid.checkText=function(a){var b=$(a),c=b.val(),d=0,b=b.attr("pattern"),e=new RegExp(b);b?e.test(c)?(d=1,$(a+"_msg").html("")):$(a+"_msg").html(" wrong format"):c?d=1:$(a+"_msg").html(" missing required field");return d};
ga.valid.checkFloat=function(a){var b=$(a),c=b.val(),d=0;c.match(/^[+-]?\d+(\.\d+)?$/)?(d=1,c<parseFloat(b.attr("min"))?(b.val(b.attr("min")),$(a+"_msg").html(" value set to minimum allowed")):c>parseFloat(b.attr("max"))?(b.val(b.attr("max")),$(a+"_msg").html(" value set to maximum allowed")):$(a+"_msg").html("")):$(a+"_msg").html(" wrong format");return d};
ga.valid.checkInt=function(a){var b=$(a),c=b.val(),d=0;c.match(/^[+-]?\d+$/)?(d=1,c<parseInt(b.attr("min"))?(b.val(b.attr("min")),$(a+"_msg").html(" value set to minimum allowed")):c>parseInt(b.attr("max"))?(b.val(b.attr("max")),$(a+"_msg").html(" value set to maximum allowed")):parseInt(c)!=c?($(a+"_msg").html(" value rounded to nearset integer"),b.val(parseInt(parseFloat(c)+.5))):$(a+"_msg").html("")):$(a+"_msg").html(" wrong format");return d};
...
b+'_button"',"g"),'"#%%id%%_button"').replace(RegExp('"'+b+'_altval"',"g"),'"%%id%%_altval"').replace(RegExp('"#'+b+'_altval"',"g"),'"#%%id%%_altval"');return'<tr><td></td><td><span id="'+b+'-span"></span></td></tr>'};
...
b&&(A.job_status=m),/^_getinput/.test(b)&&"_getinput"==b&&ga.valuen.input(a,m),"_textarea"==b&&ga.data.textarea(p,m),"_airavata"==b&&ga.data.airavata(p,m)):"-close"==b?h=1:"-close2"==b?n=1:(f||(w.text(""),w.append("<p>Unexpected results:</p>"),e=f=1),w.append("<p>"+b+" => "+m+"</p>"))});ga.value.saveLastValues(k);ga.value.saveLastValue(k,x);$(p+"_progress").html("");g&&syncState();h&&closeModal();n&&closeModal2();return A};
