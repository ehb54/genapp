...
[];for(f=0;f<e.length;f++)g=e[f].split("#")[0],g=g.replace(/\s{2,}/g," "),g=g.replace(/\t/g," "),g=g.toString().trim().replace(/(\r\n|\n|\r)/g,""),a.push(g);for(var e=parseInt(a[1]),u=parseInt(a[2+e]),y=parseInt(a[5+e+u]),A=parseInt(a[9+e+u+y]),f=0;f<a.length;f++)if(1<f&&f<2+e)for(q=a[f].trim().split(/\s+/),g=0;g<q.length;g++)k.push(q[g]);else f>2+e&&f<3+e+u?(q=a[f].trim().split(/\s+/),p.push(q[1]),p.push(q[2]),p.push(q[0]),p.push(q[3])):f>5+e+u&&f<6+e+u+y?(q=a[f].trim().split(/\s+/),l.push(q[1]),
l.push(q[2]),l.push(q[0]),l.push(q[3])):f>8+e+u+y&&f<9+e+u+y+A?(q=a[f].trim().split(/\s+/),z.push(q[1]),z.push(q[2]),z.push(q[0]),z.push(q[3])):h.push(a[f]);r.push(k);r.push(p);r.push(l);r.push(z);for(f=g=0;f<c.length;f++)if($("#"+c[f]).length)switch($("#"+c[f]).data("repeater")&&(m.push(r[g]),g++),$("#"+c[f]).attr("type")){case "text":k=new RegExp($("#"+c[f]).attr("pattern"));if(!k.test(h[f])){messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+w+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",
