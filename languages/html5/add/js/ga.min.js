...
c+="</textarea>"+help_span+"</td></tr>";break;case "checkbox":c+="<tr><td>";e.label&&(c+='<label for="'+e.id+'"'+n+">"+e.label+"</label>");c+='</td><td><input type="checkbox" id="'+e.id+'" name="'+e.id+'"'+f;e.checked&&(c+=" checked");e.readonly&&(c+=" readonly");c+=">"+help_span+"</td></tr>";break;case "listbox":c+="<tr><td>";e.label&&(c+='<label for="'+e.id+'"'+n+">"+e.label+"</label>");c+="</td>";c=e.size&&1<e.size?c+"</tr><tr><td colspan=2>":c+"<td>";c+='<select id="'+e.id+'" name="'+e.id+'"'+
f;e.fontfamily&&(c+=' style="font-family: '+e.fontfamily+';"');e.size&&(c+=" size="+e.size);c+=">";if(!e.values){h+="No values for listbox "+e.id+". ";break}if(e.returns){if(e.returns&&e.returns.length!=e.values.length){h+="Listbox values length ("+e.values.length+") does not equal return length ("+e.returns.length+") for listbox "+e.id+". ";break}for(f=0;f<e.values.length;++f)c+='<option value="'+e.returns[f]+'">'+e.values[f].replace(/ /g,"&nbsp")+"</option>"}else for(f=0;f<e.values.length;++f)c+=
'<option value="'+f+'">'+e.values[f].replace(/ /g,"&nbsp")+"</option>";e["default"]&&(g+='$("#'+e.id+" option[value='"+e["default"]+'\']").attr("selected", "true");',console.log("Listbox qeval: "+g));c+="</select>"+help_span+"</td></tr>";break;default:h+="Unknown or currently unsupported field:type "+e.type+". "}c+="</table></form>";if(b._question.buttons&&b._question.buttons.length)for(d=0;d<b._question.buttons.length;++d)switch(e=b._question.buttons[d],typeof e){case "string":bid=e.replace(/\W/g,
"").toLowerCase();r[bid]&&(h+="Duplicate id in _question fields & buttons:"+bid+". ");r[bid]=1;k.push({id:bid,label:e,cb:ga.qr.cb,adata:[b,bid]});break;case "object":!e.id&&e.label&&(e.id=e.label.replace(/\W/g,"").toLowerCase());e.id?(r[e.id]&&(h+="Duplicate id in _question fields & buttons:"+e.id+". "),r[e.id]=1,e.id!=e.id.replace(/\W/g,"")&&(h+="Buttons array object entry "+(d+1)+" id is not pure alphanumeric. ")):h+="Buttons array object entry "+(d+1)+" does not have an id. ";e.label||(h+="Buttons array object entry "+
(d+1)+" does not have a label. ");if(!h.length){k.push({id:e.id,label:e.label,cb:ga.qr.cb,adata:[b,e.id]});e.help&&(k[k.length-1].help=e.help);break}break;default:h+="Buttons array entry "+(d+1)+" unknown type: "+typeof e+". "}else bid="ok",r[bid]&&(h+="Duplicate id in _question fields & buttons:"+bid+". "),r[bid]=1,bid="cancel",r[bid]&&(h+="Duplicate id in _question fields & buttons:"+bid+". "),r[bid]=1,k=[{id:"ok",label:"OK",cb:ga.qr.cb,adata:[b,"ok"]},{id:"cancel",label:"Cancel",cb:ga.qr.cb,adata:[b,
"cancel"]}];if(h.length)return ga.qr.rerror(b,h);ga.qr.openq[q]="open";ga.msg.box({icon:"question.png",noclose:1,text:c+"<p></p>",eval:'$("#'+q+'").on("keyup keypress", function(e) { var code = e.keyCode || e.which;  if (code  == 13) { e.preventDefault(); return false; }});'+g,buttons:k,ptext:"<p></p>"},0,2)};
