...
ga.value.processInputfromFiles=function(a,b,c,e){var g=a.trim().split(/[\r\n]+/g),f=[],k=0;c=ga.repeat.map.convert(c);for(var h=0;h<g.length;h++){var d=g[h].trim().split(/\s+/);k+=d.length;f.push(k)}k=[];var m=[],r=[],l=[],y=[],u=[];d=[];var w=0;switch(b){case "whitespace_formulchcompost":a=[];for(h=0;h<g.length;h++)f=g[h].split("#")[0],f=f.replace(/\s{2,}/g," "),f=f.replace(/\t/g," "),f=f.toString().trim().replace(/(\r\n|\n|\r)/g,""),a.push(f);g=parseInt(a[1]);for(h=0;h<a.length;h++)if(2<h&&h<3+
g){var p=a[h].trim().split(/\s+/);for(f=0;f<p.length;f++)m.push(p[f])}else k.push(a[h]);d.push(m);for(h=0;h<c.length;h++)switch($("#"+c[h]).attr("type")){case "text":m=new RegExp($("#"+c[h]).attr("pattern"));if(!m.test(k[h])){ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+w+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;case "number":m=
[k[h]];if(ga.value.checkFloatIntOK("#"+c[h],m))k[h]=m[0];else{ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+w+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;default:ga.msg.box({icon:"warning.png",text:"Selected input type is currently not supported. Contact the developer",buttons:[{id:"ok",label:"OK"}]});return}break;case "whitespace_formulchrg":case "whitespace_formulchcontrast":a=
[];for(h=0;h<g.length;h++)f=g[h].split("#")[0],f=f.replace(/\s{2,}/g," "),f=f.replace(/\t/g," "),f=f.toString().trim().replace(/(\r\n|\n|\r)/g,""),a.push(f);g=parseInt(a[1]);var v=parseInt(a[2+g]),x=parseInt(a[5+g+v]),B=parseInt(a[9+g+v+x]);for(h=0;h<a.length;h++)if(1<h&&h<2+g)for(p=a[h].trim().split(/\s+/),f=0;f<p.length;f++)m.push(p[f]);else h>2+g&&h<3+g+v?(p=a[h].trim().split(/\s+/),r.push(p[1]),r.push(p[2]),r.push(p[0]),r.push(p[3])):h>5+g+v&&h<6+g+v+x?(p=a[h].trim().split(/\s+/),l.push(p[1]),
l.push(p[2]),l.push(p[0]),l.push(p[3])):h>8+g+v+x&&h<9+g+v+x+B?(p=a[h].trim().split(/\s+/),y.push(p[1]),y.push(p[2]),y.push(p[0]),y.push(p[3])):k.push(a[h]);u.push(m);u.push(r);u.push(l);u.push(y);for(h=f=0;h<c.length;h++)if($("#"+c[h]).length)switch($("#"+c[h]).data("repeater")&&(d.push(u[f]),f++),$("#"+c[h]).attr("type")){case "text":m=new RegExp($("#"+c[h]).attr("pattern"));if(!m.test(k[h])){ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+w+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",
buttons:[{id:"ok",label:"OK"}]});return}break;case "number":m=[k[h]];if(ga.value.checkFloatIntOK("#"+c[h],m))k[h]=m[0];else{ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+w+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;default:ga.msg.box({icon:"warning.png",text:"Selected input type is currently not supported. Contact the developer",buttons:[{id:"ok",
label:"OK"}]});return}else f++;break;case "whitespaceseparated":case "whitespaceseparated_reverselogic":k=a.trim().split(/\s+/);if(k.length==c.length)for(h=0;h<k.length;h++){for(u=0;u<f.length;u++)if(h+1<=f[u]){w=u+1;break}switch($("#"+c[h]).attr("type")){case "checkbox":m=/^(0|1|n|y|true|false|t|f|yes|no)$/;if(!m.test(k[h].toLowerCase())){ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Checkbox input value on the line #"+w+" is not valid. Options are: [1 | 0 | yes | no | true | false | t | f | T | F | y | n | Y | N ]. Check your input file",
buttons:[{id:"ok",label:"OK"}]});return}break;case "number":m=[k[h]];if(ga.value.checkFloatIntOK("#"+c[h],m))k[h]=m[0];else{ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+w+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;case "text":m=new RegExp($("#"+c[h]).attr("pattern"));if(!m.test(k[h])){ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+
...
ga.repeat.change=function(a,b,c){var e=[],g="#"+b;var f=$(g);var k="",h="",d,m;if(!(ga.repeat.data[a]&&ga.repeat.data[a].repeater&&ga.repeat.data[a].repeater[b]&&f.length))return!1;switch(ga.repeat.data[a].repeater[b].type){case "checkbox":f=f.prop("checked")?1:0;break;case "integer":case "listbox":f=f.val();break;default:return console.warn("ga.repeat.change( "+a+" , "+b+" ) type "+ga.repeat.data[a].repeater[b].type+" not supported"),!1}if(!c&&ga.repeat.data[a].repeater[b].value===f)return!1;var r=
ga.repeat.children(a,b);switch(ga.repeat.data[a].repeater[b].type){case "checkbox":if(f)for(d in r){var l=b+"-"+d;ga.repeat.map[d]=l;k+=ga.repeat.data[a].repeat[d].htmlr.replace(/%%id%%/g,l).replace("%%label%%","");h+=ga.repeat.data[a].repeat[d].evalr.replace(/%%id%%/g,l);ga.repeat.data[a].repeater[d]&&(ga.repeat.data[a].repeater[l]||(ga.repeat.data[a].repeater[l]=jQuery.extend({},ga.repeat.data[a].repeater[d])),e.push(l),ga.repeat.data[a].repeater[l].value&&delete ga.repeat.data[a].repeater[l].value)}break;
case "integer":if(ga.repeat.data[a].repeater[b].tableize&&0<f)for(d in r)k+=ga.repeat.data[a].repeat[d].htmls;for(m=1;m<=f;++m)for(d in r)l=b+"-"+d+"-"+(m-1),ga.repeat.map[d]=l,k+=ga.repeat.data[a].repeat[d].htmlr.replace(/%%id%%/g,l).replace("%%label%%","["+m+"]").replace(ga.repeat.data[a].repeater[b].tableize?/<td.*?><label.*?>.*?<\/label><\/td>/:"",""),h+=ga.repeat.data[a].repeat[d].evalr.replace(/%%id%%/g,l),ga.repeat.data[a].repeater[d]&&(ga.repeat.data[a].repeater[l]||(ga.repeat.data[a].repeater[l]=
jQuery.extend({},ga.repeat.data[a].repeater[d])),e.push(l),ga.repeat.data[a].repeater[l].value&&delete ga.repeat.data[a].repeater[l].value);break;case "listbox":r=b.replace(/-[0-9]+$/,"").replace(/^(.*)-([A-ZA-z0-9_]*)$/,"$2")+"-"+f;m=b+"-"+f;r=ga.repeat.children(a,r);for(d in r)l=m+"-"+d,ga.repeat.map[d]=l,k+=ga.repeat.data[a].repeat[d].htmlr.replace(/%%id%%/g,l).replace("%%label%%",""),h+=ga.repeat.data[a].repeat[d].evalr.replace(/%%id%%/g,l),ga.repeat.data[a].repeater[d]&&(ga.repeat.data[a].repeater[l]||
(ga.repeat.data[a].repeater[l]=jQuery.extend({},ga.repeat.data[a].repeater[d])),e.push(l),ga.repeat.data[a].repeater[l].value&&delete ga.repeat.data[a].repeater[l].value);break;default:return console.warn("ga.repeat.change( "+a+" , "+b+" ) type "+ga.repeat.data[a].repeater[b].type+" not supported"),!1}!/^<tr>/.test(k)&&/<\/tr>$/.test(k)&&(k="<tr>"+k);$(g+"-repeater").html(k);eval(h);ga.repeat.data[a].repeater[b].value=f;for(d=0;d<e.length;++d)ga.repeat.change(a,e[d],c);$("#global_data").data("_pull_json#"+
...
var b=ga.value.plot2d.zstack.dopop(a.data.htag);b?$.plot(a.data.htag,a.data.data,$.extend(!0,{},a.data.options,{xaxis:{min:b.xaxis.from,max:b.xaxis.to},yaxis:{min:b.yaxis.from,max:b.yaxis.to}})):$.plot(a.data.htag,a.data.data,a.data.options)});x=m+":#"+q+":last_value";$("#global_data").data(x,n);break;case "bokeh":ga.bokeh.render(a,q,n);break;case "ngl":ga.value.nglshow(m,q,n);break;case "atomicstructure":x=m+":#"+q+":last_value";C=n.file?n.file:n;_jmol_info[q].script="set background ["+ga.colors.background+
"]; set zoomlarge false;set echo top center;echo loading "+C.split("/").pop()+';refresh;load "'+C+'";';ga.set(a+":jsmoladd")&&(_jmol_info[q].script+=ga.set(a+":jsmoladd"));n.script&&(_jmol_info[q].script+=";"+n.script);$("#global_data").data(x,_jmol_info[q].script);$("#"+q).html(Jmol.getAppletHtml("jmolApplet"+q,_jmol_info[q]));break;case "checkbox":case "radio":A.prop("checked",!0);break;case "div":A.html(n);break;case "video":v=$("#"+q);z="<video ";v.attr("data-width")&&(z+=' width="'+v.attr("data-width")+
'"');v.attr("data-height")&&(z+=' height="'+v.attr("data-height")+'"');z+=" controls>";z+='<source src="'+n+'.mp4" type="video/mp4" /><source src="'+n+'.webm" type="video/webm" />';z+="</video>";v.html(z);break;case "image":v=$("#"+q);z="<img ";v.attr("data-width")&&(z+=' width="'+v.attr("data-width")+'"');v.attr("data-height")&&(z+=' height="'+v.attr("data-height")+'"');z+=' src="'+n+'">';v.html(z);break;case "filelink":B='<a href="'+n+'" target="_blank">'+n.split("/").pop()+"</a>";x=m+":#"+q+":last_value";
$("#global_data").data(x,B);$("#"+q+"_filelink").html(B);break;case "filelinkm":x=m+":#"+q+":last_value";B="";$.each(n,function(a,b){B+='<a href="'+b+'" target="_blank">'+b.split("/").pop()+"</a> "});$("#global_data").data(x,B);$("#"+q+"_filelink").html(B);break;default:$("#global_data").data("_append:"+m+"_"+q)?(A.val(A.val()+"\n"+n),A.height(parseFloat(A.prop("scrollHeight")+parseFloat(A.css("borderTopWidth"))+parseFloat(A.css("borderBottomWidth"))))):A.val(n)}else c?"_"==q.charAt(0)?/^_fs_/.test(q)&&
ga.data.nofcrefresh[a]||("_message"==q&&ga.msg.box(n),"_question"==q&&ga.qr.question(a,b),"_question_answered"==q&&ga.qr.answered(a,b),"_question_timeout"==q&&ga.qr.timeout(a,b),/^_getinput/.test(q)&&"_getinput"==q&&ga.valuen.input(a,n),"_textarea"==q&&ga.data.textarea(r,n),"_airavata"==q&&ga.data.airavata(r,n),"_status"==q&&"complete"==n&&c(e,0,0)):(f||(w.append("<p>Unexpected results:</p>"),f=1),w.append("<p>"+q+" => "+n+"</p>")):"_"==q.charAt(0)?/^_fs_/.test(q)&&ga.data.nofcrefresh[a]||($("#_state").data(q,
n),k=1,"_status"==q&&(y.job_status=n),/^_getinput/.test(q)&&"_getinput"==q&&ga.valuen.input(a,n),"_textarea"==q&&ga.data.textarea(r,n),"_airavata"==q&&ga.data.airavata(r,n),"_loginverify"==q&&ga.login.verify(n),"_loginapprove"==q&&ga.login.approve(n)):"-close"==q?h=1:"-close2"==q?d=1:(f||(w.text(""),w.append("<p>Unexpected results:</p>"),g=f=1),w.append("<p>"+q+" => "+n+"</p>"))});ga.value.saveLastValues(m);ga.value.saveLastValue(m,u);$(r+"_progress").empty();k&&syncState();h&&ga.msg.close(1);d&&
ga.msg.close(2);return y};
ga.data.textarea=function(a,b){var c=a+"_textarea",e=$(c);isatend=e[0].scrollHeight-e[0].scrollTop===e[0].clientHeight;b||(b="");e.is(":hidden")&&(e.show(),$(c+"_label").show());"__reset__\n"==b.substr(0,10)?e.val(b.substr(10)):e.val(e.val()+b);ga.set("textarea:rows")?ga.data.textarea.h[a]?e.height(ga.data.textarea.h[a]):ga.data.textarea.h[a]=parseFloat(e.prop("clientHeight"))+parseFloat(e.css("borderTopWidth"))+parseFloat(e.css("borderBottomWidth")):e.height(parseFloat(e.prop("scrollHeight"))+parseFloat(e.css("borderTopWidth"))+
parseFloat(e.css("borderBottomWidth")));isatend&&e.scrollTop(e[0].scrollHeight)};ga.data.textarea.h={};ga.data.airavata=function(a,b){a=$(a+"_airavata");a.is(":hidden")&&a.show();a.html(b)};
...
b&&a.args[1].op&&(a.args[1]=ga.calc.evaltree(a.args[1]));e&&(a.args[0]=ga.calc.arraytovals(a.args[0]),b&&(a.args[1]=ga.calc.arraytovals(a.args[1])));e=e&&a.args[0].constructor===Array;if(b){var g=a.args[1].constructor===Array;var f=e||g;if(c=e&&g)if(a.args[0].length<a.args[1].length){var k=a.args[0];var h=a.args[1];var d=1}else k=a.args[1],h=a.args[0],d=0;else if(f){h=a.args[e?0:1];var m=a.args[e?1:0]}}switch(a.op){case ",":f=e?a.args[0]:[a.args[0]];b&&"undefined"!=typeof a.args[1]&&(f=f.concat(g?
a.args[1]:[a.args[1]]));break;case "()":b?(f=e?a.args[0]:[a.args[0]],b&&a.args[1]&&(f=f.concat(g?a.args[1]:[a.args[1]]))):f=a.args[0];break;case "+":if(!b)return{_error:"operator : "+a.op+" is binary and only has one argument"};if(f)if(f=h,c)for(b=0;b<k.length;++b)f[b]+=k[b];else for(b=0;b<h.length;++b)f[b]+=m;else f=a.args[0]+a.args[1];break;case "*":if(!b)return{_error:"operator : "+a.op+" is binary and only has one argument"};if(f)if(f=h,c)for(b=0;b<k.length;++b)f[b]*=k[b];else for(b=0;b<h.length;++b)f[b]*=
m;else f=a.args[0]*a.args[1];break;case "-":if(!b)return{_error:"operator : "+a.op+" is binary and only has one argument"};if(f)if(c){f=a.args[0];for(b=0;b<k.length;++b)f[b]-=a.args[1][b];if(d)for(b=a.args[0].length;b<a.args[1].length;++b)f.push(-a.args[1][b]);else for(b=a.args[1].length;b<a.args[0].length;++b)f.push(a.args[0][b])}else if(e)for(f=a.args[0],b=0;b<a.args[0].length;++b)f[b]-=a.args[1];else for(f=[],b=0;b<a.args[1].length;++b)f.push(a.args[0]-a.args[1][b]);else f=a.args[0]-a.args[1];
...
"text-align:"+(d.grid&&d.grid.align?d.grid.align:ga.grid.align)+";";k[d.id]&&(e+="Duplicate id in _question fields:"+d.id+". ");k[d.id]=1;if(d.help){var l=' class="help_link"';help_span='<span class="help">'+d.help+"</span>"}else help_span=l="";switch(d.type){case "label":ga.grid.newrow(m);a+=ga.grid.next(m,d.grid?d.grid.label:null,r)+"<label"+l+">";d.label&&(a+=d.label);a+="</label>"+help_span+"</div>";break;case "file":ga.grid.newrow(m);d.label&&(a+=ga.grid.next(m,d.grid?d.grid.label:null,r)+'<label for="'+
d.id+'"'+l+">"+d.label+"</label></div>");a+=ga.grid.next(m,d.grid?d.grid.data:null,r)+'<input type="file" id="'+d.id+'" name="'+d.id+'[]"'+l;d.required&&(a+=" required");d.multiple&&(a+=" multiple");d.accept&&(a+=' accept="'+d.accept+'"');a+=">"+help_span+"</div>";break;case "lrfile":ga.grid.newrow(m);d.label&&(a+=ga.grid.next(m,d.grid?d.grid.label:null,r)+'<label for="'+d.id+'"'+l+">"+d.label+"</label></div>");a+=ga.grid.next(m,d.grid?d.grid.data:null,r)+'<input type="file" id="'+d.id+'" name="'+
d.id+'[]" data-add="'+d.id+'_altval"'+l;d.required&&(a+=" required");d.multiple&&(a+=" multiple");d.accept&&(a+=' accept="'+d.accept+'"');a+=">"+help_span+' or <button id="'+d.id+'_button" name="'+d.id+'_button" data-type="lrfile"'+l+'><span class="buttontext">Browse server</span></button>'+help_span+'<span id="'+d.id+'_altval"></span><input type="hidden" name="_selaltval_'+d.id+'" value="'+d.id+'_altval"</input></div>';g+='ga.altfile("'+h+'","'+d.id+'","'+d.id+'_altval" );$( "#'+h+'" ).change( function(){ $( "#'+
d.id+'_altval" ).html( "<i>Local</i>: " + $( "#'+d.id+'" ).val().replace(/^C:.fakepath./,""));});ga.altfile.button( "'+h+'","'+d.id+'","'+d.label+'","rfile",function(v){ga.altfile.button.lrfile(v,"'+h+'","'+d.id+'")}';d.required&&(g+=',"lrfile"');g+=");";g+='$("#'+d.id+'_button").on("click",function(){return ga.altfile.button.call("'+h+'","'+d.id+'");});';break;case "text":ga.grid.newrow(m);d.label&&(a+=ga.grid.next(m,d.grid?d.grid.label:null,r)+'<label for="'+d.id+'"'+l+">"+d.label+"</label></div>");
a+=ga.grid.next(m,d.grid?d.grid.data:null,r)+'<input type="text" id="'+d.id+'" name="'+d.id+'"'+l;d.required&&(a+=" required");d.readonly&&(a+=" readonly");d["default"]&&(a+=' value="'+d["default"]+'"');d.pattern&&(a+=' pattern="'+d.pattern+'"');d.maxlength&&(a+=' maxlength="'+d.maxlength+'"');d.size&&(a+=' size="'+d.size+'"');a+=">"+help_span+"</div>";break;case "integer":ga.grid.newrow(m);d.label&&(a+=ga.grid.next(m,d.grid?d.grid.label:null,r)+'<label for="'+d.id+'"'+l+">"+d.label+"</label></div>");
a+=ga.grid.next(m,d.grid?d.grid.data:null,r)+'<input type="number" id="'+d.id+'" step="1" name="'+d.id+'"'+l;d.required&&(a+=" required");d.readonly&&(a+=" readonly");d["default"]&&(a+=' value="'+d["default"]+'"');d.pattern&&(a+=' pattern="'+d.pattern+'"');d.min&&(a+=' min="'+d.min+'"');d.max&&(a+=' max="'+d.max+'"');a+=">"+help_span+"</div>";break;case "float":ga.grid.newrow(m);d.label&&(a+=ga.grid.next(m,d.grid?d.grid.label:null,r)+'<label for="'+d.id+'"'+l+">"+d.label+"</label></div>");a+=ga.grid.next(m,
d.grid?d.grid.data:null,r)+'<input type="number" id="'+d.id+'" name="'+d.id+'"'+l;d.required&&(a+=" required");d.readonly&&(a+=" readonly");d.required&&(a+=" required");d["default"]&&(a+=' value="'+d["default"]+'"');d.pattern&&(a+=' pattern="'+d.pattern+'"');d.step&&(a+=' step="'+d.step+'"');d.min&&(a+=' min="'+d.min+'"');d.max&&(a+=' max="'+d.max+'"');a+=">"+help_span+"</div>";break;case "textarea":ga.grid.newrow(m);d.label&&(a+=ga.grid.next(m,d.grid?d.grid.label:null,r)+'<label for="'+d.id+'"'+
l+">"+d.label+"</label></div>");a+=ga.grid.next(m,d.grid?d.grid.data:null,r)+'<textarea id="'+d.id+'" name="'+d.id+'"'+l;d.required&&(a+=" required");a+=" readonly";d.fontfamily&&(a+=' style="font-family: '+d.fontfamily+';"');d.cols&&(a+=" cols="+d.cols);d.rows&&(a+=" rows="+d.rows);a+=">";d["default"]&&(a+=d["default"]);a+="</textarea>"+help_span+"</div>";break;case "checkbox":ga.grid.newrow(m);d.label&&(a+=ga.grid.next(m,d.grid?d.grid.label:null,r)+'<label for="'+d.id+'"'+l+">"+d.label+"</label></div>");
a+=ga.grid.next(m,d.grid?d.grid.data:null,r)+'<input type="checkbox" id="'+d.id+'" name="'+d.id+'"'+l;d.checked&&(a+=" checked");d.readonly&&(a+=" readonly");a+=">"+help_span+"</div>";break;case "listbox":ga.grid.newrow(m);if(d.header){d.label&&ga.grid.next(m,d.grid?d.grid.label:null);a+='<div style="';if(d.fontfamily||d.fontsize)d.fontfamily&&(a+="font-family:"+d.fontfamily+";"),d.fontsize&&(a+="font-size:"+d.fontsize+";");a+=ga.grid.nextstyle(m,d.grid?d.grid.data:null,r);a+='"';d.width&&(d.header=
d.header.padEnd(d.width));d.header=d.header.replace(/ /g,"&nbsp;");a+=">"+d.header+"</div>";ga.grid.newrow(m)}d.label&&(a+=ga.grid.next(m,d.grid?d.grid.label:null,r)+'<label for="'+d.id+'"'+l+">"+d.label+"</label></div>");a+=ga.grid.next(m,d.grid?d.grid.data:null,r)+'<select id="'+d.id+'" name="'+d.id+'"'+l;d.required&&(a+=" required");d.fontfamily&&(a+=' style="font-family: '+d.fontfamily+';"');d.size&&(a+=" size="+d.size);d.multiple&&(a+=" multiple");a+=">";if(!d.values){e+="No values for listbox "+
d.id+". ";break}if(d.returns){if(d.returns&&d.returns.length!=d.values.length){e+="Listbox values length ("+d.values.length+") does not equal return length ("+d.returns.length+") for listbox "+d.id+". ";break}for(l=0;l<d.values.length;++l)a+='<option value="'+d.returns[l]+'">'+d.values[l].replace(/ /g,"&nbsp")+"</option>"}else for(l=0;l<d.values.length;++l)a+='<option value="'+l+'">'+d.values[l].replace(/ /g,"&nbsp")+"</option>";d["default"]&&(g+='$("#'+d.id+" option[value='"+d["default"]+'\']").attr("selected", "true");');
...
(e+="Duplicate id in _question fields:"+d.id+". "),k[d.id]=1,d.help?(l=' class="help_link"',m=' class="highlight help_link"',help_span='<span class="help">'+d.help+"</span>"):(l="",m=' class="highlight"',help_span=""),d.type){case "label":a+="<tr><td colspan=2><label"+l+">";d.label&&(a+=d.label);a+="</label>"+help_span+"</td></tr>";break;case "file":a+="<tr><td>";d.label&&(a+='<label for="'+d.id+'"'+l+">"+d.label+"</label>");a+='</td><td><input type="file" id="'+d.id+'" name="'+d.id+'[]"'+l;d.required&&
(a+=" required");d.multiple&&(a+=" multiple");d.accept&&(a+=' accept="'+d.accept+'"');a+=">"+help_span+"</td></tr>";break;case "lrfile":a+="<tr><td>";d.label&&(a+='<label for="'+d.id+'"'+l+">"+d.label+"</label>");a+='</td><td><input type="file" id="'+d.id+'" name="'+d.id+'[]" data-add="'+d.id+'_altval"'+l;d.required&&(a+=" required");d.multiple&&(a+=" multiple");d.accept&&(a+=' accept="'+d.accept+'"');a+=">"+help_span+' or <button id="'+d.id+'_button" name="'+d.id+'_button" data-type="lrfile"'+l+
...
h+'","'+d.id+'")}';d.required&&(g+=',"lrfile"');g+=");";g+='$("#'+d.id+'_button").on("click",function(){return ga.altfile.button.call("'+h+'","'+d.id+'");});';break;case "text":a+="<tr><td>";d.label&&(a+='<label for="'+d.id+'"'+l+">"+d.label+"</label>");a+='</td><td><input type="text" id="'+d.id+'" name="'+d.id+'"'+l;d.required&&(a+=" required");d.readonly&&(a+=" readonly");d["default"]&&(a+=' value="'+d["default"]+'"');d.pattern&&(a+=' pattern="'+d.pattern+'"');d.maxlength&&(a+=' maxlength="'+d.maxlength+
'"');d.size&&(a+=' size="'+d.size+'"');a+=">"+help_span+"</td></tr>";break;case "integer":a+="<tr><td>";d.label&&(a+='<label for="'+d.id+'"'+l+">"+d.label+"</label>");a+='</td><td><input type="number" id="'+d.id+'" step="1" name="'+d.id+'"'+l;d.required&&(a+=" required");d.readonly&&(a+=" readonly");d["default"]&&(a+=' value="'+d["default"]+'"');d.pattern&&(a+=' pattern="'+d.pattern+'"');d.min&&(a+=' min="'+d.min+'"');d.max&&(a+=' max="'+d.max+'"');a+=">"+help_span+"</td></tr>";break;case "float":a+=
"<tr><td>";d.label&&(a+='<label for="'+d.id+'"'+l+">"+d.label+"</label>");a+='</td><td><input type="number" id="'+d.id+'" name="'+d.id+'"'+l;d.required&&(a+=" required");d.readonly&&(a+=" readonly");d.required&&(a+=" required");d["default"]&&(a+=' value="'+d["default"]+'"');d.pattern&&(a+=' pattern="'+d.pattern+'"');d.step&&(a+=' step="'+d.step+'"');d.min&&(a+=' min="'+d.min+'"');d.max&&(a+=' max="'+d.max+'"');a+=">"+help_span+"</td></tr>";break;case "textarea":a+="<tr><td>";d.label&&(a+='<label for="'+
d.id+'"'+l+">"+d.label+"</label>");a+='</td><td><textarea id="'+d.id+'" name="'+d.id+'"'+l;d.required&&(a+=" required");a+=" readonly";d.fontfamily&&(a+=' style="font-family: '+d.fontfamily+';"');d.cols&&(a+=" cols="+d.cols);d.rows&&(a+=" rows="+d.rows);a+=">";d["default"]&&(a+=d["default"]);a+="</textarea>"+help_span+"</td></tr>";break;case "checkbox":a+="<tr><td>";d.label&&(a+='<label for="'+d.id+'"'+m+">"+d.label+"</label>");a+='</td><td><input type="checkbox" id="'+d.id+'" name="'+d.id+'"'+l;
d.checked&&(a+=" checked");d.readonly&&(a+=" readonly");a+=">"+help_span+"</td></tr>";break;case "listbox":if(d.header){a+="<tr><td colspan=2";if(d.fontfamily||d.fontsize)a+=' style="',d.fontfamily&&(a+="font-family:"+d.fontfamily+";"),d.fontsize&&(a+="font-size:"+d.fontsize+";"),a+='"';d.width&&(d.header=d.header.padEnd(d.width));d.header=d.header.replace(/ /g,"&nbsp;");a+=">"+d.header+"</td></tr>"}a+="<tr><td>";d.label&&(a+='<label for="'+d.id+'"'+m+">"+d.label+"</label>");a+="</td>";a=d.size&&
1<d.size?a+"</tr><tr><td colspan=2>":a+"<td>";a+='<select id="'+d.id+'" name="'+d.id+'"'+l;d.required&&(a+=" required");d.fontfamily&&(a+=' style="font-family: '+d.fontfamily+';"');d.size&&(a+=" size="+d.size);d.multiple&&(a+=" multiple");a+=">";if(!d.values){e+="No values for listbox "+d.id+". ";break}if(d.returns){if(d.returns&&d.returns.length!=d.values.length){e+="Listbox values length ("+d.values.length+") does not equal return length ("+d.returns.length+") for listbox "+d.id+". ";break}for(l=
0;l<d.values.length;++l)a+='<option value="'+d.returns[l]+'">'+d.values[l].replace(/ /g,"&nbsp")+"</option>"}else for(l=0;l<d.values.length;++l)a+='<option value="'+l+'">'+d.values[l].replace(/ /g,"&nbsp")+"</option>";d["default"]&&(g+='$("#'+d.id+" option[value='"+d["default"]+'\']").attr("selected", "true");');a+="</select>"+help_span+"</td></tr>";break;default:e+="Unknown or currently unsupported field:type "+d.type+". "}a+="</table></form>"}if(b._question.buttons&&b._question.buttons.length)for(c=
...
ga.layout.process=function(){ga.layout.fields.b_submit||(ga.layout.fields.b_submit={},ga.layout.fields.b_submit.lhtml='<label class=""></label>',ga.layout.fields.b_submit.dhtml='<button id="b_submit_button"><span class="buttontext">Submit</span></button><div id="b_submit_buttonval"></div>',ga.layout.fields.b_submit.eval="");ga.layout.fields.b_reset||(ga.layout.fields.b_reset={},ga.layout.fields.b_reset.lhtml='<label class=""></label>',ga.layout.fields.b_reset.lhtml+="",ga.layout.fields.b_reset.dhtml=
'<button id="b_reset_button"><span class="buttontext">Reset</span></button><div id="b_reset_buttonval"></div>',ga.layout.fields.b_reset.dhtml+="",ga.layout.fields.b_reset.eval="");ga.layout.panel.panels?ga.layout.setup():console.error("error JSON contains no panels")};
...
"\n",c=0;c<ga.layout.panelfields[a].length;++c){var e="",g="",f="",k=ga.layout.panelfields[a][c].id;ga.layout.panelfields[a][c].role&&(f+="ga-field-"+ga.layout.panelfields[a][c].role+" ",ga.layout.buttonsused||"output"!=ga.layout.panelfields[a][c].role||(b+=ga.layout.buttons()));ga.layout.panelfields[a][c].type&&(f+="ga-type-"+ga.layout.panelfields[a][c].type+" ");ga.layout.panelfields[a][c].lgr&&(e+="grid-row:"+ga.layout.panelfields[a][c].lgr+";");ga.layout.panelfields[a][c].lgc&&(e+="grid-column:"+
ga.layout.panelfields[a][c].lgc+";");ga.layout.panelfields[a][c].layout.align&&(e+="text-align:"+ga.layout.panelfields[a][c].layout.align+";");ga.layout.panelfields[a][c].dgr&&(g+="grid-row:"+ga.layout.panelfields[a][c].dgr+";");ga.layout.panelfields[a][c].dgc&&(g+="grid-column:"+ga.layout.panelfields[a][c].dgc+";");ga.layout.panelfields[a][c].layout.align&&(g+="text-align:"+ga.layout.panelfields[a][c].layout.align+";");ga.layout.fields[k].lhtml&&ga.layout.fields[k].lhtml.length&&(b+="<div id=ga-label-"+
