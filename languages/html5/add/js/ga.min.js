...
label:"Submit",cb:ga.login.verify.change["do"]},{id:"_changeemailcancel",label:"Cancel"}]})};
ga.login.verify.change["do"]=function(){var a=$("#sys_login");if(!a)return messagebox({icon:"toast.png",text:"Internal error: form missing"});a.append('<input type="hidden" name="_resendverify" class="toclear">');a.append('<input type="hidden" name="_changeemail" class="toclear">');a.append('<input type="hidden" name="_changeemail1" value="'+$("#_changeemail1").val()+'" class="toclear">');a.append('<input type="hidden" name="_changeemail2" value="'+$("#_changeemail2").val()+'" class="toclear">');
do_sys_login_submit(a)};ga.login.verify.cancel=function(){var a=$("#sys_login");if(!a)return messagebox({icon:"toast.png",text:"Internal error: form missing"});a.append('<input type="hidden" name="_cancel" class="toclear">');do_sys_login_submit(a)};
...
ga.login.approve.resend=function(){var a=$("#sys_login");if(!a)return messagebox({icon:"toast.png",text:"Internal error: form missing"});a.append('<input type="hidden" name="_resendapprove" class="toclear">');do_sys_login_submit(a)};ga.login.approve.cancel=function(){var a=$("#sys_login");if(!a)return messagebox({icon:"toast.png",text:"Internal error: form missing"});a.append('<input type="hidden" name="_cancel" class="toclear">');do_sys_login_submit(a)};ga.admin={};ga.admin.ajax=function(a,b,c,d){};
ga.colors=function(a){ga.colors.background=ga.colors.makeRGB(a.background);ga.colors.text=ga.colors.makeRGB(a.text)};ga.colors.makeRGB=function(a){if(/\d{1,3},\s*\d{1,3},\s*\d{1,3}$/.test(a))return a;a=ga.color.toRGB(a);return a.r+","+a.g+","+a.b};ga.plot_options=function(){var a="rgb( "+ga.colors.text+" )";return{font:{color:a},grid:{hoverable:!1},xaxis:{color:"gray",lineWidth:.5,font:{color:a}},yaxis:{color:"gray",lineWidth:.5,font:{color:a}},lines:{lineWidth:1},zoom:{interactive:!1},pan:{interactive:!1}}};
ga.color=function(a){ga.color.data=a;ga.colors(a.body);ga.color.apply()};ga.color.defaults=function(a){ga.browser();ga.color.defaults.data=a;ga.color(a)};ga.color.toRGB=function(a){var b,c;if("#"===a.slice(0,1))return c=parseInt(a.slice(1),16),b=parseInt(c/256),c-=256*b,a=parseInt(b/256),{r:a,b:c,g:b-256*a};(bits=/^rgb\(\s*(\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})\s*\)$/.exec(a))?(a=parseInt(bits[1]),b=parseInt(bits[2]),c=parseInt(bits[3])):c=b=a=128;return{r:a,b:c,g:b}};
...
b&&(A.job_status=m),/^_getinput/.test(b)&&"_getinput"==b&&ga.valuen.input(a,m),"_textarea"==b&&ga.data.textarea(p,m),"_airavata"==b&&ga.data.airavata(p,m),"_loginverify"==b&&ga.login.verify(m),"_loginapprove"==b&&ga.login.approve(m)):"-close"==b?h=1:"-close2"==b?n=1:(f||(w.text(""),w.append("<p>Unexpected results:</p>"),e=f=1),w.append("<p>"+b+" => "+m+"</p>"))});ga.value.saveLastValues(k);ga.value.saveLastValue(k,x);$(p+"_progress").html("");g&&syncState();h&&closeModal();n&&closeModal2();return A};
