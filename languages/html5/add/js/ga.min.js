...
Object.size=function(a){var c=0,b;for(b in a)a.hasOwnProperty(b)&&c++;return c};ga.set=function(a,c){c&&(ga.set.data[a]=c);return ga.set.data[a]};ga.set.data={};ga.restricted={};ga.restricted.add=function(a,c){ga.restricted.ids[a]=ga.restricted.ids[a]||[];ga.restricted.ids[a].push(c)};ga.restricted.hideall=function(){var a;ga.restricted.data={};for(a in ga.restricted.ids)$(ga.restricted.ids[a].join()).hide()};
ga.restricted.show=function(a){for(var c in a)ga.restricted.data[a[c]]=1,ga.restricted.ids[a[c]]&&$(ga.restricted.ids[a[c]].join()).show()};ga.specproj=function(a,c){var b={};b.id=a;b.value=c;ga.specproj.data.push(b)};ga.specproj.data=[];ga.specproj.clear=function(){ga.specproj.data=[]};
...
text:"Internal error: no url defined for cache clear"})};ga.trytilltrue=function(a,c,b,e){if(eval(a))return eval(c);b--;if(0>b)console.warn("ga.trytilltrue failed ... to many tries");else return setTimeout(ga.trytilltrue,e,a,c,b,e)};ga.login={};
ga.login.verify=function(a){var c={text:a.text||"You must verify your email address."};a.useroptions&&(a.useroptions.resend&&(c.buttons=c.buttons||[],c.buttons.push({id:"resend",label:"Resend the verification email",cb:ga.login.verify.resend})),a.useroptions.resend&&(c.buttons=c.buttons||[],c.buttons.push({id:"changeaddress",label:"Change your email address and resend the verification",cb:ga.login.verify.change})),a.useroptions.cancel&&(c.buttons=c.buttons||[],c.buttons.push({id:"cancelregistration",
label:"Cancel your registration",cb:ga.login.verify.cancel})));ga.msg.box(c)};ga.login.verify.resend=function(){var a=$("#sys_login");if(!a)return ga.msg.box({icon:"toast.png",text:"Internal error: form missing"});a.append('<input type="hidden" name="_resendverify" class="toclear">');do_sys_login_submit(a)};
...
ga.admin.ajax=function(a,c,b,e,f){$.get(ga.admin.ajax.url,{tagmode:"any",format:"json",_window:window.name,_logon:$("#_state").data("_logon"),_cmd:a,_name:c,_id:b,_manageid:e,_jid:f}).done(function(a,b,c){a=JSON.parse(a.replace(/^\s*[\r\n]/gm,"").split(/\r?\n/)[1]);"true"!=a.success&&ga.msg.box({icon:"toast.png",text:a.error?a.error:"unknown error"});a._submitid&&$("#"+a._submitid).trigger("click")}).fail(function(a,b,c){ga.msg.box({icon:"toast.png",text:"Error: system user management backend command failed to run: "+
c})})};ga.admin.ajax.remove=function(a,c,b,e){ga.msg.box({icon:"admin.png",text:"Are you sure you want to permanently remove this user, all of their job history and their stored data?",buttons:[{id:"yes",label:"Yes, remove this user",cb:ga.admin.ajax,adata:[a,c,b,e]},{id:"cancel",label:"Cancel"}]})};
...
adata:[a,c,b,e]},{id:"cancel",label:"Cancel"}]})};
ga.admin.ajax.group.cb=function(a,c,b,e,f){$.get(ga.admin.ajax.url,{tagmode:"any",format:"json",_window:window.name,_logon:$("#_state").data("_logon"),_cmd:a,_name:c,_id:b,_manageid:e,_group:$("#sys_musergrp_text").val()}).done(function(a,b,c){a=JSON.parse(a.replace(/^\s*[\r\n]/gm,"").split(/\r?\n/)[1]);"true"!=a.success&&ga.msg.box({icon:"toast.png",text:a.error?a.error:"unknown error"});a._submitid&&$("#"+a._submitid).trigger("click")}).fail(function(a,b,c){ga.msg.box({icon:"toast.png",text:"Error: system user management backend command failed to run: "+
...
ga.extrahidden=function(a){if(ga.set.data.extrahidden&&ga.set.data.extrahidden[a]){var c=$("#"+a),b,e="";for(b in ga.set.data.extrahidden[a])e+='<input type="hidden" name="'+b+'" value="'+ga.set.data.extrahidden[a][b]+'">';c.append(e);delete ga.set.data.extrahidden[a]}};ga.hhelp={};ga.hhelp.reset=function(){if($("#global_data").data("hoverhelp")||0!=$("#global_data").data("hoverhelp"))$(".help_link").removeClass("help_link_on"),$(".help_link").addClass("help_link_on")};
...
ga.color.toRGB=function(a){var c,b;if("#"===a.slice(0,1))return b=parseInt(a.slice(1),16),c=parseInt(b/256),b-=256*c,a=parseInt(c/256),{r:a,b:b,g:c-256*a};(bits=/^rgb\(\s*(\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})\s*\)$/.exec(a))?(a=parseInt(bits[1]),c=parseInt(bits[2]),b=parseInt(bits[3])):b=c=a=128;return{r:a,b:b,g:c}};
ga.color.apply=function(){var a,c,b,e,f,d,g;if(ga.directives.data&&ga.directives.data.usercolors&&!/^(off|0|false)$/.test(ga.directives.data.usercolors)){for(a in ga.color.data)ga.color.data.hasOwnProperty(a)&&(c=$(a))&&c.css(ga.color.data[a]);$(".sidebar ul li, .title").css("color",ga.color.data.body.color).hover(function(){$(this).css("color",ga.color.defaults.data.hovercolor.color)},function(){$(this).css("color",ga.color.data.body.color)});$(".svgmenu").css({color:ga.color.data.body.color,stroke:ga.color.data.body.color}).hover(function(){$(this).css({color:ga.color.defaults.data.hovercolor.color,
...
ga.event=function(a,c,b){ga.event.log.push({menu:a,module:c,action:b,when:new Date})};ga.event.log=[];ga.event.list=function(){for(var a=0,c=ga.event.log.length,b="Client Date/Time is "+(new Date).toUTCString()+"\n";a<c;a++)b+=ga.event.log[a].menu+" "+ga.event.log[a].module+" "+ga.event.log[a].action+" "+ga.event.log[a].when.toUTCString()+"\n";return b};
ga.fc=function(a,c){var b,e;ga.fc.cache[a]?c(ga.fc.cache[a]):ga.fc.waits[a]?ga.fc.waits[a].push(c):(ga.fc.waits[a]=[c],$.ajax(ga.fc.url+a).success(function(c){e=ga.fc.waits[a];delete ga.fc.waits[a];c=$.parseJSON(c);ga.fc.cache[a]=c;for(b=e.length;b--;)e[b](c)}).error(function(c){e=ga.fc.waits[a];delete ga.fc.waits[a];console.log("ajax error");console.dir(c);for(b=e.length;b--;)e[b]("**error**");ga.fc.ajax_error_msg("ajax get error: "+c.statusText)}));return!0};
ga.fc.clear=function(a){var c,b=ga.fc.children(a);for(c in b)ga.fc.cache[c]&&delete ga.fc.cache[c];ga.fc.cache[a]&&delete ga.fc.cache[a];for(c in ga.fc.trees)$(c).length&&("#"!==a?$(c).jstree(!0).refresh_node(a):$(c).jstree(!0).refresh())};ga.fc.refresh=function(a){for(var c in ga.fc.trees)$(c).length&&ga.fc.cache[a]&&("#"!==a?$(c).jstree(!0).refresh_node(a):$(c).jstree(!0).refresh())};
...
a.join(",")))};ga.fc.delete_node_message=function(a){var c="You are about to permanently remove "+a.length+" file";if(!a.length)return"Can not remove a directory from here";c=1<a.length?c+"s and/or directories":c+" or directory";return c+" and the contents, including subdirectories, of any directory listed below<p>"};
...
ga.altfile.add=function(a){var c,b="",e="#"+a;$(e+" ._hidden_altfiles").remove();$(e+" ._hidden_buttonvals").remove();if(ga.altfile.data[a])for(c in ga.altfile.data[a])b+='<input type="hidden" name="_selaltval_'+c+'" value="'+ga.altfile.data[a][c]+'" class="_hidden_altfiles">';if(ga.altfile.bdata[a])for(c in ga.altfile.bdata[a])b+='<input type="hidden" name="'+c+'" value="'+ga.altfile.bdata[a][c].val+'" class="_hidden_buttonvals">';b.length&&$(e).append(b)};
ga.altfile.button=function(a,c,b,e,f,d){ga.altfile.bdata[a]=ga.altfile.bdata[a]||{};ga.altfile.bdata[a][c]={};ga.altfile.bdata[a][c].val={};ga.altfile.bdata[a][c].text=b;ga.altfile.bdata[a][c].call=e;ga.altfile.bdata[a][c].cb=f;ga.altfile.bdata[a][c].req=d||0};ga.altfile.button.value=function(a,c,b){ga.altfile.bdata[a][c].val=b};
ga.altfile.button.call=function(a,c){var b;ga.altfile.bdata[a][c].call.length?(b=$("#_state").data("_logon"))&&b.length?($("#configbody").load("etc/"+ga.altfile.bdata[a][c].call+".html",function(){ga.altfile.bdata[ga.altfile.bdata[a][c].call]={};ga.altfile.bdata[ga.altfile.bdata[a][c].call].cb=ga.altfile.bdata[a][c].cb;$("#"+ga.altfile.bdata[a][c].call+"text_label").text(ga.altfile.bdata[a][c].text)}),ga.repeats.save(),$(".modalDialog").addClass("modalDialog_on")):ga.msg.box({icon:"warning.png",text:"You must login to browse server information",
buttons:[{id:"ok",label:"OK"}]}):ga.altfile.bdata[a][c].cb("cb");return!1};ga.altfile.button.simplecall=function(a,c){$("#configbody").load("ajax/"+a+"/"+c+".html",function(){});ga.repeats.save();$(".modalDialog").addClass("modalDialog_on");return!1};ga.altfile.button.cb=function(){ga.msg.close(1)};
ga.altfile.button.lrfile=function(a,c,b){var e=[],f="#"+c,d="#"+b,g="",h="_hidden_lrfile_sels_"+b;$(f+" ."+h).remove();$.each($(a).jstree("get_checked",!0),function(){this.children.length||(g+='<input type="hidden" name="'+b+'_altval[]" value="'+this.id+'" class="'+h+'" data-type="rfile_val">',e.push($.base64.decode(this.id).substr(2)))});e.length&&($(d+"_altval").html("<i>Server</i>: "+e),$(d+"_msg").empty(),$(d).val(""),$(f).append(g),ga.value.input[c]&&ga.value.input[c][b]&&ga.value.setInputfromRFile(e,
ga.value.input[c][b].mode,ga.value.input[c][b].ids))};
...
ga.altfile.button.rfile=function(a,c,b){var e=[],f="#"+c,d="#"+b,g="",h="_hidden_rfile_sels_"+b;$(a).jstree(!0);$(f+" ."+h).remove();$.each($(a).jstree("get_checked",!0),function(){this.children.length||(g+='<input type="hidden" name="'+b+'_altval[]" value="'+this.id+'" class="'+h+'" data-type="rfile_val">',e.push($.base64.decode(this.id).substr(2)))});e.length&&($(d+"_altval").html("<i>Server</i>: "+e),$(d+"_msg").empty(),$(d).val(""),$(f).append(g),ga.value.input[c]&&ga.value.input[c][b]&&ga.value.setInputfromRFile(e,
ga.value.input[c][b].mode,ga.value.input[c][b].ids))};ga.altfile.button.job=function(a,c,b){var e="";c="#"+c;var f="#"+b,d="",g="_hidden_job_sels_"+b;$(a).jstree(!0);$(c+" ."+g).remove();$.each($(a).jstree("get_checked",!0),function(){this.children.length||(d+='<input type="hidden" name="'+b+'_altval[]" value="'+this.id+'" class="'+g+'">',e+="<tr><td>"+this.parent+":"+this.text+"</td></tr>")});e.length&&($(f+"_altval").html("<table>"+e+"</table>"),$(f+"_msg").empty(),$(f).val(""),$(c).append(d))};
ga.altfile.button.getnames=function(a,c){var b=[];switch(c){case "rpath":b.push(a+"[]");break;case "rfile":b.push(a+"_altval[]");break;case "lrfile":a=a.replace(/_button$/,"");b.push(a+"_altval[]");break;default:console.warn("ga.altfile.button.getnames( "+a+" , "+c+" )")}return b};
ga.altfile.button.getnamesinput=function(a,c){var b=[];switch(c){case "rpath":b.push(a);break;case "rfile":b.push(a+"_altval");break;case "lrfile":a=a.replace(/_button$/,"");b.push(a+"_altval");break;default:console.warn("ga.altfile.button.getnames( "+a+" , "+c+" )")}return b};
ga.altfile.button.addhtml=function(a,c,b,e){var f="";switch(b){case "rpath":b="_hidden_rpath_sels_"+c;f+='<input type="hidden" name="'+c+'[]" value="'+e[0]+'" class="'+b+'"><input type="hidden" name="_decodepath_'+c+'" class="'+b+'">';break;case "rfile":f+='<input type="hidden" name="'+c+'_altval[]" value="'+e[0]+'" class="'+("_hidden_rfile_sels_"+c)+'">';break;case "lrfile":c=c.replace(/_button$/,"");f+='<input type="hidden" name="'+c+'_altval[]" value="'+e[0]+'" class="'+("_hidden_lrfile_sels_"+
...
ga.valid.checkFloat=function(a){var c=$(a),b=c.val(),e=0;b.match(/^-?(([1-9][0-9]*)|(0))?([.][0-9]+)?([eE][-+]?[0-9]+)?$/)?(e=1,b<parseFloat(c.attr("min"))?(c.val(c.attr("min")),$(a+"_msg").html(" value set to minimum allowed")):b>parseFloat(c.attr("max"))?(c.val(c.attr("max")),$(a+"_msg").html(" value set to maximum allowed")):$(a+"_msg").empty()):b.length||c.prop("required")?$(a+"_msg").html(" wrong format"):e=1;return e};
ga.valid.checkInt=function(a){var c=$(a),b=c.val(),e=0;b.match(/^-?((0)|([1-9][0-9]*))$/)?(e=1,b<parseInt(c.attr("min"))?(c.val(c.attr("min")),$(a+"_msg").html(" value set to minimum allowed")):b>parseInt(c.attr("max"))?(c.val(c.attr("max")),$(a+"_msg").html(" value set to maximum allowed")):parseInt(b)!=b?($(a+"_msg").html(" value rounded to nearset integer"),c.val(parseInt(parseFloat(b)+.5))):$(a+"_msg").empty()):b.length||c.prop("required")?$(a+"_msg").html(" wrong format"):e=1;return e};
ga.valid.safeFile=function(a){var c=$(a);c.val().match("^[a-zA-Z0-9]+([a-zA-Z0-9_.-]+|/[a-zA-Z0-9_-])+$")?$(a+"_msg").empty():(c.val(c.prop("defaultValue")),$(a+"_msg").html("Not an acceptable filename, reset to default"))};ga.valid.checkLrfile=function(a){var c=$(a),b=$(a+"_altval > i");a=$(a+"_msg");var e=0;if(!c||!c.is(":visible"))return 1;c&&c.val()&&c.val().length?e=1:b&&b.html()&&b.html().length&&"Server"===b.html()&&(e=1);e||a.html(" missing required field");return e};
ga.valid.checkRpath=function(a){var c=$(a),b=$(a+"_altval > i");a=$(a+"_msg");var e=0;if(!c||!c.is(":visible"))return 1;b&&b.html()&&b.html().length&&"Server"===b.html()&&(e=1);e||a.html(" missing required field");return e};ga.valid.checkRfile=function(a){var c=$(a),b=$(a+"_altval > i");a=$(a+"_msg");var e=0;if(!c||!c.is(":visible"))return 1;b&&b.html()&&b.html().length&&"Server"===b.html()&&(e=1);e||a.html(" missing required field");return e};
ga.valid.checksubmit=function(a){var c,b=1;if(!ga.altfile.bdata[a]&&!ga.value.types[a])return 1;ga.valid.clearerrorcounter(a);for(c in ga.altfile.bdata[a])if(ga.altfile.bdata[a][c].req)switch(ga.altfile.bdata[a][c].req){case "lrfile":b=b&&ga.valid.checkLrfile("#"+c);$("#"+c).length&&!ga.valid.checkLrfile("#"+c)&&++ga.fielderrors[a];break;case "rpath":b=b&&ga.valid.checkRpath("#"+c);$("#"+c).length&&!ga.valid.checkRpath("#"+c)&&++ga.fielderrors[a];break;case "rfile":b=b&&ga.valid.checkRfile("#"+c);
$("#"+c).length&&!ga.valid.checkRfile("#"+c)&&++ga.fielderrors[a];break;default:console.log("ga.valid.checksubmit() unsupported required check "+ga.altfile.bdata[a][c].req)}for(c in ga.value.types[a])if(ga.value.types[a][c].req)switch(ga.value.types[a][c].req){case "float":$("#"+c).length&&!ga.valid.checkFloat("#"+c)&&++ga.fielderrors[a];break;case "integer":$("#"+c).length&&!ga.valid.checkInt("#"+c)&&++ga.fielderrors[a];break;case "text":$("#"+c).length&&!ga.valid.checkText("#"+c)&&++ga.fielderrors[a];
break;case "file":$("#"+c).length&&!ga.valid.checkLrfile("#"+c)&&++ga.fielderrors[a];break;default:console.log("ga.valid.checksubmit() unsupported required check "+ga.value.types[a][c].req)}0<ga.fielderrors[a]&&(b=0);return b};ga.valid.showerrormessage=function(a){ga.msg.box({icon:"warning.png",text:""+ga.fielderrors[a]+" fields are missing or not set correctly!",buttons:[{id:"ok",label:"OK"}]});ga.fielderrors[a]=0};ga.valid.clearerrorcounter=function(a){ga.fielderrors[a]=0};ga.airavata={};
...
ga.value.setInputfromFile=function(a,c,b,e){$(a).hide();var f=b.split(",");$(a).change(function(b){b=$(a)[0].files[0];var g=new FileReader;g.onload=function(a){ga.value.processInputfromFiles(a.target.result,c,f,e)};g.readAsText(b)})};
ga.value.setLastValue=function(a,c,b){var e=a+":"+c+":last_value",f=a+":"+c+":default_value",d=$(c);if(!/_output$/.test(a))return!1;if(void 0==$("#global_data").data(e))switch(d.attr("type")){case "checkbox":case "radio":$("#global_data").data(e,d.is(":checked"));$("#global_data").data(f,d.is(":checked"));break;case "div":case "msgs":$("#global_data").data(e,d.html());$("#global_data").data(f,d.html());break;case "plot3d":case "plotly":c=c.replace(/^#/,"");Plotly.newPlot(c,[],{});Plotly.purge(c);
break;case "plot2d":break;case "ngl":ga.ngl.clear(e,c);break;case "bokeh":ga.bokeh.renderdata(a,c.replace(/^#/,""));break;case "filelink":case "filelinkm":$("#global_data").data(e,$(c+"_filelink").html());break;default:b&&d.val(b),$("#global_data").data(e,d.val()),$("#global_data").data(f,d.val())}else switch(d.attr("type")){case "checkbox":case "radio":d.prop("checked",$("#global_data").data(e));break;case "div":case "msgs":d.html($("#global_data").data(e));break;case "atomicstructure":c=c.replace(/^#/,
"");$("#global_data").data(e)?(_jmol_info[c].script=$("#global_data").data(e),d.html(Jmol.getAppletHtml("jmolApplet"+c,_jmol_info[c]))):d.empty();break;case "plot2d":a=gd.data(e);a.data?(ga.value.set.plot2d(c,a.options),d.plot(a.data,ga.value.get.plot2d.plot_options(c,a.options))):d.plot(a,ga.value.get.plot2d.plot_options(c));break;case "plot3d":case "plotly":d=gd.data(e);d.data&&Plotly.plot(c.replace(/^#/,""),d.data,d.layout);break;case "bokeh":break;case "ngl":(d=gd.data(e))&&ga.value.nglshow(a,
c.replace(/^#/,""),d);break;case "filelink":case "filelinkm":$(c+"_filelink").html($("#global_data").data(e));break;default:d.val($("#global_data").data(e))}};
ga.value.saveLastValue=function(a,c){var b=$(c);switch(b.attr("type")){case "file":break;case "checkbox":case "radio":$("#global_data").data(a+":"+c+":last_value",b.is(":checked"));break;case "div":case "msgs":$("#global_data").data(a+":"+c+":last_value",b.html());break;case "plot2d":break;case "bokeh":break;case "filelink":case "filelinkm":$("#global_data").data(a+":"+c+":last_value",$(c+"_filelink").html());break;case "atomicstructure":b=c.replace(/^#/,"");_jmol_info&&_jmol_info[b]&&_jmol_info[b].length?
$("#global_data").data(a+":"+c+":last_value",_jmol_info[b].script):$("#global_data").data(a+":"+c+":last_value","");break;default:$("#global_data").data(a+":"+c+":last_value",b.val())}};ga.value.saveLastValues=function(a){$("#"+a+" :input").each(function(){ga.value.saveLastValue(a,"#"+$(this).attr("id"))})};
...
"_filelink").html(" ");break;case "plot2d":console.log("ga.value.resetDefaultValue() plot2d, t is "+c);$("#global_data").data(a+":"+c+":last_value",[[]]);ga.value.clear.plot2d(c);b.plot([[]],ga.value.get.plot2d.plot_options(c));$(c+"_showcollapse").length&&($(c+"_div").hide(),$(c+"_savetofile").length&&($(c+"_savetofile").hide(),$(c+"_savetofile_link").hide()),$(c+"_changescalex").length&&($(c+"_changescalex").hide(),$(c+"_changescalex_message").hide()),$(c+"_changescaley").length&&($(c+"_changescaley").hide(),
$(c+"_changescaley_message").hide()),$(c+"_showcollapse").length&&$(c+"_showcollapse").addClass("hidden"));break;case "bokeh":ga.bokeh.reset(a,c.replace(/^#/,""));break;case "plot3d":case "plotly":console.log("reset default value for plot3d: "+c);Plotly.purge(c.replace(/^#/,""));$(c+"_showcollapse").length&&$(c+"_showcollapse").addClass("hidden");break;case "image":b.empty();break;case "video":b.empty();break;case "atomicstructure":c.replace(/^#/,"");$("#global_data").data(a+":"+c+":last_value","");
$(c).empty();break;default:b.val(b.attr("value"))}ga.value.saveLastValue(a,c);$(c+"_msg").empty()};ga.value.resetDefaultValues=function(a,c){var b;if(!/_output$/.test(a))return!1;$("#"+a+" :input").each(function(){ga.value.resetDefaultValue(a,"#"+$(this).attr("id"))});ga.sync.reset(a);for(b in ga.value.extra_resets.data)ga.value.resetDefaultValue(a,"#"+b);c&&(ga.value.resetDefaultValue(a,"#"+a+"_msgs"),b="#"+a+"_textarea",ga.value.resetDefaultValue(a,b),$(b).hide(),$(b+"_label").hide())};
...
ga.value.set.plot2d=function(a,c){var b=a+"_xlabel",e=a+"_ylabel";$(a+"_title").html(c.title?c.title:"");$(b).html(c.xlabel?c.xlabel:"");$(e).html(c.ylabel?c.ylabel:"")};ga.value.clear.plot2d=function(a){var c=a+"_xlabel",b=a+"_ylabel";tagxy=a+"_xy";$(a+"_title").empty();$(c).empty();$(b).empty();$(tagxy).empty()};ga.value.set.plot2d.pan=function(a,c){ga.value.settings[a]=ga.value.settings[a]||{};ga.value.settings[a].pan=c?!0:!1};
ga.value.set.plot2d.zoom=function(a,c,b){tagxy=a+"_xy";ga.value.settings[a]=ga.value.settings[a]||{};ga.value.settings[a].zoom=c?!0:!1;c&&(ga.value.settings[a].pkg=b,$(a+"_title,"+a+"_xlabel,"+a+"_ylabel,"+a+"_xy").on("click",ga.value.set.plot2d.zoom.click))};ga.value.set.plot2d.pkg=function(a,c){ga.value.settings[c]=ga.value.settings[c]||{};ga.value.settings[c].pkg=a;$(c+"_title,"+c+"_xlabel,"+c+"_ylabel,"+c+"_xy").on("click",ga.value.set.plot2d.reset)};
ga.value.set.plot2d.reset=function(a){var c="#"+a.target.id.replace(/(_title|_xlabel|_ylabel|_xy)$/,"");a.preventDefault();ga.value.setLastValue(ga.value.settings[c].pkg,c)};ga.value.set.plot2d.hover=function(a,c){ga.value.settings[a]=ga.value.settings[a]||{};ga.value.settings[a].hover=c?!0:!1};ga.value.set.plot2d.selzoom=function(a,c){ga.value.settings[a]=ga.value.settings[a]||{};ga.value.settings[a].selzoom=c?!0:!1};
ga.value.set.plot2d.backgroundcolor=function(a,c){ga.value.settings[a]=ga.value.settings[a]||{};ga.value.settings[a].backgroundcolor=c};ga.value.get.plot2d={};
ga.value.get.plot2d.plot_options=function(a,c){var b=ga.plot_options();ga.value.settings[a].backgroundcolor&&(b.grid={backgroundColor:ga.value.settings[a].backgroundcolor});ga.value.settings[a].selzoom&&(b.selection={mode:"xy"});c&&(c.grid&&(b.grid=$.extend({},b.grid,c.grid)),c.selection&&(b.selection=$.extend({},b.selection,c.selection)));b.pan.interactive=ga.value.settings[a].pan?!0:!1;b.zoom.interactive=ga.value.settings[a].zoom?!0:!1;b.grid.hoverable=ga.value.settings[a].hover?!0:!1;if(c){c.legend&&
(b.legend=c.legend,c.legend.container&&(b.legend.container=$(a+"_legend")));c.xmin&&(b.xaxis.min=c.xmin);c.xmax&&(b.xaxis.max=c.xmax);if(c.xscale)switch(c.xscale){case "log":b.xaxis.transform=function(a){return 0<a?Math.log(a):1E-99};b.xaxis.inverseTransform=function(a){return Math.exp(a)};b.xaxis.tickFormatter=ga.value.plot2d.ticformatter;break;default:console.log("ga.value.get.plot2d.plot_options( "+a+" , options ) has unsupported xscale of "+c.xscale)}c.xtics&&(b.xaxis.ticks=c.xtics);c.ymin&&(b.yaxis.min=
c.ymin);c.ymax&&(b.yaxis.max=c.ymax);if(c.yscale)switch(c.yscale){case "log":b.yaxis.transform=function(a){return 0<a?Math.log(a):1E-99};b.yaxis.inverseTransform=function(a){return Math.exp(a)};b.yaxis.tickFormatter=ga.value.plot2d.ticformatter;break;default:console.log("ga.value.get.plot2d.plot_options( "+a+" , options ) has unsupported yscale of "+c.yscale)}c.ytics&&(b.yaxis.ticks=c.ytics)}return b};ga.value.plot2d={};ga.value.plot2d.zstack={};
...
ga.value.plot2d.ticformatter=function(a,c){var b;if(!c._ehb||a<=c.min)return c._ehb={},c._ehb.pv=a,c._ehb.min=Math.min(c.min,c.max),c._ehb.max=Math.max(c.min,c.max),c._ehb.tmin=c.options.transform(c._ehb.min),c._ehb.tmax=c.options.transform(c._ehb.max),c._ehb.tmaxr=1/c._ehb.tmax,c._ehb.rnge=c._ehb.max-c._ehb.min,ga.value.plot2d.toFP(a,c.tickDecimals);if(!c._ehb.snd)return c._ehb.snd=!0,c._ehb.sndv=a,c._ehb.ptd=(c.options.transform(a)-c._ehb.tmin)*c._ehb.tmaxr,ga.value.plot2d.toFP(a,c.tickDecimals);
if(!c._ehb.tr)return c._ehb.tr=2*Math.abs(a-c._ehb.sndv)/c._ehb.rnge,c._ehb.ptd=Math.abs(c.options.transform(a)-c._ehb.tmin)*c._ehb.tmaxr,ga.value.plot2d.toFP(a,c.tickDecimals);b=(c.options.transform(a)-c._ehb.tmin)*c._ehb.tmaxr;return Math.min(Math.abs(b-c._ehb.ptd),1-b)>=c._ehb.tr?(c._ehb.ptd=b,ga.value.plot2d.toFP(a,c.tickDecimals)):""};ga.ngl={};ga.ngl.types="backbone ball+stick cartoon contact helixorient hyperball label licorice line point ribbon rocket rope spacefill surface trace tube".split(" ");
ga.ngl.clear=function(a,c){ga.ngl[a]&&(ga.ngl[a].stage&&ga.ngl[a].stage.dispose(),delete ga.ngl[a]);$(c+"_plot").empty();$(c+"_buttons").empty();$("#global_data").removeData(a)};
ga.value.nglshow=function(a,c,b){var e="#"+c;ga.ngl=ga.ngl||{};var f=a+":"+e+":last_value";ga.ngl.clear(f,e);b.loadname&&(b.loadparams||(b.loadparams={}),b.representation||(b.representation="cartoon"),ga.ngl[f]={},ga.ngl[f].stage=new NGL.Stage(c+"_plot"),ga.ngl[f].stage.loadFile(b.loadname,b.loadparams).then(function(a){ga.ngl[f].component=a;ga.ngl[f].reps={};ga.ngl[f].reps[b.representation]=a.addRepresentation(b.representation);a.autoView();a=ga.ngl.types.length;for(var c="",h="",m=0;m<a;++m)c+=
'<button id="'+ga.ngl.types[m].replace("+","")+'">'+ga.ngl.types[m]+"</button>",h+='$("#'+ga.ngl.types[m].replace("+","")+'").on("click", function() { var sk = ga.ngl["'+f+'"]; var comp = sk.component; var crep = sk.reps["'+ga.ngl.types[m]+'"]; if ( comp && crep ) { comp.removeRepresentation( crep ); delete sk.reps["'+ga.ngl.types[m]+'"]; } else { sk.reps["'+ga.ngl.types[m]+'"] = comp.addRepresentation("'+ga.ngl.types[m]+'");} return false; });';$(e+"_buttons").html(c);eval(h)}),$("#global_data").data(f,
b));ga.value.extra_resets(c)};ga.pull={};ga.pull.clearPull=function(a){a&&"string"==typeof a&&0!=a.length&&"#__fields:repeat__"!=a||(a="");$("#global_data").data("_pull_json"+a,{});$("#global_data").data("_pull_update"+a,{});$("#global_data").data("_pull_type"+a,{})};
ga.pull.toPull=function(a,c,b,e,f){f&&"string"==typeof f&&0!=f.length&&"#__fields:repeat__"!=f||(f="");a=$("#global_data");var d=a.data("_pull_json"+f)||{},g=a.data("_pull_update"+f)||{};d[e]=0;"object"!=typeof g[e]&&(g[e]={});g[e][c]=b;a.data("_pull_json"+f,d);a.data("_pull_update"+f,g)};
ga.pull.doPull=function(a){a&&"string"==typeof a&&0!=a.length&&"#__fields:repeat__"!=a||(a="");var c=$("#global_data"),b=$("#_state").data("_logon");if(b&&b.length){var e=c.data("_pull_json"+a);e._window=window.name;e._logon=b;2<Object.size(e)&&$.getJSON("ajax/sys_config/sys_pull.php",e).done(function(b,d,e){var h=c.data("_pull_update"+a);$.each(b,function(a,b){"object"==typeof h[a]&&$.each(h[a],function(a,c){var d=$(a);switch(c){case "checkbox":d.prop("checked","on"==b);break;case "text":"color"==
...
ga.repeat.repeat=function(a,c,b,e){ga.repeat.data[a]=ga.repeat.data[a]||{};ga.repeat.data[a].repeat=ga.repeat.data[a].repeat||{};ga.repeat.data[a].repeat[c]={};ga.repeat.data[a].repeat[c].html=b;ga.repeat.data[a].repeat[c].eval=e;ga.repeat.map[c]=c;ga.repeat.data[a].repeat[c].htmlr=b.replace(/<\/label>/,"%%label%%</label>").replace(RegExp('id="'+c+'"'),'id="%%id%%"').replace(RegExp('name="'+c),'name="%%id%%').replace(RegExp('for="'+c+'"'),'for="%%id%%"').replace(RegExp('id="'+c+'_msg"'),'id="%%id%%_msg"').replace(RegExp('id="'+
c+'_tr"'),'id="%%id%%_tr"').replace(RegExp('id="'+c+'_button"'),'id="%%id%%_button"').replace(RegExp('="'+c+'_altval"',"g"),'="%%id%%_altval"').replace(RegExp('name="_selaltval_'+c+'"'),'name="_selaltval_%%id%%"').replace(RegExp('id="'+c+'-repeater"'),'id="%%id%%-repeater"');ga.repeat.data[a].repeat[c].htmls=b.replace(/<td><label.*?>(.*?)<\/label>\s*<\/td>/,"%%td%%$1%%etd%%").replace(/(<td[^>]*>).*?<\/td>/g,"$1</td>").replace(/<input[^>]*>/g,"").replace(/<span[^>]*>.*?<\/span>/g,"").replace(/\s*id=".*?"\s*/g,
"").replace("%%td%%","<td>").replace("%%etd%%","</td>").replace("<td></td>","");ga.repeat.data[a].repeat[c].evalr=e.replace(RegExp('"#'+c+'"',"g"),'"#%%id%%"').replace(RegExp('"#'+c+" option","g"),'"#%%id%% option').replace(RegExp(":"+c+":","g"),":%%id%%:").replace(RegExp('"#'+c+'_msg"',"g"),'"#%%id%%_msg"').replace(RegExp('"'+c+'"',"g"),'"%%id%%"').replace(RegExp('"#'+c+'_button"',"g"),'"#%%id%%_button"').replace(RegExp('"'+c+'_altval"',"g"),'"%%id%%_altval"').replace(RegExp('"#'+c+'_altval"',"g"),
'"#%%id%%_altval"');return'<tr><td></td><td><span id="'+c+'-span"></span></td></tr>'};
ga.repeat.repeatOn=function(a,c,b){var e=/^(.*):(.*)$/.exec(b);b=b.replace(":","-");ga.repeat.data[a].repeater=ga.repeat.data[a].repeater||{};ga.repeat.data[a].repeater[b]=ga.repeat.data[a].repeater[b]||{};ga.repeat.data[a].repeater[b].child=ga.repeat.data[a].repeater[b].child||[];ga.repeat.data[a].repeater[b].child.push(c);ga.repeat.data[a].repeat[c].refid=b;e&&(b=e[1],e=e[2],ga.repeat.data[a].repeater[b]=ga.repeat.data[a].repeater[b]||{},ga.repeat.data[a].repeater[b].child=ga.repeat.data[a].repeater[b].child||
[],ga.repeat.data[a].repeater[b].choice=ga.repeat.data[a].repeater[b].choice||[],ga.repeat.data[a].repeater[b].child.push(c),ga.repeat.data[a].repeater[b].choice.push(e))};
ga.repeat.repeater=function(a,c,b,e){ga.repeat.data[a]=ga.repeat.data[a]||{};ga.repeat.data[a].repeater=ga.repeat.data[a].repeater||{};ga.repeat.data[a].repeater[c]=ga.repeat.data[a].repeater[c]||{};ga.repeat.data[a].repeater[c].type=b;e&&"__fields:tableize__"!=e&&!/^(off|false)$/i.test(e)&&(ga.repeat.data[a].repeater[c].tableize=1)};
ga.repeat.children=function(a,c,b){var e;b=b||{};if(!(ga.repeat.data[a]&&ga.repeat.data[a].repeater&&ga.repeat.data[a].repeater[c]&&ga.repeat.data[a].repeater[c].child))return b;for(e=0;e<ga.repeat.data[a].repeater[c].child.length;++e)b[ga.repeat.data[a].repeater[c].child[e]]=!0,ga.repeat.data[a].repeater[e]&&(b=ga.repeat.children(a,e,b));return b};
...
ga.repeat.children(a,c);switch(ga.repeat.data[a].repeater[c].type){case "checkbox":if(e)for(k in g)l=c+"-"+k,ga.repeat.map[k]=l,h+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,l).replace("%%label%%",""),m+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,l),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[l]||(ga.repeat.data[a].repeater[l]=jQuery.extend({},ga.repeat.data[a].repeater[k])),f.push(l),ga.repeat.data[a].repeater[l].value&&delete ga.repeat.data[a].repeater[l].value);
break;case "integer":if(ga.repeat.data[a].repeater[c].tableize&&0<e)for(k in g)h+=ga.repeat.data[a].repeat[k].htmls;for(r=1;r<=e;++r)for(k in g)l=c+"-"+k+"-"+(r-1),ga.repeat.map[k]=l,h+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,l).replace("%%label%%","["+r+"]").replace(ga.repeat.data[a].repeater[c].tableize?/<td.*?><label.*?>.*?<\/label><\/td>/:"",""),m+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,l),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[l]||(ga.repeat.data[a].repeater[l]=
jQuery.extend({},ga.repeat.data[a].repeater[k])),f.push(l),ga.repeat.data[a].repeater[l].value&&delete ga.repeat.data[a].repeater[l].value);break;case "listbox":g=c.replace(/-[0-9]+$/,"").replace(/^(.*)-([A-ZA-z0-9_]*)$/,"$2")+"-"+e;r=c+"-"+e;g=ga.repeat.children(a,g);for(k in g)l=r+"-"+k,ga.repeat.map[k]=l,h+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,l).replace("%%label%%",""),m+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,l),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[l]||
(ga.repeat.data[a].repeater[l]=jQuery.extend({},ga.repeat.data[a].repeater[k])),f.push(l),ga.repeat.data[a].repeater[l].value&&delete ga.repeat.data[a].repeater[l].value);break;default:return console.warn("ga.repeat.change( "+a+" , "+c+" ) type "+ga.repeat.data[a].repeater[c].type+" not supported"),!1}!/^<tr>/.test(h)&&/<\/tr>$/.test(h)&&(h="<tr>"+h);$(d+"-repeater").html(h);eval(m);ga.repeat.data[a].repeater[c].value=e;for(k=0;k<f.length;++k)ga.repeat.change(a,f[k],b);$("#global_data").data("_pull_json#"+
c)&&ga.pull.doPull("#"+c);ga.hhelp.reset()};ga.repeat.map.convert=function(a){var c,b=[];for(c=0;c<a.length;++c)b[c]=ga.repeat.map[a[c]]||a[c];return b};ga.hide=function(a,c){ga.hide.data=ga.hide.data||{};ga.hide.data[a]=ga.hide.data[a]||{};ga.hide.data[a][c]=ga.hide.data[a][c]||{};ga.hide.data[a][c].active=1};ga.hide.data={};
ga.hide.update=function(a,c){var b;if(ga.hide.data[a]&&ga.hide.data[a][c])if(ga.hide.data[a][c].hides){if($(c).prop("checked"))for(b in ga.hide.data[a][c].hides)$(b+"-itd").html(" "),$(b).hide();else for(b in ga.hide.data[a][c].hides)$(b+"-itd").html(ga.hide.data[a][c].hides[b]),$(b).show();ga.hhelp.set()}else console.log("ga.hide.update( "+a+" , "+c+" ) error, no hides attached to this hider");else console.log("ga.hide.update( "+a+" , "+c+" ) error, hider has not been defined")};
ga.hide.add=function(a,c,b){ga.hide.data=ga.hide.data||{};ga.hide.data[a]=ga.hide.data[a]||{};ga.hide.data[a][b]=ga.hide.data[a][b]||{};ga.hide.data[a][b].hides=ga.hide.data[a][b].hides||{};ga.hide.data[a][b].hides[c]=$(c+"-itd").html()};ga.directives=function(a,c){ga.directives.data=ga.directives.data||{};ga.directives.data[a]=c};ga.hideifnot=function(a,c){ga.directives.data&&ga.directives.data[c]&&!/^(off|false|0$)/.test(ga.directives.data[c].toLowerCase())||$(a).hide()};ga.data={};
ga.data.nofcrefresh={};function showTooltip(a,c,b,e){$('<div id="rtooltip">'+b+"</div>").css({position:"absolute",top:c+5,left:a+5,border:"1px solid #181616",padding:"2px","background-color":e,color:"white"}).appendTo("body")}ga.data.dataURLtoFile=function(a,c){for(var b=a.split(","),e=b[0].match(/:(.*?);/)[1],b=atob(b[1]),f=b.length,d=new Uint8Array(f);f--;)d[f]=b.charCodeAt(f);return new File([d],c,{type:e})};
ga.data.create_image_htmltocanvas=function(a){if($("#"+a+"_savetofile").length){var c=$("#"+a+"_div");html2canvas(c.get(0),{background:"#ffffff",onrendered:function(b){b='<a href="'+b.toDataURL("image/png").replace("image/png","image/octet-stream")+'" target="_blank" download="plot.png">plot.png</a>';$("#"+a+"_savetofile_link").html(b)}})}};
ga.data.create_image=function(a,c){if($("#"+a+"_savetofile").length){var b=document.getElementById(a+"_savetofile"),e=c.getCanvas().toDataURL("image/png").replace("image/png","image/octet-stream"),e=ga.data.dataURLtoFile(e,"plot.png");b.href=URL.createObjectURL(e);$("#"+a+"_savetofile").removeClass("hidden")}};
ga.data.update=function(a,c,b,e){var f=0,d=0,g=0,h=0,m=0,k=a+"_output",r="#"+k,l=$(r),u={},v=r+"_msgs",y=$(v),p,w,z,B,x,C,A;b&&($("#"+a+"_progress").empty(),y.text(""));$.each(c,function(q,n){A=l.find("#"+q);if(A.length)switch(f||(y.text(""),f=1),A.attr("type")){case "plot3d":case "plotly":p="#"+q;n.layout=$.extend({},n.layout,{showlegend:!1});ga.plot3dglobal=n.layout;ga.dataplot3dglobal=n.data;ga.plotted3d[a]=0;$(p+"_showcollapse").length&&$(p+"_showcollapse").removeClass("hidden");Plotly.plot(q,
n.data,n.layout);ga.showcollapse3d[a]&&(ga.plotted3d[a]=1,$(p+"_showcollapse").trigger("click"));z=k+":#"+q+":last_value";$("#global_data").data(z,n);break;case "plot2d":p="#"+q;ga.value.plot2d.zstack.reset(p);if(n.data){if(ga.value.set.plot2d(p,n.options),ga.plotglobal=n.options,ga.dataplotglobal=n.data,$.plot(p,n.data,ga.value.get.plot2d.plot_options(p,n.options)),ga.customtooltips[a]){var v=null;$(p).bind("plothover",function(a,b,c){c?v!=c.dataIndex&&(v=c.dataIndex,$("#rtooltip").remove(),c.datapoint[0].toFixed(2),
c.datapoint[1].toFixed(2),c.series.tooltips.length&&showTooltip(c.pageX,c.pageY,c.series.tooltips[c.dataIndex],c.series.color)):($("#rtooltip").remove(),v=null)})}}else $.plot(p,n,ga.value.get.plot2d.plot_options(p));$(p+"_savetofile").length&&$(p+"_savetofile").removeClass("hidden");$(p+"_changescalex").length&&($(p+"_changescalex").removeClass("hidden"),"log"==n.options.xscale?$(p+"_changescalex_message").html("X-log"):$(p+"_changescalex_message").html("X-lin"));$(p+"_changescaley").length&&($(p+
"_changescaley").removeClass("hidden"),"log"==n.options.yscale?$(p+"_changescaley_message").html("Y-log"):$(p+"_changescaley_message").html("Y-lin"));$(p+"_showcollapse").length&&($(p+"_showcollapse").removeClass("hidden"),$(p).show(),ga.plotted2d[a]=1,$(p+"_showcollapse").trigger("click"));if(ga.value.settings[p].selzoom||n.options&&n.options.selection&&n.options.selection.mode&&"xy"==n.options.selection.mode)$(p).on("plotselected",{htag:p,data:n.data?n.data:n,options:n.data?ga.value.get.plot2d.plot_options(p,
n.options):ga.value.get.plot2d.plot_options(p)},function(a,b){1E-5>b.xaxis.to-b.xaxis.from&&(b.xaxis.to=b.xaxis.from+1E-5);1E-5>b.yaxis.to-b.yaxis.from&&(b.yaxis.to=b.yaxis.from+1E-5);ga.value.plot2d.zstack.dopush(a.data.htag,b);$.plot(a.data.htag,a.data.data,$.extend(!0,{},a.data.options,{xaxis:{min:b.xaxis.from,max:b.xaxis.to},yaxis:{min:b.yaxis.from,max:b.yaxis.to}}))}).on("contextmenu",{htag:p,data:n.data?n.data:n,options:n.data?ga.value.get.plot2d.plot_options(p,n.options):ga.value.get.plot2d.plot_options(p)},
function(a){a.preventDefault();var b=ga.value.plot2d.zstack.dopop(a.data.htag);b?$.plot(a.data.htag,a.data.data,$.extend(!0,{},a.data.options,{xaxis:{min:b.xaxis.from,max:b.xaxis.to},yaxis:{min:b.yaxis.from,max:b.yaxis.to}})):$.plot(a.data.htag,a.data.data,a.data.options)});z=k+":#"+q+":last_value";$("#global_data").data(z,n);break;case "bokeh":ga.bokeh.render(a,q,n);break;case "ngl":ga.value.nglshow(k,q,n);break;case "atomicstructure":z=k+":#"+q+":last_value";C=n.file?n.file:n;_jmol_info[q].script=
"set background ["+ga.colors.background+"]; set zoomlarge false;set echo top center;echo loading "+C.split("/").pop()+';refresh;load "'+C+'";';ga.set(a+":jsmoladd")&&(_jmol_info[q].script+=ga.set(a+":jsmoladd"));n.script&&(_jmol_info[q].script+=";"+n.script);$("#global_data").data(z,_jmol_info[q].script);$("#"+q).html(Jmol.getAppletHtml("jmolApplet"+q,_jmol_info[q]));break;case "checkbox":case "radio":A.prop("checked",!0);break;case "div":A.html(n);break;case "video":w=$("#"+q);x="<video ";w.attr("data-width")&&
(x+=' width="'+w.attr("data-width")+'"');w.attr("data-height")&&(x+=' height="'+w.attr("data-height")+'"');x+=" controls>";x+='<source src="'+n+'.mp4" type="video/mp4" /><source src="'+n+'.webm" type="video/webm" />';x+="</video>";w.html(x);break;case "image":w=$("#"+q);x="<img ";w.attr("data-width")&&(x+=' width="'+w.attr("data-width")+'"');w.attr("data-height")&&(x+=' height="'+w.attr("data-height")+'"');x+=' src="'+n+'">';w.html(x);break;case "filelink":B='<a href="'+n+'" target="_blank">'+n.split("/").pop()+
"</a>";z=k+":#"+q+":last_value";$("#global_data").data(z,B);$("#"+q+"_filelink").html(B);break;case "filelinkm":z=k+":#"+q+":last_value";B="";$.each(n,function(a,b){B+='<a href="'+b+'" target="_blank">'+b.split("/").pop()+"</a> "});$("#global_data").data(z,B);$("#"+q+"_filelink").html(B);break;default:$("#global_data").data("_append:"+k+"_"+q)?(A.val(A.val()+"\n"+n),A.height(parseFloat(A.prop("scrollHeight")+parseFloat(A.css("borderTopWidth"))+parseFloat(A.css("borderBottomWidth"))))):A.val(n)}else b?
"_"==q.charAt(0)?/^_fs_/.test(q)&&ga.data.nofcrefresh[a]||("_message"==q&&ga.msg.box(n),"_question"==q&&ga.qr.question(a,c),"_question_answered"==q&&ga.qr.answered(a,c),"_question_timeout"==q&&ga.qr.timeout(a,c),/^_getinput/.test(q)&&"_getinput"==q&&ga.valuen.input(a,n),"_textarea"==q&&ga.data.textarea(r,n),"_airavata"==q&&ga.data.airavata(r,n),"_status"==q&&"complete"==n&&b(e,0,0)):(d||(y.append("<p>Unexpected results:</p>"),d=1),y.append("<p>"+q+" => "+n+"</p>")):"_"==q.charAt(0)?/^_fs_/.test(q)&&
ga.data.nofcrefresh[a]||($("#_state").data(q,n),g=1,"_status"==q&&(u.job_status=n),/^_getinput/.test(q)&&"_getinput"==q&&ga.valuen.input(a,n),"_textarea"==q&&ga.data.textarea(r,n),"_airavata"==q&&ga.data.airavata(r,n),"_loginverify"==q&&ga.login.verify(n),"_loginapprove"==q&&ga.login.approve(n)):"-close"==q?h=1:"-close2"==q?m=1:(d||(y.text(""),y.append("<p>Unexpected results:</p>"),f=d=1),y.append("<p>"+q+" => "+n+"</p>"))});ga.value.saveLastValues(k);ga.value.saveLastValue(k,v);$(r+"_progress").empty();
g&&syncState();h&&ga.msg.close(1);m&&ga.msg.close(2);return u};
ga.data.textarea=function(a,c){var b=a+"_textarea",e=$(b);isatend=e[0].scrollHeight-e[0].scrollTop===e[0].clientHeight;c||(c="");e.is(":hidden")&&(e.show(),$(b+"_label").show());"__reset__\n"==c.substr(0,10)?e.val(c.substr(10)):e.val(e.val()+c);ga.set("textarea:rows")?ga.data.textarea.h[a]?e.height(ga.data.textarea.h[a]):ga.data.textarea.h[a]=parseFloat(e.prop("clientHeight"))+parseFloat(e.css("borderTopWidth"))+parseFloat(e.css("borderBottomWidth")):e.height(parseFloat(e.prop("scrollHeight"))+parseFloat(e.css("borderTopWidth"))+
parseFloat(e.css("borderBottomWidth")));isatend&&e.scrollTop(e[0].scrollHeight)};ga.data.textarea.h={};ga.data.airavata=function(a,c){var b=$(a+"_airavata");b.is(":hidden")&&b.show();b.html(c)};
ga.sync=function(a,c,b){var e,f=$("#"+c),d;if(f&&ga.sync.data&&ga.sync.data[a]&&ga.sync.data[a][b]){ga.sync.data[a][b][c]=!0;for(e in ga.sync.data[a][b])if(e!=c&&(d=$("#"+e))&&$.isNumeric(d.val())){f.val(d.val());f.change();return}ga.sync.data[a][b]._lastval&&$.isNumeric(ga.sync.data[a][b]._lastval)&&(f.val(ga.sync.data[a][b]._lastval),f.change())}else ga.sync.data=ga.sync.data||{},ga.sync.data[a]=ga.sync.data[a]||{},ga.sync.data[a][b]=ga.sync.data[a][b]||{},ga.sync.data[a][b][c]=!0};
ga.sync.change=function(a,c,b){var e,f=$("#"+c);if(f&&$.isNumeric(f.val())&&ga.sync.data&&ga.sync.data[a]&&ga.sync.data[a][b])for(e in ga.sync.data[a][b]._lastval=f.val(),ga.sync.data[a][b])e!=c&&(a=$("#"+e))&&a.val()!=f.val()&&(a.val(f.val()),a.change())};ga.sync.reset=function(a){var c;if(ga.sync.data&&ga.sync.data[a])for(c in ga.sync.data[a])ga.sync.data[a][c]._lastval&&delete ga.sync.data[a][c]._lastval};ga.valuen={};ga.valuen.data={};ga.valuen.html={};ga.valuen.dflt={};ga.valuen.dflt.data={};
ga.valuen.dflt.html={};ga.valuen.lastload="";
ga.valuen.restore=function(a,c,b){var e="#"+a,f=$(e),d=f.find(":input").get(),g={},h,m;c=c||ga.valuen.data[a];b=b||ga.valuen.html[a];if(c){$(e+" .field_msg").empty();do h=!1,$.each(d,function(){var b,e,l=$(this),m,v;if(l.attr("data-repeater")&&!g[this.name]){g[this.name]=!0;if(this.name&&(c[this.name]||/checkbox|radio/i.test(this.type))){e=c[this.name];if(/checkbox|radio/i.test(this.type)){m=l.val();v=!1;if(e)for(b=0;b<e.length;b++)if(e[b]==m){v=!0;break}l.prop("checked",v)}else l.val(e[0]);h=!0;
...
ga.license=function(a){a=a.split(",");var c=[],b,e=[],f;a.length&&(b="<p>Submitting to this module requires "+(1<a.length?"approved licenses":"an approved license")+" for <em>"+a.join("</em> and <em>")+"</em></p>");for(f in a)if(e.push({id:a[f],label:a[f]+" Management",data:a[f],cb:function(a){return ga.altfile.button.simplecall("license",a)}}),ga.license.data[a[f]]&&ga.license.data[a[f]].status)switch(ga.license.data[a[f]].status){case "approved":break;case "denied":b+="<p>Your license request for <em>"+
a[f]+"</em> has been <strong>denied</strong>.</p>";c.push(a[f]);break;case "pending":b+="<p>Your license request for <em>"+a[f]+"</em> is pending approval.</p>";c.push(a[f]);break;default:console.warn("ga.license() "+a[f]+" unknown status "+ga.license.data[a[f]].status),c.push(a[f])}else c.push(a[f]);return c.length?(ga.msg.box({icon:"warning.png",text:b,buttons:e}),!1):!0};ga.license.data={};
ga.license.get=function(){ga.license.data={};ga.license.url&&$.getJSON(ga.license.url,{tagmode:"any",format:"json",_window:window.name,_logon:$("#_state").data("_logon")}).done(function(a,c,b){a.license&&(ga.license.data=a.license);a.restricted?ga.restricted.show(a.restricted):ga.restricted.hideall()}).fail(function(a,c,b){console.warn("could not get license data")})};
ga.captcha=function(a,c){var b="";$.ajax({url:ga.captcha.url,data:{_window:window.name}}).success(function(e){e=$.parseJSON(e);e.error?ga.msg.box({icon:"toast.png",text:e.error}):(b='<img src="data:image/png;base64,'+e.captcha+'"><form id="sys_captcha"><label for="sys_captcha_text">Verify </label><input id="sys_captcha_text" class="help_link" type="text" maxlength="6" size="6" required><span class="help">Enter the 6 character alphanumeric code shown in the image and then press submit</span><input id="sys_captcha_id" type="hidden" value="'+
e.id+'"></form>',ga.msg.box({text:b,eval:"ga.hhelp.reset();$('#sys_captcha').on('keyup keypress', function(e) { var code = e.keyCode || e.which;  if (code  == 13) { e.preventDefault(); return false; }});",buttons:[{id:"submit",label:"Submit",cb:ga.captcha.verify,data:{cb:a,data:c}}]}))}).error(function(a){ga.msg.box({icon:"toast.png",text:"Error contacting server"})})};
ga.captcha.verify=function(a){$.ajax({url:ga.captcha.url_verify,data:{_window:window.name,captcha:$("#sys_captcha_text").val(),id:$("#sys_captcha_id").val()}}).success(function(c){c=$.parseJSON(c);if(c.error)ga.msg.box({icon:"toast.png",text:c.error});else if(c.success)a.cb(a.data);else return ga.captcha(a.cb,a.data)}).error(function(a){ga.msg.box({icon:"toast.png",text:"Error contacting server"})})};
ga.register=function(a){a=a.split(",");var c=[],b,e=[],f;a.length&&(b="<p>Submitting to this module requires "+(1<a.length?"approved registers":"an approved register")+" for <em>"+a.join("</em> and <em>")+"</em></p>");for(f in a)if(e.push({id:a[f],label:a[f]+" Management",data:a[f],cb:function(a){return ga.altfile.button.simplecall("register",a)}}),ga.register.data[a[f]]&&ga.register.data[a[f]].status)switch(ga.register.data[a[f]].status){case "approved":break;case "denied":b+="<p>Your register request for <em>"+
a[f]+"</em> has been <strong>denied</strong>.</p>";c.push(a[f]);break;case "pending":b+="<p>Your register request for <em>"+a[f]+"</em> is pending approval.</p>";c.push(a[f]);break;default:console.warn("ga.register() "+a[f]+" unknown status "+ga.register.data[a[f]].status),c.push(a[f])}else c.push(a[f]);return c.length?(ga.msg.box({icon:"warning.png",text:b,buttons:e}),!1):!0};ga.register.data={};
ga.register.get=function(){ga.register.data={};ga.register.url&&$.getJSON(ga.register.url,{tagmode:"any",format:"json",_window:window.name,_logon:$("#_state").data("_logon")}).done(function(a,c,b){a.register&&(ga.register.data=a.register);a.restricted?ga.restricted.show(a.restricted):ga.restricted.hideall()}).fail(function(a,c,b){console.warn("could not get register data")})};ga.calc={};ga.calc.data={};ga.calc.str_atom_numeric="(?:(?:-?[1-9][0-9]*)|(?:-?0))?(?:[.][0-9]+)?(?:[eE][-+]?[0-9]+)?";
...
ga.calc.register=function(a,c,b){ga.calc.data[a]=ga.calc.data[a]||{};ga.calc.data[a].calc=ga.calc.data[a].calc||{};ga.calc.data[a].calc[c]={};ga.calc.data[a].calc[c].calc=b;ga.calc.data[a].calc[c].tokens=ga.calc.tokens(b);ga.calc.data[a].calc[c].tokens._error?ga.msg.box({icon:"toast.png",text:"Module field calc internal error: "+ga.calc.data[a].calc[c].tokens._error+" in calc field id "+c}):(ga.calc.repeatertokens(a,c),ga.calc.data[a].calc[c].dependents=ga.calc.dependents(a,c),99<ga.calc.depthofdeps(a,
c)?ga.msg.box({icon:"toast.png",text:"Module field calc internal error: maximum recursion depth found in calc field id "+c}):(ga.calc.data[a].calc[c].tree=ga.calc.mktree(ga.calc.data[a].calc[c].tokens),ga.calc.data[a].calc[c].tree._error?ga.msg.box({icon:"toast.png",text:"Module field calc internal error: "+ga.calc.data[a].calc[c].tree._error+" in calc field id "+c}):ga.calc.install(a,c)))};
ga.calc.repeatertokens=function(a,c){var b,e;if(/-/.test(c))for(i in b=c.split("-"),/\d+/.test(b[b.length-1])?(e=b.slice(0,b.length-2).join(":")+":",b=":"+b[b.length-1]):(e=b.slice(0,b.length-1).join(":")+":",b=""),ga.calc.data[a].calc[c].tokens)ga.calc.is_atom_id.test(ga.calc.data[a].calc[c].tokens[i])&&!$("#"+ga.calc.data[a].calc[c].tokens[i]).length&&(ga.calc.data[a].calc[c].tokens[i]=e+ga.calc.data[a].calc[c].tokens[i]+b)};
ga.calc.depthofdeps=function(a,c,b){var e,f,d=0;b=b||0;ga.calc.data[a].calc[c].dependents&&b++;if(99<b)return b;for(e=0;e<ga.calc.data[a].calc[c].dependents.length;++e)ga.calc.data[a].calc[ga.calc.data[a].calc[c].dependents[e]]&&(f=ga.calc.depthofdeps(a,ga.calc.data[a].calc[c].dependents[e],b),d<f&&(d=f));return b+d};
ga.calc.dependents=function(a,c){var b,e=[];for(b in ga.calc.data[a].calc[c].tokens)ga.calc.is_atom_id.test(ga.calc.data[a].calc[c].tokens[b])&&e.push(ga.calc.data[a].calc[c].tokens[b]);return e};ga.calc.install=function(a,c){for(var b in ga.calc.data[a].calc[c].dependents)$("#"+ga.calc.data[a].calc[c].dependents[b].replace(/:/g,"-")).on("change",function(){ga.calc.process(a,c)})};
ga.calc.process=function(a,c){var b=ga.calc.evaltree(jQuery.extend(!0,{},ga.calc.data[a].calc[c].tree));b._error?ga.msg.box({icon:"toast.png",text:"Module field calc internal error: "+ga.calc.data[a].calc[c].tokens._error+" in calc field id "+c}):$("#"+c).val(b).trigger("change")};
...
break;case "-":if(!b)return{_error:"operator : "+a.op+" is binary and only has one argument"};if(c)if(d){c=a.args[0];for(b=0;b<g.length;++b)c[b]-=a.args[1][b];if(m)for(b=a.args[0].length;b<a.args[1].length;++b)c.push(-a.args[1][b]);else for(b=a.args[1].length;b<a.args[0].length;++b)c.push(a.args[0][b])}else if(e)for(c=a.args[0],b=0;b<a.args[0].length;++b)c[b]-=a.args[1];else for(c=[],b=0;b<a.args[1].length;++b)c.push(a.args[0]-a.args[1][b]);else c=a.args[0]-a.args[1];break;case "/":if(!b)return{_error:"operator : "+
a.op+" is binary and only has one argument"};if(c)if(d){c=a.args[0];for(b=0;b<g.length;++b)c[b]/=a.args[1][b];if(m)for(b=a.args[0].length;b<a.args[1].length;++b)c.push(0);else for(b=a.args[1].length;b<a.args[0].length;++b)c.push(a.args[0][b]/0)}else if(e)for(c=a.args[0],b=0;b<a.args[0].length;++b)c[b]/=a.args[1];else for(c=[],b=0;b<a.args[1].length;++b)c.push(a.args[0]/a.args[1][b]);else c=a.args[0]/a.args[1];break;case "^":if(!b)return{_error:"operator : "+a.op+" is binary and only has one argument"};
if(c)if(d){c=a.args[0];for(b=0;b<g.length;++b)c[b]=Math.pow(c[b],a.args[1][b]);if(m)for(b=a.args[0].length;b<a.args[1].length;++b)c.push(0);else for(b=a.args[1].length;b<a.args[0].length;++b)c.push(1)}else if(e)for(c=a.args[0],b=0;b<a.args[0].length;++b)c[b]=pow(c[b],a.args[1]);else for(c=[],b=0;b<a.args[1].length;++b)c.push(pow(a.args[0],a.args[1][b]));else c=Math.pow(a.args[0],a.args[1]);break;case "abs(":if(b)return{_error:"operator : "+a.op+" has 2 arguments but only accepts one"};c=a.args[0];
if(e)for(b=0;b<c.length;++b)c[b]=Math.abs(c[b]);else c=Math.abs(a.args[0]);break;case "acos(":if(b)return{_error:"operator : "+a.op+" has 2 arguments but only accepts one"};c=a.args[0];if(e)for(b=0;b<c.length;++b)c[b]=Math.acos(c[b]);else c=Math.acos(a.args[0]);break;case "asin(":if(b)return{_error:"operator : "+a.op+" has 2 arguments but only accepts one"};c=a.args[0];if(e)for(b=0;b<c.length;++b)c[b]=Math.asin(c[b]);else c=Math.asin(a.args[0]);break;case "atan(":if(b)return{_error:"operator : "+
a.op+" has 2 arguments but only accepts one"};c=a.args[0];if(e)for(b=0;b<c.length;++b)c[b]=Math.atan(c[b]);else c=Math.atan(a.args[0]);break;case "ceil(":if(b)return{_error:"operator : "+a.op+" has 2 arguments but only accepts one"};c=a.args[0];if(e)for(b=0;b<c.length;++b)c[b]=Math.ceil(c[b]);else c=Math.ceil(a.args[0]);break;case "cos(":if(b)return{_error:"operator : "+a.op+" has 2 arguments but only accepts one"};c=a.args[0];if(e)for(b=0;b<c.length;++b)c[b]=Math.cos(c[b]);else c=Math.cos(a.args[0]);
break;case "exp(":if(b)return{_error:"operator : "+a.op+" has 2 arguments but only accepts one"};c=a.args[0];if(e)for(b=0;b<c.length;++b)c[b]=Math.exp(c[b]);else c=Math.exp(a.args[0]);break;case "floor(":if(b)return{_error:"operator : "+a.op+" has 2 arguments but only accepts one"};c=a.args[0];if(e)for(b=0;b<c.length;++b)c[b]=Math.floor(c[b]);else c=Math.floor(a.args[0]);break;case "log(":if(b)return{_error:"operator : "+a.op+" has 2 arguments but only accepts one"};c=a.args[0];if(e)for(b=0;b<c.length;++b)c[b]=
Math.log(c[b]);else c=Math.log(a.args[0]);break;case "random(":if(b)return{_error:"operator : "+a.op+" has 2 arguments but only accepts one"};c=a.args[0];if(e)for(b=0;b<c.length;++b)c[b]=Math.random();else c=Math.random();break;case "round(":if(b)return{_error:"operator : "+a.op+" has 2 arguments but only accepts one"};c=a.args[0];if(e)for(b=0;b<c.length;++b)c[b]=Math.round(c[b]);else c=Math.round(a.args[0]);break;case "sin(":if(b)return{_error:"operator : "+a.op+" has 2 arguments but only accepts one"};
c=a.args[0];if(e)for(b=0;b<c.length;++b)c[b]=Math.sin(c[b]);else c=Math.sin(a.args[0]);break;case "sqrt(":if(b)return{_error:"operator : "+a.op+" has 2 arguments but only accepts one"};c=a.args[0];if(e)for(b=0;b<c.length;++b)c[b]=Math.sqrt(c[b]);else c=Math.sqrt(a.args[0]);break;case "tan(":if(b)return{_error:"operator : "+a.op+" has 2 arguments but only accepts one"};c=a.args[0];if(e)for(b=0;b<c.length;++b)c[b]=Math.tan(c[b]);else c=Math.tan(a.args[0]);break;case "max(":if(b)return{_error:"operator : "+
a.op+" has 2 arguments but only accepts one"};c=e?Math.max.apply(null,a.args[0]):a.args[0];break;case "min(":if(b)return{_error:"operator : "+a.op+" has 2 arguments but only accepts one"};c=e?Math.min.apply(null,a.args[0]):a.args[0];break;case "atan2(":if(b)return{_error:"operator : "+a.op+" has 2 arguments but only accepts one"};if(!e)return{_error:"operator : "+a.op+"  needs the first argument to be an even sized array"};c=[];for(b=0;b<a.args[0].length;b+=2)c.push(Math.atan2(a.args[0][b],a.args[0][b+
1]));break;case "pow(":if(b)return{_error:"operator : "+a.op+" has 2 arguments but only accepts one"};if(!e)return{_error:"operator : "+a.op+"  needs the first argument to be an even sized array"};c=[];for(b=0;b<a.args[0].length;b+=2)c.push(Math.pow(a.args[0][b],a.args[0][b+1]));1==c.length&&(c=c[0]);break;default:return{_error:"operator : "+a.op+" unknown or unsupported"}}return c};ga.bokeh={};ga.bokeh.data={};
ga.bokeh.getdata=function(a,c){var b={},e=c.split(/\r?\n/),f=e.length,d,g=0;for(d=0;d<f;++d)if(g||-1!==e[d].indexOf("var docs_json ="))if(!g)g=1,b.docs_json=$.parseJSON(e[d].replace(/^\s*var\s+docs_json\s+=\s+/,"").replace(/(^'|('|);$)/g,""));else if(1==g&&-1!==e[d].indexOf("var render_items =")){b.render_items=$.parseJSON(e[d].replace(/^\s*var\s+render_items\s+=\s+/,"").replace(/;$/,""));break}return b};
ga.bokeh.render=function(a,c,b){b=ga.bokeh.getdata(c,b);a+="_output";ga.bokeh.savedata(a,c,b);ga.bokeh.renderdata(a,c)};
ga.bokeh.renderdata=function(a,c){var b,e,f="";if(ga.bokeh.data[a]&&ga.bokeh.data[a][c]&&ga.bokeh.data[a][c].docs_json&&ga.bokeh.data[a][c].render_items){e=ga.bokeh.data[a][c].render_items.length;for(b=0;b<e;++b)f+='<div class="bk-root"><div class="bk-plotdiv" id="'+ga.bokeh.data[a][c].render_items[b].elementid+'"></div></div>';$("#"+c).html(f);Bokeh.embed.embed_items(ga.bokeh.data[a][c].docs_json,ga.bokeh.data[a][c].render_items)}};
ga.bokeh.savedata=function(a,c,b){ga.bokeh.data[a]=ga.bokeh.data[a]||{};ga.bokeh.data[a][c]=b};ga.bokeh.reset=function(a,c){ga.bokeh.data[a]&&ga.bokeh.data[a][c]&&(ga.bokeh.data[a][c]={},$("#"+c).empty())};ga.grid={};ga.grid.rc=function(a,c,b,e){var f="";a&&b&&a<b-1?f+="grid-row-start:"+a+";grid-row-end:"+b+";":a&&(f+="grid-row:"+a+";");c&&e&&c<e-1?f+="grid-column-start:"+c+";grid-column-end:"+e+";":c&&(f+="grid-column:"+c+";");return f};
ga.grid.rcs=function(a,c,b,e){return'style="'+ga.grid.rc(a,c,b,e)+'"'};ga.grid.drcs=function(a,c,b,e,f,d){f=(f||"")+ga.grid.rc(a,c,b,e);return d?'<div class="'+d+'" style="'+f+'">':'<div style="'+f+'">'};ga.grid.init=function(){return{row:0,col:1}};ga.grid.newrow=function(a){a.row++;a.col=1};
ga.grid.next=function(a,c,b,e){var f,d;a=a||{row:1,col:1};c?(a.col=c[0]?c[0]:a.col,f=c[1]?c[1]+a.col:a.col,a.row=c[2]?c[2]:a.row,d=c[3]?c[3]+a.row:a.row):(f=a.col,d=a.row);c=a.col;a.col=f+1;return ga.grid.drcs(a.row,c,d,f,b,e)};ga.grid.nextstyle=function(a,c,b){var e,f;b=b||"";a=a||{row:1,col:1};c?(a.col=c[0]?c[0]:a.col,e=c[1]?c[1]+a.col:a.col,a.row=c[2]?c[2]:a.row,f=c[3]?c[3]+a.row:a.row):(e=a.col,f=a.row);c=a.col;a.col=e+1;return ga.grid.rc(a.row,c,f,e,b)+b};ga.qr={};ga.qr.openq={};
ga.qr.question=function(a,c){var b="",e,f,d,g="",h="",m,k=[],r={},l,u;c._uuid||(g+="no _uuid in received data. ");c._msgid||(g+="no _msgid in received data. ");c._question||(g+="no _question in received data. ");c._question.fields||(g+="no _question:fields in received data. ");if(g.length)return ga.qr.rerror(c,g);c._question.title&&(b+="<h3>"+c._question.title+"</h3>");c._question.text&&(b+=c._question.text);m=c._uuid+"-"+c._msgid;if(c._question.grid){ga.grid.align=ga.grid.align||"left";d="display:grid;grid-gap:";
l=ga.grid.init();if("object"===typeof c._question.grid){c._question.grid.colwidths?"object"!==typeof c._question.grid.colwidths&&(g+=" _question:grid:colwidths specified, but it is not an array."):g+=" _question:grid specified, but it is not a number and colwidths not defined.";if(g.length)return ga.qr.rerror(c,g);d=c._question.grid.gap?d+c._question.grid.gap:d+"10px";d+=";grid-template-columns:";for(e=0;e<c._question.grid.colwidths.length;++e)d+=c._question.grid.colwidths[e]+"fr "}else for(d+="10px;grid-template-columns:auto ",
2>c._question.grid&&(c._question.grid=2),e=1;e<c._question.grid;++e)d+=" auto";b+='<form id="'+m+'" style="'+(d+";")+'">';for(e=row_start=0;e<c._question.fields.length;++e)if(d=c._question.fields[e],d.id||(g+="No id in field "+e+". "),/^[A-Za-z][A-Za-z0-9_]*$/.test(d.id)||(g+="Invalid id for field "+e+' with id value "'+d.id+'". Only alpha in first character and alphanumeric and underscores subsequently allowed.'),d.type||(g+="No type in field "+e+". "),d.id&&d.type)switch(u="text-align:"+(d.grid&&
...
ga.qr.rerror=function(a,c){var b={};b._uuid=a._uuid;b._msgid=parseFloat(a._msgid);b._response={};b._response.error=c;a._question&&a._question.id&&(b._response.id=a._question.id);ga.qr.post(b);ga.msg.box({icon:"toast.png",text:c})};
...
a.buttons[i],function(a){a.data.cb(a.data.adata[0],a.data.adata[1],a.data.adata[2])&&ga.msg.close(b)});else $("#_mbb_"+a.buttons[i].id).off().one("click",a.buttons[i],function(a){a.data.cb(a.data.adata[0],a.data.adata[1],a.data.adata[2]);ga.msg.close(b)});break;case 4:$("#_mbb_"+a.buttons[i].id).off().one("click",a.buttons[i],function(a){a.data.cb(a.data.adata[0],a.data.adata[1],a.data.adata[2],a.data.adata[3]);ga.msg.close(b)});break;case 5:$("#_mbb_"+a.buttons[i].id).off().one("click",a.buttons[i],
...
function(a){a.data.cb(a.data.data);ga.msg.close(b)});else $("#_mbb_"+a.buttons[i].id).off().one("click",a.buttons[i],function(a){ga.msg.close(b)})}a.ptext&&$("#configbody"+b).append(a.ptext);a.eval&&eval(a.eval);ga.repeats.save();ga.hhelp.reset();$(".modalDialog"+b).addClass("modalDialog"+b+"_on")}};ga.msg.close1=function(){ga.repeats.restore();$(".modalDialog").removeClass("modalDialog_on");setTimeout(function(){$("#configbody").empty()},400);ga.usesplash&&setTimeout(function(){splashlogin()},500)};
ga.msg.close2=function(){ga.repeats.restore();$(".modalDialog2").removeClass("modalDialog2_on");$("#configbody2").empty();ga.usesplash&&setTimeout(function(){splashlogin()},500)};ga.msg.close3=function(){ga.repeats.restore();$(".modalDialog3").removeClass("modalDialog3_on");$("#configbody3").empty();var a=$("#_state").data("__msgs");if(a&&a.length){var c=a.shift();$("#_state").data("__msgs",a);ga.msg.box(c,1)}ga.usesplash&&setTimeout(function(){splashlogin()},500)};
...
ga.ws.unsub=function(a,c){if(c in ga.ws.subd)if(delete ga.ws.subd[c],$("#_state").data("_ws")){var b=$("#_state").data("_wssession");b?(b.unsubscribe(a),$("#_state").data("_wssub:"+a,null)):console.log("ga.ws.sub: connection is not open")}else console.log("ga.ws.sub: ws not defined")};ga.ws.generic=function(a,c){console.log("ga.ws.generic "+a+" : "+c.json)};
