...
c})})};ga.admin.ajax.remove=function(a,b,c,d){messagebox({icon:"admin.png",text:"Are you sure you want to permanently remove this user, all of their job history and their stored data?",buttons:[{id:"yes",label:"Yes, remove this user",cb:ga.admin.ajax,adata:[a,b,c,d]},{id:"cancel",label:"Cancel"}]})};
...
g+'">'+e.resources[g][m]+"</label></td></tr>";messagebox({icon:"question.png",text:f+"</table>",buttons:h});return"deferred";default:return messagebox({icon:"toast.png",text:"ga.airavata.select, unknown selection type '"+a+"'"}),"abort"}};ga.resource={};ga.xsede={};ga.resource.xsedeprojecttokeys=function(){var a;ga.resource.xsedepkeys={};for(a in ga.resource.xsedeproject)ga.resource.xsedepkeys[ga.resource.xsedeproject[a]]=1};
ga.xsede.select=function(a,b,c){var d=ga.xsede.data;a="__resource__"==a?ga.resource.defaultval:a;var e="",f=[],h;delete ga.xsede.useproject;if(!(ga.resource.xsedeproject&&a in ga.resource.xsedepkeys))return b(c);if(!d||!d.length)return messagebox({icon:"warning.png",text:"No XSEDE projects currently defined.  Create one under the user configuration button at the top right."}),"abort";if(1==d.length)return ga.xsede.useproject=d[0],b(c);f.push({id:"submit_xsedeproject",label:"Submit",data:[b,c,d],cb:function(a){ga.xsede.useproject=
...
q.push(n[2]),q.push(n[0]),q.push(n[3])):g>8+e+v+z&&g<9+e+v+z+B?(n=a[g].trim().split(/\s+/),y.push(n[1]),y.push(n[2]),y.push(n[0]),y.push(n[3])):h.push(a[g]);u.push(k);u.push(r);u.push(q);u.push(y);for(g=f=0;g<c.length;g++)if($("#"+c[g]).length)switch($("#"+c[g]).data("repeater")&&(m.push(u[f]),f++),$("#"+c[g]).attr("type")){case "text":k=new RegExp($("#"+c[g]).attr("pattern"));if(!k.test(h[g])){messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+x+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",
buttons:[{id:"ok",label:"OK"}]});return}break;case "number":k=[h[g]];if(ga.value.checkFloatIntOK("#"+c[g],k))h[g]=k[0];else{messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+x+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;default:messagebox({icon:"warning.png",text:"Selected input type is currently not supported. Contact the developer",buttons:[{id:"ok",
label:"OK"}]});return}else f++;break;case "whitespaceseparated":case "whitespaceseparated_reverselogic":h=a.trim().split(/\s+/);if(h.length==c.length)for(g=0;g<h.length;g++){for(u=0;u<f.length;u++)if(g+1<=f[u]){x=u+1;break}switch($("#"+c[g]).attr("type")){case "checkbox":k=/^(0|1|n|y|true|false|t|f|yes|no)$/;if(!k.test(h[g].toLowerCase())){messagebox({icon:"warning.png",text:"Wrong format of the input file! Checkbox input value on the line #"+x+" is not valid. Options are: [1 | 0 | yes | no | true | false | t | f | T | F | y | n | Y | N ]. Check your input file",
...
bid+". "),m[bid]=1,g=[{id:"ok",label:"OK",cb:ga.qr.cb,adata:[b,"ok"]},{id:"cancel",label:"Cancel",cb:ga.qr.cb,adata:[b,"cancel"]}];if(f.length)return ga.qr.rerror(b,f);console.log("qtext:"+c);ga.qr.openq[h]="open";messagebox({icon:"question.png",text:c,eval:'$("#'+h+'").on("keyup keypress", function(e) { var code = e.keyCode || e.which;  if (code  == 13) { e.preventDefault(); return false; }});',buttons:g})};
ga.qr.cb=function(a,b){console.log("ga.qr.cb( data )");console.dir(a);console.dir(data);var c=a._uuid+"-"+a._msgid;if(ga.qr.openq[c]){switch(ga.qr.openq[c]){case "open":break;case "answered":messagebox({icon:"information.png",text:"Question has already been answered in another session"});delete ga.qr.openq[c];return;case "timeout":messagebox({icon:"information.png",text:"The time for answering a question has expired"});delete ga.qr.openq[c];return;default:messagebox({icon:"toast.png",text:"Internal error, unknown message state"});
...
ga.qr.rerror=function(a,b){console.log("ga.qr.rerror( q, "+b+" )");console.dir(a);console.dir(data);var c={};c._uuid=a._uuid;c._msgid=parseFloat(a._msgid);c._response={};c._response.error=b;a._question&&a._question.id&&(c._response.id=a._question.id);ga.qr.post(c);messagebox({icon:"toast.png",text:b})};
ga.qr.post=function(a){console.log("ga.qr.post( r )");console.dir(a);$.ajax({url:ga.qr.url,data:{_window:window.name,_data:a},dataType:"json",method:"POST"}).success(function(a){console.log("ajax delete done");a.error&&a.error.length&&messagebox({icon:"toast.png",text:"ajax data error: "+a.error,buttons:[{id:"ok",label:"OK"}]})}).error(function(a){console.log("ajax error");console.dir(a);messagebox({icon:"toast.png",text:"ajax error: "+a.statusText,buttons:[{id:"ok",label:"OK"}]})})};
