...
g+'">'+e.resources[g][p]+"</label></td></tr>";messagebox({icon:"question.png",text:f+"</table>",buttons:h});return"deferred";default:return messagebox({icon:"toast.png",text:"ga.airavata.select, unknown selectoin type '"+a+"'"}),"abort"}};ga.value={};ga.value.settings={};
...
ga.value.processInputfromFiles=function(a,b,c,d){var e=a.trim().split(/[\r\n]+/g),f=[],h=0;c=ga.repeat.map.convert(c);for(var g=0;g<e.length;g++){var p=e[g].trim().split(/\s+/),h=h+p.length;f.push(h)}var h=[],k=[],q=[],m=[],y=[],r=[],p=[],w=0;switch(b){case "whitespace_formulchcompost":a=[];for(g=0;g<e.length;g++)f=e[g].split("#")[0],f=f.replace(/\s{2,}/g," "),f=f.replace(/\t/g," "),f=f.toString().trim().replace(/(\r\n|\n|\r)/g,""),a.push(f);for(var e=parseInt(a[1]),n,g=0;g<a.length;g++)if(2<g&&g<
3+e)for(n=a[g].trim().split(/\s+/),f=0;f<n.length;f++)k.push(n[f]);else h.push(a[g]);p.push(k);for(g=0;g<c.length;g++)switch($("#"+c[g]).attr("type")){case "text":k=new RegExp($("#"+c[g]).attr("pattern"));if(!k.test(h[g])){messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+w+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;case "number":k=[h[g]];
...
[];for(g=0;g<e.length;g++)f=e[g].split("#")[0],f=f.replace(/\s{2,}/g," "),f=f.replace(/\t/g," "),f=f.toString().trim().replace(/(\r\n|\n|\r)/g,""),a.push(f);for(var e=parseInt(a[1]),u=parseInt(a[2+e]),x=parseInt(a[5+e+u]),A=parseInt(a[9+e+u+x]),g=0;g<a.length;g++)if(1<g&&g<2+e)for(n=a[g].trim().split(/\s+/),f=0;f<n.length;f++)k.push(n[f]);else g>2+e&&g<3+e+u?(n=a[g].trim().split(/\s+/),q.push(n[1]),q.push(n[2]),q.push(n[0]),q.push(n[3])):g>5+e+u&&g<6+e+u+x?(n=a[g].trim().split(/\s+/),m.push(n[1]),
m.push(n[2]),m.push(n[0]),m.push(n[3])):g>8+e+u+x&&g<9+e+u+x+A?(n=a[g].trim().split(/\s+/),y.push(n[1]),y.push(n[2]),y.push(n[0]),y.push(n[3])):h.push(a[g]);r.push(k);r.push(q);r.push(m);r.push(y);for(g=f=0;g<c.length;g++)if($("#"+c[g]).length)switch($("#"+c[g]).data("repeater")&&(p.push(r[f]),f++),$("#"+c[g]).attr("type")){case "text":k=new RegExp($("#"+c[g]).attr("pattern"));if(!k.test(h[g])){messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+w+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",
buttons:[{id:"ok",label:"OK"}]});return}break;case "number":k=[h[g]];if(ga.value.checkFloatIntOK("#"+c[g],k))h[g]=k[0];else{messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+w+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;default:messagebox({icon:"warning.png",text:"Selected input type is currently not supported. Contact the developer",buttons:[{id:"ok",
label:"OK"}]});return}else f++;break;case "whitespaceseparated":case "whitespaceseparated_reverselogic":h=a.trim().split(/\s+/);if(h.length==c.length)for(g=0;g<h.length;g++){for(r=0;r<f.length;r++)if(g+1<=f[r]){w=r+1;break}switch($("#"+c[g]).attr("type")){case "checkbox":k=/^(0|1|n|y|true|false|t|f|yes|no)$/;if(!k.test(h[g].toLowerCase())){messagebox({icon:"warning.png",text:"Wrong format of the input file! Checkbox input value on the line #"+w+" is not valid. Options are: [1 | 0 | yes | no | true | false | t | f | T | F | y | n | Y | N ]. Check your input file",
...
ga.repeat.change=function(a,b,c){var d,e=[],f="#"+b;d=$(f);var h,g="",p="",k,q,m;if(!(ga.repeat.data[a]&&ga.repeat.data[a].repeater&&ga.repeat.data[a].repeater[b]&&d.length))return!1;switch(ga.repeat.data[a].repeater[b].type){case "checkbox":d=d.prop("checked")?1:0;break;case "integer":case "listbox":d=d.val();break;default:return console.warn("ga.repeat.change( "+a+" , "+b+" ) type "+ga.repeat.data[a].repeater[b].type+" not supported"),!1}if(!c&&ga.repeat.data[a].repeater[b].value===d)return!1;h=
ga.repeat.children(a,b);switch(ga.repeat.data[a].repeater[b].type){case "checkbox":if(d)for(k in h)m=b+"-"+k,ga.repeat.map[k]=m,g+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,m).replace("%%label%%",""),p+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,m),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[m]||(ga.repeat.data[a].repeater[m]=jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(m),ga.repeat.data[a].repeater[m].value&&delete ga.repeat.data[a].repeater[m].value);
break;case "integer":if(ga.repeat.data[a].repeater[b].tableize&&0<d)for(k in h)g+=ga.repeat.data[a].repeat[k].htmls;for(q=1;q<=d;++q)for(k in h)m=b+"-"+k+"-"+(q-1),ga.repeat.map[k]=m,g+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,m).replace("%%label%%","["+q+"]").replace(ga.repeat.data[a].repeater[b].tableize?/<td.*?><label.*?>.*?<\/label><\/td>/:"",""),p+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,m),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[m]||(ga.repeat.data[a].repeater[m]=
jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(m),ga.repeat.data[a].repeater[m].value&&delete ga.repeat.data[a].repeater[m].value);break;case "listbox":h=b.replace(/-[0-9]+$/,"").replace(/^(.*)-([A-ZA-z0-9_]*)$/,"$2")+"-"+d;q=b+"-"+d;h=ga.repeat.children(a,h);for(k in h)m=q+"-"+k,ga.repeat.map[k]=m,g+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,m).replace("%%label%%",""),p+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,m),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[m]||
...
ga.data.create_image_htmltocanvas=function(a){if($("#"+a+"_savetofile").length){var b=document.getElementById(a+"_savetofile"),c=$("#"+a+"_div");html2canvas(c.get(0),{background:"#ffffff",onrendered:function(c){c=c.toDataURL("image/png").replace("image/png","image/octet-stream");b.href=c;b.download="plot.png";$("#"+a+"_savetofile").removeClass("hidden")}})}};
...
ga.data.update=function(a,b,c,d){var e=0,f=0,h=0,g=0,p=0,k=a+"_output",q="#"+k,m=$(q),y={},r=q+"_msgs",w=$(r),n,u,x,A,v,B,z;c&&($("#"+a+"_progress").html(""),w.text(""));$.each(b,function(b,l){z=m.find("#"+b);if(z.length)switch(e||(w.text(""),e=1),z.attr("type")){case "plot2d":n="#"+b;ga.value.plot2d.zstack.reset(n);l.data?(ga.value.set.plot2d(n,l.options),ga.plotglobal=l.options,ga.dataplotglobal=l.data,$.plot(n,l.data,ga.value.get.plot2d.plot_options(n,l.options))):$.plot(n,l,ga.value.get.plot2d.plot_options(n));
$(n+"_changescale").length&&$(n+"_changescale").removeClass("hidden");ga.data.create_image_htmltocanvas(b);if(ga.value.settings[n].selzoom||l.options&&l.options.selection&&l.options.selection.mode&&"xy"==l.options.selection.mode)$(n).on("plotselected",{htag:n,data:l.data?l.data:l,options:l.data?ga.value.get.plot2d.plot_options(n,l.options):ga.value.get.plot2d.plot_options(n)},function(a,b){1E-5>b.xaxis.to-b.xaxis.from&&(b.xaxis.to=b.xaxis.from+1E-5);1E-5>b.yaxis.to-b.yaxis.from&&(b.yaxis.to=b.yaxis.from+
1E-5);ga.value.plot2d.zstack.dopush(a.data.htag,b);$.plot(a.data.htag,a.data.data,$.extend(!0,{},a.data.options,{xaxis:{min:b.xaxis.from,max:b.xaxis.to},yaxis:{min:b.yaxis.from,max:b.yaxis.to}}))}).on("contextmenu",{htag:n,data:l.data?l.data:l,options:l.data?ga.value.get.plot2d.plot_options(n,l.options):ga.value.get.plot2d.plot_options(n)},function(a){a.preventDefault();var b=ga.value.plot2d.zstack.dopop(a.data.htag);b?$.plot(a.data.htag,a.data.data,$.extend(!0,{},a.data.options,{xaxis:{min:b.xaxis.from,
max:b.xaxis.to},yaxis:{min:b.yaxis.from,max:b.yaxis.to}})):$.plot(a.data.htag,a.data.data,a.data.options)});x=k+":#"+b+":last_value";$("#global_data").data(x,l);break;case "atomicstructure":x=k+":#"+b+":last_value";B=l.file?l.file:l;_jmol_info[b].script="set background ["+ga.colors.background+"]; set zoomlarge false;set echo top center;echo loading "+B.split("/").pop()+';refresh;load "'+B+'";';ga.set(a+":jsmoladd")&&(_jmol_info[b].script+=ga.set(a+":jsmoladd"));l.script&&(_jmol_info[b].script+=";"+
l.script);$("#global_data").data(x,_jmol_info[b].script);$("#"+b).html(Jmol.getAppletHtml("jmolApplet"+b,_jmol_info[b]));break;case "checkbox":case "radio":z.prop("checked",!0);break;case "div":z.html(l);break;case "video":u=$("#"+b);v="<video ";u.attr("data-width")&&(v+=' width="'+u.attr("data-width")+'"');u.attr("data-height")&&(v+=' height="'+u.attr("data-height")+'"');v+=" controls>";v+='<source src="'+l+'.mp4" type="video/mp4" /><source src="'+l+'.webm" type="video/webm" />';v+="</video>";u.html(v);
break;case "image":u=$("#"+b);v="<img ";u.attr("data-width")&&(v+=' width="'+u.attr("data-width")+'"');u.attr("data-height")&&(v+=' height="'+u.attr("data-height")+'"');v+=' src="'+l+'">';u.html(v);break;case "filelink":A='<a href="'+l+'" target="_blank">'+l.split("/").pop()+"</a>";x=k+":#"+b+":last_value";$("#global_data").data(x,A);$("#"+b+"_filelink").html(A);break;case "filelinkm":x=k+":#"+b+":last_value";A="";$.each(l,function(a,b){A+='<a href="'+b+'" target="_blank">'+b.split("/").pop()+"</a> "});
$("#global_data").data(x,A);$("#"+b+"_filelink").html(A);break;default:$("#global_data").data("_append:"+k+"_"+b)?(z.val(z.val()+"\n"+l),z.height(parseFloat(z.prop("scrollHeight")+parseFloat(z.css("borderTopWidth"))+parseFloat(z.css("borderBottomWidth"))))):z.val(l)}else c?"_"==b.charAt(0)?/^_fs_/.test(b)&&ga.data.nofcrefresh[a]||("_message"==b&&messagebox(l),/^_getinput/.test(b)&&"_getinput"==b&&ga.valuen.input(a,l),"_textarea"==b&&ga.data.textarea(q,l),"_airavata"==b&&ga.data.airavata(q,l),"_status"==
b&&"complete"==l&&c(d,0,0)):(f||(w.append("<p>Unexpected results:</p>"),f=1),w.append("<p>"+b+" => "+l+"</p>")):"_"==b.charAt(0)?/^_fs_/.test(b)&&ga.data.nofcrefresh[a]||($("#_state").data(b,l),h=1,"_status"==b&&(y.job_status=l),/^_getinput/.test(b)&&"_getinput"==b&&ga.valuen.input(a,l),"_textarea"==b&&ga.data.textarea(q,l),"_airavata"==b&&ga.data.airavata(q,l),"_loginverify"==b&&ga.login.verify(l),"_loginapprove"==b&&ga.login.approve(l)):"-close"==b?g=1:"-close2"==b?p=1:(f||(w.text(""),w.append("<p>Unexpected results:</p>"),
e=f=1),w.append("<p>"+b+" => "+l+"</p>"))});ga.value.saveLastValues(k);ga.value.saveLastValue(k,r);$(q+"_progress").html("");h&&syncState();g&&closeModal();p&&closeModal2();return y};
...
ga.valuen.restore=function(a,b,c){var d="#"+a,e=$(d),f=e.find(":input").get(),h={},g,p;b=b||ga.valuen.data[a];c=c||ga.valuen.html[a];if(b){$(d+" .field_msg").html("");do g=!1,$.each(f,function(){var c,d,m=$(this),p,r;if(m.attr("data-repeater")&&!h[this.name]){h[this.name]=!0;if(this.name&&(b[this.name]||/checkbox|radio/i.test(this.type))){d=b[this.name];if(/checkbox|radio/i.test(this.type)){p=m.val();r=!1;if(d)for(c=0;c<d.length;c++)if(d[c]==p){r=!0;break}m.prop("checked",r)}else m.val(d[0]);g=!0;
