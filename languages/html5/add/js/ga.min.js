...
(e.header=e.header.padEnd(e.width));e.header=e.header.replace(/ /g,"&nbsp;");console.log(e.header);c+=">"+e.header+"</td></tr>"}c+="<tr><td>";e.label&&(c+='<label for="'+e.id+'"'+n+">"+e.label+"</label>");c+="</td>";c=e.size&&1<e.size?c+"</tr><tr><td colspan=2>":c+"<td>";c+='<select id="'+e.id+'" name="'+e.id+'"'+f;e.fontfamily&&(c+=' style="font-family: '+e.fontfamily+';"');e.size&&(c+=" size="+e.size);e.multiple&&(c+=" multiple");c+=">";if(!e.values){h+="No values for listbox "+e.id+". ";break}if(e.returns){if(e.returns&&
e.returns.length!=e.values.length){h+="Listbox values length ("+e.values.length+") does not equal return length ("+e.returns.length+") for listbox "+e.id+". ";break}for(f=0;f<e.values.length;++f)c+='<option value="'+e.returns[f]+'">'+e.values[f].replace(/ /g,"&nbsp")+"</option>"}else for(f=0;f<e.values.length;++f)c+='<option value="'+f+'">'+e.values[f].replace(/ /g,"&nbsp")+"</option>";e["default"]&&(g+='$("#'+e.id+" option[value='"+e["default"]+'\']").attr("selected", "true");',console.log("Listbox qeval: "+
g));c+="</select>"+help_span+"</td></tr>";break;default:h+="Unknown or currently unsupported field:type "+e.type+". "}c+="</table></form>";if(b._question.buttons&&b._question.buttons.length)for(d=0;d<b._question.buttons.length;++d)switch(e=b._question.buttons[d],typeof e){case "string":bid=e.replace(/\W/g,"").toLowerCase();r[bid]&&(h+="Duplicate id in _question fields & buttons:"+bid+". ");r[bid]=1;k.push({id:bid,label:e,cb:ga.qr.cb,adata:[b,bid]});break;case "object":!e.id&&e.label&&(e.id=e.label.replace(/\W/g,
