...
ga.data.update=function(a,b,c,d){var e=0,f=0,h=0,g=0,p=0,k=a+"_output",q="#"+k,m=$(q),y={},r=q+"_msgs",w=$(r),n,u,x,A,v,B,z;c&&($("#"+a+"_progress").html(""),w.text(""));$.each(b,function(b,l){z=m.find("#"+b);if(z.length)switch(e||(w.text(""),e=1),z.attr("type")){case "plot2d":n="#"+b;var r,C;ga.value.plot2d.zstack.reset(n);l.data?(ga.value.set.plot2d(n,l.options),ga.plotglobal=l.options,ga.dataplotglobal=l.data,$.plot(n,l.data,ga.value.get.plot2d.plot_options(n,l.options))):$.plot(n,l,ga.value.get.plot2d.plot_options(n));
$(n+"_changescale").length&&$(n+"_changescale").removeClass("hidden");if($(n+"_savetofile").length){var D=document.getElementById(b+"_savetofile"),E=$(n+"_div");html2canvas(E.get(0),{background:"#ffffff",onrendered:function(a){r=a.toDataURL("image/png").replace("image/png","image/octet-stream");C=dataURLtoFile(r,"plot.png");D.href=URL.createObjectURL(C);$(n+"_savetofile").removeClass("hidden")}})}if(ga.value.settings[n].selzoom||l.options&&l.options.selection&&l.options.selection.mode&&"xy"==l.options.selection.mode)$(n).on("plotselected",
{htag:n,data:l.data?l.data:l,options:l.data?ga.value.get.plot2d.plot_options(n,l.options):ga.value.get.plot2d.plot_options(n)},function(a,b){1E-5>b.xaxis.to-b.xaxis.from&&(b.xaxis.to=b.xaxis.from+1E-5);1E-5>b.yaxis.to-b.yaxis.from&&(b.yaxis.to=b.yaxis.from+1E-5);ga.value.plot2d.zstack.dopush(a.data.htag,b);$.plot(a.data.htag,a.data.data,$.extend(!0,{},a.data.options,{xaxis:{min:b.xaxis.from,max:b.xaxis.to},yaxis:{min:b.yaxis.from,max:b.yaxis.to}}))}).on("contextmenu",{htag:n,data:l.data?l.data:l,
options:l.data?ga.value.get.plot2d.plot_options(n,l.options):ga.value.get.plot2d.plot_options(n)},function(a){a.preventDefault();var b=ga.value.plot2d.zstack.dopop(a.data.htag);b?$.plot(a.data.htag,a.data.data,$.extend(!0,{},a.data.options,{xaxis:{min:b.xaxis.from,max:b.xaxis.to},yaxis:{min:b.yaxis.from,max:b.yaxis.to}})):$.plot(a.data.htag,a.data.data,a.data.options)});x=k+":#"+b+":last_value";$("#global_data").data(x,l);break;case "atomicstructure":x=k+":#"+b+":last_value";B=l.file?l.file:l;_jmol_info[b].script=
"set background ["+ga.colors.background+"]; set zoomlarge false;set echo top center;echo loading "+B.split("/").pop()+';refresh;load "'+B+'";';ga.set(a+":jsmoladd")&&(_jmol_info[b].script+=ga.set(a+":jsmoladd"));l.script&&(_jmol_info[b].script+=";"+l.script);$("#global_data").data(x,_jmol_info[b].script);$("#"+b).html(Jmol.getAppletHtml("jmolApplet"+b,_jmol_info[b]));break;case "checkbox":case "radio":z.prop("checked",!0);break;case "div":z.html(l);break;case "video":u=$("#"+b);v="<video ";u.attr("data-width")&&
(v+=' width="'+u.attr("data-width")+'"');u.attr("data-height")&&(v+=' height="'+u.attr("data-height")+'"');v+=" controls>";v+='<source src="'+l+'.mp4" type="video/mp4" /><source src="'+l+'.webm" type="video/webm" />';v+="</video>";u.html(v);break;case "image":u=$("#"+b);v="<img ";u.attr("data-width")&&(v+=' width="'+u.attr("data-width")+'"');u.attr("data-height")&&(v+=' height="'+u.attr("data-height")+'"');v+=' src="'+l+'">';u.html(v);break;case "filelink":A='<a href="'+l+'" target="_blank">'+l.split("/").pop()+
"</a>";x=k+":#"+b+":last_value";$("#global_data").data(x,A);$("#"+b+"_filelink").html(A);break;case "filelinkm":x=k+":#"+b+":last_value";A="";$.each(l,function(a,b){A+='<a href="'+b+'" target="_blank">'+b.split("/").pop()+"</a> "});$("#global_data").data(x,A);$("#"+b+"_filelink").html(A);break;default:$("#global_data").data("_append:"+k+"_"+b)?(z.val(z.val()+"\n"+l),z.height(parseFloat(z.prop("scrollHeight")+parseFloat(z.css("borderTopWidth"))+parseFloat(z.css("borderBottomWidth"))))):z.val(l)}else c?
"_"==b.charAt(0)?/^_fs_/.test(b)&&ga.data.nofcrefresh[a]||("_message"==b&&messagebox(l),/^_getinput/.test(b)&&"_getinput"==b&&ga.valuen.input(a,l),"_textarea"==b&&ga.data.textarea(q,l),"_airavata"==b&&ga.data.airavata(q,l),"_status"==b&&"complete"==l&&c(d,0,0)):(f||(w.append("<p>Unexpected results:</p>"),f=1),w.append("<p>"+b+" => "+l+"</p>")):"_"==b.charAt(0)?/^_fs_/.test(b)&&ga.data.nofcrefresh[a]||($("#_state").data(b,l),h=1,"_status"==b&&(y.job_status=l),/^_getinput/.test(b)&&"_getinput"==b&&
ga.valuen.input(a,l),"_textarea"==b&&ga.data.textarea(q,l),"_airavata"==b&&ga.data.airavata(q,l),"_loginverify"==b&&ga.login.verify(l),"_loginapprove"==b&&ga.login.approve(l)):"-close"==b?g=1:"-close2"==b?p=1:(f||(w.text(""),w.append("<p>Unexpected results:</p>"),e=f=1),w.append("<p>"+b+" => "+l+"</p>"))});ga.value.saveLastValues(k);ga.value.saveLastValue(k,r);$(q+"_progress").html("");h&&syncState();g&&closeModal();p&&closeModal2();return y};
