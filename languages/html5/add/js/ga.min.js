...
p.push(m[2]),p.push(m[0]),p.push(m[3])):h>8+e+v+y&&h<9+e+v+y+B?(m=a[h].trim().split(/\s+/),z.push(m[1]),z.push(m[2]),z.push(m[0]),z.push(m[3])):g.push(a[h]);u.push(k);u.push(r);u.push(p);u.push(z);for(h=f=0;h<c.length;h++)if($("#"+c[h]).length)switch($("#"+c[h]).data("repeater")&&(q.push(u[f]),f++),$("#"+c[h]).attr("type")){case "text":k=new RegExp($("#"+c[h]).attr("pattern"));if(!k.test(g[h])){ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+x+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",
...
ga.value.setLastValue=function(a,b,c){var d=a+":"+b+":last_value",e=a+":"+b+":default_value",f=$(b);if(!/_output$/.test(a))return!1;if(void 0==$("#global_data").data(d))switch(f.attr("type")){case "checkbox":case "radio":$("#global_data").data(d,f.is(":checked"));$("#global_data").data(e,f.is(":checked"));break;case "div":case "msgs":$("#global_data").data(d,f.html());$("#global_data").data(e,f.html());break;case "plot3d":console.log("PLOT3D: "+b);f=b.replace(/^#/,"");console.log("PLOT3D: "+f);Plotly.newPlot(f,
[],{});Plotly.purge(f);break;case "plot2d":break;case "bokeh":ga.bokeh.renderdata(a,b.replace(/^#/,""));break;case "filelink":case "filelinkm":$("#global_data").data(d,$(b+"_filelink").html());break;default:c&&f.val(c),$("#global_data").data(d,f.val()),$("#global_data").data(e,f.val())}else switch(f.attr("type")){case "checkbox":case "radio":f.prop("checked",$("#global_data").data(d));break;case "div":case "msgs":f.html($("#global_data").data(d));break;case "atomicstructure":b=b.replace(/^#/,"");
$("#global_data").data(d)?(_jmol_info[b].script=$("#global_data").data(d),f.html(Jmol.getAppletHtml("jmolApplet"+b,_jmol_info[b]))):f.html("");break;case "plot2d":d=gd.data(d);d.data?(ga.value.set.plot2d(b,d.options),f.plot(d.data,ga.value.get.plot2d.plot_options(b,d.options))):f.plot(d,ga.value.get.plot2d.plot_options(b));break;case "bokeh":break;case "filelink":case "filelinkm":$(b+"_filelink").html($("#global_data").data(d));break;default:f.val($("#global_data").data(d))}};
...
jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(p),ga.repeat.data[a].repeater[p].value&&delete ga.repeat.data[a].repeater[p].value);break;case "listbox":g=b.replace(/-[0-9]+$/,"").replace(/^(.*)-([A-ZA-z0-9_]*)$/,"$2")+"-"+d;r=b+"-"+d;g=ga.repeat.children(a,g);for(k in g)p=r+"-"+k,ga.repeat.map[k]=p,h+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,p).replace("%%label%%",""),q+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,p),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[p]||
(ga.repeat.data[a].repeater[p]=jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(p),ga.repeat.data[a].repeater[p].value&&delete ga.repeat.data[a].repeater[p].value);break;default:return console.warn("ga.repeat.change( "+a+" , "+b+" ) type "+ga.repeat.data[a].repeater[b].type+" not supported"),!1}!/^<tr>/.test(h)&&/<\/tr>$/.test(h)&&(h="<tr>"+h);$(f+"-repeater").html(h);eval(q);ga.repeat.data[a].repeater[b].value=d;for(k=0;k<e.length;++k)ga.repeat.change(a,e[k],c);$("#global_data").data("_pull_json#"+
...
ga.data.nofcrefresh={};function showTooltip(a,b,c,d){$('<div id="rtooltip">'+c+"</div>").css({position:"absolute",top:b+5,left:a+5,border:"1px solid #181616",padding:"2px","background-color":d,color:"white"}).appendTo("body")}ga.data.dataURLtoFile=function(a,b){for(var c=a.split(","),d=c[0].match(/:(.*?);/)[1],c=atob(c[1]),e=c.length,f=new Uint8Array(e);e--;)f[e]=c.charCodeAt(e);return new File([f],b,{type:d})};
...
l),g=1,"_status"==n&&(z.job_status=l),/^_getinput/.test(n)&&"_getinput"==n&&ga.valuen.input(a,l),"_textarea"==n&&ga.data.textarea(r,l),"_airavata"==n&&ga.data.airavata(r,l),"_loginverify"==n&&ga.login.verify(l),"_loginapprove"==n&&ga.login.approve(l)):"-close"==n?h=1:"-close2"==n?q=1:(f||(x.text(""),x.append("<p>Unexpected results:</p>"),e=f=1),x.append("<p>"+n+" => "+l+"</p>"))});ga.value.saveLastValues(k);ga.value.saveLastValue(k,u);$(r+"_progress").html("");g&&syncState();h&&ga.msg.close(1);q&&
...
ga.qr.question=function(a,b){var c="",d,e,f,g="",h,q=[],k={};b._uuid||(g+="no _uuid in received data. ");b._msgid||(g+="no _msgid in received data. ");b._question||(g+="no _question in received data. ");b._question.fields||(g+="no _question:fields in received data. ");if(g.length)return ga.qr.rerror(b,g);b._question.title&&(c+="<h3>"+b._question.title+"</h3>");b._question.text&&(c+=b._question.text);h=b._uuid+"-"+b._msgid;c+='<form id="'+h+'"><table>';for(d=0;d<b._question.fields.length;++d)if(f=
b._question.fields[d],f.id||(g+="No id in field "+d+". "),f.type||(g+="No type in field "+d+". "),f.id&&f.type)switch(k[f.id]&&(g+="Duplicate id in _question fields:"+f.id+". "),k[f.id]=1,f.type){case "label":c+="<tr><td colspan=2>";f.label&&(c+=f.label);c+="</td></tr>";break;case "text":c+="<tr><td>";f.label&&(c+='<label for="'+f.id+'">'+f.label+"</label>");c+='</td><td><input type="text" id="'+f.id+'"';f.required&&(c+=" required");f.readonly&&(c+=" readonly");f["default"]&&(c+=' value="'+f["default"]+
'"');f.pattern&&(c+=' pattern="'+f.pattern+'"');f.maxlength&&(c+=' maxlength="'+f.maxlength+'"');f.size&&(c+=' size="'+f.size+'"');c+="></td></tr>";break;case "textarea":c+="<tr><td>";f.label&&(c+='<label for="'+f.id+'">'+f.label+"</label>");c+='</td><td><textarea id="'+f.id+'"';f.required&&(c+=" required");c+=" readonly";f.fontfamily&&(c+=' style="font-family: '+f.fontfamily+';"');f.cols&&(c+=" cols="+f.cols+"");f.rows&&(c+=" rows="+f.rows+"");c+=">";f["default"]&&(c+=f["default"]);c+="</textarea></td></tr>";
break;case "checkbox":c+="<tr><td>";f.label&&(c+='<label for="'+f.id+'" class="highlight">'+f.label+"</label>");c+='</td><td><input type="checkbox" id="'+f.id+'"';f.checked&&(c+=" checked");f.readonly&&(c+=" readonly");c+="></td></tr>";break;case "listbox":c+="<tr><td>";f.label&&(c+='<label for="'+f.id+'" class="highlight">'+f.label+"</label>");c+="</td>";c=f.size&&1<f.size?c+"</tr><tr><td colspan=2>":c+"<td>";c+='<select id="'+f.id+'"';f.fontfamily&&(c+=' style="font-family: '+f.fontfamily+';"');
f.size&&(c+=" size="+f.size);c+=">";if(!f.values){g+="No values for listbox "+f.id+". ";break}if(f.returns){if(f.returns&&f.returns.length!=f.values.length){g+="Listbox values length ("+f.values.length+") does not equal return length ("+f.returns.length+") for listbox "+f.id+". ";break}for(e=0;e<f.values.length;++e)c+='<option value="'+f.returns[e]+'">'+f.values[e].replace(/ /g,"&nbsp")+"</option>"}else for(e=0;e<f.values.length;++e)c+='<option value="'+e+'">'+f.values[e].replace(/ /g,"&nbsp")+"</option>";
c+="</select></td></tr>";break;default:g+="Unknown or currently unsupported field:type "+f.type+". "}c+="</table></form>";if(b._question.buttons&&b._question.buttons.length)for(d=0;d<b._question.buttons.length;++d)switch(e=b._question.buttons[d],typeof e){case "string":bid=e.replace(/\W/g,"").toLowerCase();k[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". ");k[bid]=1;q.push({id:bid,label:e,cb:ga.qr.cb,adata:[b,bid]});break;case "object":e.id?(k[e.id]&&(g+="Duplicate id in _question fields & buttons:"+
e.id+". "),k[e.id]=1,e.id!=e.id.replace(/\W/g,"")&&(g+="Buttons array object entry "+(d+1)+" id is not pure alphanumeric. ")):g+="Buttons array object entry "+(d+1)+" does not have an id. ";e.label||(g+="Buttons array object entry "+(d+1)+" does not have a label. ");if(!g.length){q.push({id:e.id,label:e.label,cb:ga.qr.cb,adata:[b,e.id]});break}break;default:g+="Buttons array entry "+(d+1)+" unknown type: "+typeof e+". "}else bid="ok",k[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". "),
k[bid]=1,bid="cancel",k[bid]&&(g+="Duplicate id in _question fields & buttons:"+bid+". "),k[bid]=1,q=[{id:"ok",label:"OK",cb:ga.qr.cb,adata:[b,"ok"]},{id:"cancel",label:"Cancel",cb:ga.qr.cb,adata:[b,"cancel"]}];if(g.length)return ga.qr.rerror(b,g);ga.qr.openq[h]="open";ga.msg.box({icon:"question.png",text:c,eval:'$("#'+h+'").on("keyup keypress", function(e) { var code = e.keyCode || e.which;  if (code  == 13) { e.preventDefault(); return false; }});',buttons:q})};
