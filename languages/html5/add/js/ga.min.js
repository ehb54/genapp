...
ga.value.input[b][c].mode,ga.value.input[b][c].ids))};ga.altfile.button.job=function(a,b,c){var d="";b="#"+b;var e="#"+c,f="",h="_hidden_job_sels_"+c;$(a).jstree(!0);$(b+" ."+h).remove();$.each($(a).jstree("get_checked",!0),function(){this.children.length||(f+='<input type="hidden" name="'+c+'_altval[]" value="'+this.id+'" class="'+h+'">',d+="<tr><td>"+this.parent+":"+this.text+"</td></tr>")});d.length&&($(e+"_altval").html("<table>"+d+"</table>"),$(e+"_msg").html(""),$(e).val(""),$(b).append(f))};
...
ga.airavata.select=function(a,b,c,d){var e=ga.airavata.data,f="",h=[],g,m,k;if("__resource__"==a&&!e.defaultresource||"airavata"!=a&&"__resource__"!=a)return"notused";if(!e.resources||!e.resources.length)return ga.msg.box({icon:"warning.png",text:"No resources currently enabled for Airavata submission"}),"abort";if(1==e.resources.length)return Object.keys(e.resources[k])[0];a="__airavataselect__"!=b?b:e.select.length?e.select:"random";switch(a){case "random":return k=Math.floor(e.resources.length*
Math.random()),Object.keys(e.resources[k])[0];case "choose":h.push({id:"submit_module",label:"Submit",data:[c,d,e.resources],cb:function(a){a[0](a[1],Object.keys(a[2][$("#airavata input[name=selectresource]:checked").val()])[0])}});f='<h3>Select a compute resource and press submit</h3><form id="airavata"><table>';for(g in e.resources)for(m in e.resources[g])f+='<tr><td><input type="radio" name="selectresource" id="airavata_'+g+'" value="'+g+'"'+(0==g?'checked="checked"':"")+'></td><td class="hoverhighlight" style="text-align:left"><label for="airavata_'+
g+'">'+e.resources[g][m]+"</label></td></tr>";ga.msg.box({icon:"question.png",text:f+"</table>",buttons:h});return"deferred";default:return ga.msg.box({icon:"toast.png",text:"ga.airavata.select, unknown selection type '"+a+"'"}),"abort"}};ga.resource={};ga.xsede={};ga.resource.xsedeprojecttokeys=function(){var a;ga.resource.xsedepkeys={};for(a in ga.resource.xsedeproject)ga.resource.xsedepkeys[ga.resource.xsedeproject[a]]=1};
ga.xsede.select=function(a,b,c){var d=ga.xsede.data;a="__resource__"==a?ga.resource.defaultval:a;var e="",f=[],h;delete ga.xsede.useproject;if(!(ga.resource.xsedeproject&&a in ga.resource.xsedepkeys))return b(c);if(!d||!d.length)return ga.msg.box({icon:"warning.png",text:"No XSEDE projects currently defined.  Create one under the user configuration button at the top right."}),"abort";if(1==d.length)return ga.xsede.useproject=d[0],b(c);f.push({id:"submit_xsedeproject",label:"Submit",data:[b,c,d],cb:function(a){ga.xsede.useproject=
a[2][$("#xsedeproject input[name=selectxsedeproject]:checked").val()];a[0](a[1])}});e='<h3>Select an XSEDE project and press submit</h3><form id="xsedeproject"><table>';for(h in d)e+='<tr><td><input type="radio" name="selectxsedeproject" id="xsedeproject_'+h+'" value="'+h+'"'+(0==h?'checked="checked"':"")+'></td><td class="hoverhighlight" style="text-align:left"><label for="xsedeproject_'+h+'">'+d[h]+"</label></td></tr>";ga.msg.box({icon:"question.png",text:e+"</table>",buttons:f});return"deferred"};
...
ga.value.processInputfromFiles=function(a,b,c,d){var e=a.trim().split(/[\r\n]+/g),f=[],h=0;c=ga.repeat.map.convert(c);for(var g=0;g<e.length;g++){var m=e[g].trim().split(/\s+/),h=h+m.length;f.push(h)}var h=[],k=[],r=[],q=[],y=[],u=[],m=[],x=0;switch(b){case "whitespace_formulchcompost":a=[];for(g=0;g<e.length;g++)f=e[g].split("#")[0],f=f.replace(/\s{2,}/g," "),f=f.replace(/\t/g," "),f=f.toString().trim().replace(/(\r\n|\n|\r)/g,""),a.push(f);for(var e=parseInt(a[1]),n,g=0;g<a.length;g++)if(2<g&&g<
3+e)for(n=a[g].trim().split(/\s+/),f=0;f<n.length;f++)k.push(n[f]);else h.push(a[g]);m.push(k);for(g=0;g<c.length;g++)switch($("#"+c[g]).attr("type")){case "text":k=new RegExp($("#"+c[g]).attr("pattern"));if(!k.test(h[g])){ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+x+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;case "number":k=[h[g]];
...
[];for(g=0;g<e.length;g++)f=e[g].split("#")[0],f=f.replace(/\s{2,}/g," "),f=f.replace(/\t/g," "),f=f.toString().trim().replace(/(\r\n|\n|\r)/g,""),a.push(f);for(var e=parseInt(a[1]),v=parseInt(a[2+e]),z=parseInt(a[5+e+v]),B=parseInt(a[9+e+v+z]),g=0;g<a.length;g++)if(1<g&&g<2+e)for(n=a[g].trim().split(/\s+/),f=0;f<n.length;f++)k.push(n[f]);else g>2+e&&g<3+e+v?(n=a[g].trim().split(/\s+/),r.push(n[1]),r.push(n[2]),r.push(n[0]),r.push(n[3])):g>5+e+v&&g<6+e+v+z?(n=a[g].trim().split(/\s+/),q.push(n[1]),
q.push(n[2]),q.push(n[0]),q.push(n[3])):g>8+e+v+z&&g<9+e+v+z+B?(n=a[g].trim().split(/\s+/),y.push(n[1]),y.push(n[2]),y.push(n[0]),y.push(n[3])):h.push(a[g]);u.push(k);u.push(r);u.push(q);u.push(y);for(g=f=0;g<c.length;g++)if($("#"+c[g]).length)switch($("#"+c[g]).data("repeater")&&(m.push(u[f]),f++),$("#"+c[g]).attr("type")){case "text":k=new RegExp($("#"+c[g]).attr("pattern"));if(!k.test(h[g])){ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+x+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",
buttons:[{id:"ok",label:"OK"}]});return}break;case "number":k=[h[g]];if(ga.value.checkFloatIntOK("#"+c[g],k))h[g]=k[0];else{ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+x+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;default:ga.msg.box({icon:"warning.png",text:"Selected input type is currently not supported. Contact the developer",buttons:[{id:"ok",
label:"OK"}]});return}else f++;break;case "whitespaceseparated":case "whitespaceseparated_reverselogic":h=a.trim().split(/\s+/);if(h.length==c.length)for(g=0;g<h.length;g++){for(u=0;u<f.length;u++)if(g+1<=f[u]){x=u+1;break}switch($("#"+c[g]).attr("type")){case "checkbox":k=/^(0|1|n|y|true|false|t|f|yes|no)$/;if(!k.test(h[g].toLowerCase())){ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Checkbox input value on the line #"+x+" is not valid. Options are: [1 | 0 | yes | no | true | false | t | f | T | F | y | n | Y | N ]. Check your input file",
...
ga.repeat.change=function(a,b,c){var d,e=[],f="#"+b;d=$(f);var h,g="",m="",k,r,q;if(!(ga.repeat.data[a]&&ga.repeat.data[a].repeater&&ga.repeat.data[a].repeater[b]&&d.length))return!1;switch(ga.repeat.data[a].repeater[b].type){case "checkbox":d=d.prop("checked")?1:0;break;case "integer":case "listbox":d=d.val();break;default:return console.warn("ga.repeat.change( "+a+" , "+b+" ) type "+ga.repeat.data[a].repeater[b].type+" not supported"),!1}if(!c&&ga.repeat.data[a].repeater[b].value===d)return!1;h=
ga.repeat.children(a,b);switch(ga.repeat.data[a].repeater[b].type){case "checkbox":if(d)for(k in h)q=b+"-"+k,ga.repeat.map[k]=q,g+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,q).replace("%%label%%",""),m+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,q),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[q]||(ga.repeat.data[a].repeater[q]=jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(q),ga.repeat.data[a].repeater[q].value&&delete ga.repeat.data[a].repeater[q].value);
break;case "integer":if(ga.repeat.data[a].repeater[b].tableize&&0<d)for(k in h)g+=ga.repeat.data[a].repeat[k].htmls;for(r=1;r<=d;++r)for(k in h)q=b+"-"+k+"-"+(r-1),ga.repeat.map[k]=q,g+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,q).replace("%%label%%","["+r+"]").replace(ga.repeat.data[a].repeater[b].tableize?/<td.*?><label.*?>.*?<\/label><\/td>/:"",""),m+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,q),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[q]||(ga.repeat.data[a].repeater[q]=
jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(q),ga.repeat.data[a].repeater[q].value&&delete ga.repeat.data[a].repeater[q].value);break;case "listbox":h=b.replace(/-[0-9]+$/,"").replace(/^(.*)-([A-ZA-z0-9_]*)$/,"$2")+"-"+d;r=b+"-"+d;h=ga.repeat.children(a,h);for(k in h)q=r+"-"+k,ga.repeat.map[k]=q,g+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,q).replace("%%label%%",""),m+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,q),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[q]||
(ga.repeat.data[a].repeater[q]=jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(q),ga.repeat.data[a].repeater[q].value&&delete ga.repeat.data[a].repeater[q].value);break;default:return console.warn("ga.repeat.change( "+a+" , "+b+" ) type "+ga.repeat.data[a].repeater[b].type+" not supported"),!1}!/^<tr>/.test(g)&&/<\/tr>$/.test(g)&&(g="<tr>"+g);$(f+"-repeater").html(g);eval(m);ga.repeat.data[a].repeater[b].value=d;for(k=0;k<e.length;++k)ga.repeat.change(a,e[k],c);$("#global_data").data("_pull_json#"+
...
ga.data.update=function(a,b,c,d){var e=0,f=0,h=0,g=0,m=0,k=a+"_output",r="#"+k,q=$(r),y={},u=r+"_msgs",x=$(u),n,v,z,B,w,C,A;c&&($("#"+a+"_progress").html(""),x.text(""));$.each(b,function(p,l){A=q.find("#"+p);if(A.length)switch(e||(x.text(""),e=1),A.attr("type")){case "plot2d":n="#"+p;ga.value.plot2d.zstack.reset(n);l.data?(ga.value.set.plot2d(n,l.options),ga.plotglobal=l.options,ga.dataplotglobal=l.data,$.plot(n,l.data,ga.value.get.plot2d.plot_options(n,l.options))):$.plot(n,l,ga.value.get.plot2d.plot_options(n));
$(n+"_savetofile").length&&$(n+"_savetofile").removeClass("hidden");$(n+"_changescalex").length&&($(n+"_changescalex").removeClass("hidden"),"log"==l.options.xscale?$(n+"_changescalex_message").html("X-log"):$(n+"_changescalex_message").html("X-lin"));$(n+"_changescaley").length&&($(n+"_changescaley").removeClass("hidden"),"log"==l.options.yscale?$(n+"_changescaley_message").html("Y-log"):$(n+"_changescaley_message").html("Y-lin"));if(ga.value.settings[n].selzoom||l.options&&l.options.selection&&
l.options.selection.mode&&"xy"==l.options.selection.mode)$(n).on("plotselected",{htag:n,data:l.data?l.data:l,options:l.data?ga.value.get.plot2d.plot_options(n,l.options):ga.value.get.plot2d.plot_options(n)},function(a,b){1E-5>b.xaxis.to-b.xaxis.from&&(b.xaxis.to=b.xaxis.from+1E-5);1E-5>b.yaxis.to-b.yaxis.from&&(b.yaxis.to=b.yaxis.from+1E-5);ga.value.plot2d.zstack.dopush(a.data.htag,b);$.plot(a.data.htag,a.data.data,$.extend(!0,{},a.data.options,{xaxis:{min:b.xaxis.from,max:b.xaxis.to},yaxis:{min:b.yaxis.from,
max:b.yaxis.to}}))}).on("contextmenu",{htag:n,data:l.data?l.data:l,options:l.data?ga.value.get.plot2d.plot_options(n,l.options):ga.value.get.plot2d.plot_options(n)},function(a){a.preventDefault();var b=ga.value.plot2d.zstack.dopop(a.data.htag);b?$.plot(a.data.htag,a.data.data,$.extend(!0,{},a.data.options,{xaxis:{min:b.xaxis.from,max:b.xaxis.to},yaxis:{min:b.yaxis.from,max:b.yaxis.to}})):$.plot(a.data.htag,a.data.data,a.data.options)});z=k+":#"+p+":last_value";$("#global_data").data(z,l);break;case "bokeh":ga.bokeh.render(a,
p,l);break;case "atomicstructure":z=k+":#"+p+":last_value";C=l.file?l.file:l;_jmol_info[p].script="set background ["+ga.colors.background+"]; set zoomlarge false;set echo top center;echo loading "+C.split("/").pop()+';refresh;load "'+C+'";';ga.set(a+":jsmoladd")&&(_jmol_info[p].script+=ga.set(a+":jsmoladd"));l.script&&(_jmol_info[p].script+=";"+l.script);$("#global_data").data(z,_jmol_info[p].script);$("#"+p).html(Jmol.getAppletHtml("jmolApplet"+p,_jmol_info[p]));break;case "checkbox":case "radio":A.prop("checked",
...
w+=' src="'+l+'">';v.html(w);break;case "filelink":B='<a href="'+l+'" target="_blank">'+l.split("/").pop()+"</a>";z=k+":#"+p+":last_value";$("#global_data").data(z,B);$("#"+p+"_filelink").html(B);break;case "filelinkm":z=k+":#"+p+":last_value";B="";$.each(l,function(a,b){B+='<a href="'+b+'" target="_blank">'+b.split("/").pop()+"</a> "});$("#global_data").data(z,B);$("#"+p+"_filelink").html(B);break;default:$("#global_data").data("_append:"+k+"_"+p)?(A.val(A.val()+"\n"+l),A.height(parseFloat(A.prop("scrollHeight")+
...
f=1),x.append("<p>"+p+" => "+l+"</p>")):"_"==p.charAt(0)?/^_fs_/.test(p)&&ga.data.nofcrefresh[a]||($("#_state").data(p,l),h=1,"_status"==p&&(y.job_status=l),/^_getinput/.test(p)&&"_getinput"==p&&ga.valuen.input(a,l),"_textarea"==p&&ga.data.textarea(r,l),"_airavata"==p&&ga.data.airavata(r,l),"_loginverify"==p&&ga.login.verify(l),"_loginapprove"==p&&ga.login.approve(l)):"-close"==p?g=1:"-close2"==p?m=1:(f||(x.text(""),x.append("<p>Unexpected results:</p>"),e=f=1),x.append("<p>"+p+" => "+l+"</p>"))});
ga.value.saveLastValues(k);ga.value.saveLastValue(k,u);$(r+"_progress").html("");h&&syncState();g&&closeModal();m&&closeModal2();return y};
...
ga.valuen.restore=function(a,b,c){var d="#"+a,e=$(d),f=e.find(":input").get(),h={},g,m;b=b||ga.valuen.data[a];c=c||ga.valuen.html[a];if(b){$(d+" .field_msg").html("");do g=!1,$.each(f,function(){var c,d,m=$(this),y,u;if(m.attr("data-repeater")&&!h[this.name]){h[this.name]=!0;if(this.name&&(b[this.name]||/checkbox|radio/i.test(this.type))){d=b[this.name];if(/checkbox|radio/i.test(this.type)){y=m.val();u=!1;if(d)for(c=0;c<d.length;c++)if(d[c]==y){u=!0;break}m.prop("checked",u)}else m.val(d[0]);g=!0;
...
ga.valuen.input=function(a,b){var c=$("#"+a),d=c.find(":input").get(),e={},f;if(b){do f=!1,$.each(d,function(){var h,g,m=$(this),k,r;if(m.attr("data-repeater")&&!e[this.name]){e[this.name]=!0;if(this.name&&(b[this.name]||/checkbox|radio/i.test(this.type))){g=b[this.name];"[object Array]"!==Object.prototype.toString.call(g)&&(g=[g]);if(/checkbox|radio/i.test(this.type)){k=m.val();r=!1;if(g)for(h=0;h<g.length;h++)if(g[h]==k){r=!0;break}m.prop("checked",r)}else m.val(g[0]);f=!0;ga.repeat.change(a,this.name,
...
ga.qr.question=function(a,b){console.log("ga.qr.question( "+a+", q )");var c="",d,e,f="",h,g=[],m={};b._uuid||(f+="no _uuid in received data. ");b._msgid||(f+="no _msgid in received data. ");b._question||(f+="no _question in received data. ");b._question.fields||(f+="no _question:fields in received data. ");if(f.length)return ga.qr.rerror(b,f);b._question.title&&(c+="<h3>"+b._question.title+"</h3>");b._question.text&&(c+=b._question.text);h=b._uuid+"-"+b._msgid;c+='<form id="'+h+'"><table>';for(d=
0;d<b._question.fields.length;++d)if(e=b._question.fields[d],e.id||(f+="No id in field "+d+". "),e.type||(f+="No type in field "+d+". "),e.id&&e.type)switch(m[e.id]&&(f+="Duplicate id in _question fields:"+e.id+". "),m[e.id]=1,e.type){case "label":c+="<tr><td colspan=2>";e.label&&(c+=e.label);c+="</td></tr>";break;case "text":c+="<tr><td>";e.label&&(c+='<label for="'+e.id+'">'+e.label+"</label>");c+='</td><td><input type="text" id="'+e.id+'"';e.required&&(c+=" required");e.readonly&&(c+=" readonly");
e["default"]&&(c+=' value="'+e["default"]+'"');e.pattern&&(c+=' pattern="'+e.pattern+'"');e.maxlength&&(c+=' maxlength="'+e.maxlength+'"');e.size&&(c+=' size="'+e.size+'"');c+="></td></tr>";break;case "checkbox":c+="<tr><td>";e.label&&(c+='<label for="'+e.id+'" class="highlight">'+e.label+"</label>");c+='</td><td><input type="checkbox" id="'+e.id+'"';e.checked&&(c+=" checked");e.readonly&&(c+=" readonly");c+="></td></tr>";break;default:f+="Unknown or currently unsupported field:type "+e.fype+". "}c+=
"</table></form>";if(b._question.buttons&&b._question.buttons.length)for(d=0;d<b._question.buttons.length;++d)switch(e=b._question.buttons[d],typeof e){case "string":bid=e.replace(/\W/g,"").toLowerCase();m[bid]&&(f+="Duplicate id in _question fields & buttons:"+bid+". ");m[bid]=1;g.push({id:bid,label:e,cb:ga.qr.cb,adata:[b,bid]});break;case "object":e.id?(m[e.id]&&(f+="Duplicate id in _question fields & buttons:"+e.id+". "),m[e.id]=1,e.id!=e.id.replace(/\W/g,"")&&(f+="Buttons array object entry "+
(d+1)+" id is not pure alphanumeric. ")):f+="Buttons array object entry "+(d+1)+" does not have an id. ";e.label||(f+="Buttons array object entry "+(d+1)+" does not have a label. ");if(!f.length){g.push({id:e.id,label:e.label,cb:ga.qr.cb,adata:[b,e.id]});break}break;default:f+="Buttons array entry "+(d+1)+" unknown type: "+typeof e+". "}else bid="ok",m[bid]&&(f+="Duplicate id in _question fields & buttons:"+bid+". "),m[bid]=1,bid="cancel",m[bid]&&(f+="Duplicate id in _question fields & buttons:"+
bid+". "),m[bid]=1,g=[{id:"ok",label:"OK",cb:ga.qr.cb,adata:[b,"ok"]},{id:"cancel",label:"Cancel",cb:ga.qr.cb,adata:[b,"cancel"]}];if(f.length)return ga.qr.rerror(b,f);console.log("qtext:"+c);ga.qr.openq[h]="open";ga.msg.box({icon:"question.png",text:c,eval:'$("#'+h+'").on("keyup keypress", function(e) { var code = e.keyCode || e.which;  if (code  == 13) { e.preventDefault(); return false; }});',buttons:g})};
...
"<center><table><tr>";for(i=0;i<a.buttons.length;i++)tmp=tmp+'<td><button id="_mbb_'+a.buttons[i].id+'">'+a.buttons[i].label+"</button></td>";tmp+="</tr></table><center>";$("#configbody"+c).append(tmp);for(i=0;i<a.buttons.length;i++)if(a.buttons[i].cb)if(a.buttons[i].adata)switch(a.buttons[i].adata.length){case 2:$("#_mbb_"+a.buttons[i].id).off().one("click",a.buttons[i],function(a){a.data.cb(a.data.adata[0],a.data.adata[1]);closeaModal(c)});break;case 3:$("#_mbb_"+a.buttons[i].id).off().one("click",
a.buttons[i],function(a){a.data.cb(a.data.adata[0],a.data.adata[1],a.data.adata[2]);closeaModal(c)});break;case 4:$("#_mbb_"+a.buttons[i].id).off().one("click",a.buttons[i],function(a){a.data.cb(a.data.adata[0],a.data.adata[1],a.data.adata[2],a.data.adata[3]);closeaModal(c)});break;case 5:$("#_mbb_"+a.buttons[i].id).off().one("click",a.buttons[i],function(a){a.data.cb(a.data.adata[0],a.data.adata[1],a.data.adata[2],a.data.adata[3],a.data.adata[4]);closeaModal(c)});break;case 6:$("#_mbb_"+a.buttons[i].id).off().one("click",
a.buttons[i],function(a){a.data.cb(a.data.adata[0],a.data.adata[1],a.data.adata[2],a.data.adata[3],a.data.adata[4],a.data.adata[5]);closeaModal(c)});break;default:console.warn("in ga.msg.box unsupported number of adata arguments "+a.buttons[i].adata.length)}else $("#_mbb_"+a.buttons[i].id).off().one("click",a.buttons[i],function(a){a.data.cb(a.data.data);closeaModal(c)});else $("#_mbb_"+a.buttons[i].id).off().one("click",a.buttons[i],function(a){closeaModal(c)})}a.ptext&&$("#configbody"+c).append(a.ptext);
a.eval&&eval(a.eval);ga.repeats.save();$(".modalDialog"+c).addClass("modalDialog"+c+"_on")}};
