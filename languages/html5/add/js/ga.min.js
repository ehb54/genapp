...
ga.bokeh.getdata=function(a,b){var c={},d=b.split(/\r?\n/),e=d.length,f,g=0;for(f=0;f<e;++f)if(g||-1!==d[f].indexOf("var docs_json ="))if(!g)g=1,c.docs_json=$.parseJSON(d[f].replace(/^\s*var\s+docs_json\s+=\s+/,"").replace(/;$/,""));else if(1==g&&-1!==d[f].indexOf("var render_items =")){c.render_items=$.parseJSON(d[f].replace(/^\s*var\s+render_items\s+=\s+/,"").replace(/;$/,""));break}return c};
