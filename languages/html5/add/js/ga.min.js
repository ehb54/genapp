...
function create_image(a){if($("#"+a+"_savetofile").length){var b=document.getElementById(a+"_savetofile"),c=$("#"+a+"_div");html2canvas(c.get(0),{background:"#ffffff",onrendered:function(c){image=c.toDataURL("image/png").replace("image/png","image/octet-stream");file=dataURLtoFile(image,"plot.png");b.href=URL.createObjectURL(file);$("#"+a+"_savetofile").removeClass("hidden")}})}}
...
$(n+"_changescale").length&&$(n+"_changescale").removeClass("hidden");create_image(b);if(ga.value.settings[n].selzoom||l.options&&l.options.selection&&l.options.selection.mode&&"xy"==l.options.selection.mode)$(n).on("plotselected",{htag:n,data:l.data?l.data:l,options:l.data?ga.value.get.plot2d.plot_options(n,l.options):ga.value.get.plot2d.plot_options(n)},function(a,b){1E-5>b.xaxis.to-b.xaxis.from&&(b.xaxis.to=b.xaxis.from+1E-5);1E-5>b.yaxis.to-b.yaxis.from&&(b.yaxis.to=b.yaxis.from+1E-5);ga.value.plot2d.zstack.dopush(a.data.htag,
