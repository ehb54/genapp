var ga={tmp:{},multistage:{},stages:{},stagesdefault:{},currentstage:0,fielderrors:0,customtooltips:0,showcollapse3d:0,firstplotted3d:0,showcollapse2d:0};
...
ga.altfile.button.lrfile=function(a,b,c){var d=[],f="#"+b,e="#"+c,h="",g="_hidden_lrfile_sels_"+c;$(f+" ."+g).remove();$.each($(a).jstree("get_checked",!0),function(){this.children.length||(h+='<input type="hidden" name="'+c+'_altval[]" value="'+this.id+'" class="'+g+'" data-type="rfile_val">',d.push($.base64.decode(this.id).substr(2)))});d.length&&($(e+"_altval").html("<i>Server</i>: "+d),$(e+"_msg").html(""),$(e).val(""),$(f).append(h),ga.value.input[b]&&ga.value.input[b][c]&&ga.value.setInputfromRFile(d,
ga.value.input[b][c].mode,ga.value.input[b][c].ids))};
ga.altfile.button.rpath=function(a,b,c){var d=[];b="#"+b;var f="#"+c,e="",h="_hidden_rpath_sels_"+c,g=$(a).jstree(!0);$(b+" ."+h).remove();$.each(g.get_top_checked(!0),function(){g.is_leaf(this)||(e+='<input type="hidden" name="'+c+'[]" value="'+this.id+'" class="'+h+'"><input type="hidden" name="_decodepath_'+c+'" class="'+h+'">',d.push($.base64.decode(this.id).substr(2)))});d.length&&($(f+"_altval").html("<i>Server</i>: "+d),$(f+"_msg").html(""),$(f).val(""),$(b).append(e))};
ga.altfile.button.rfile=function(a,b,c){var d=[],f="#"+b,e="#"+c,h="",g="_hidden_rfile_sels_"+c;$(a).jstree(!0);$(f+" ."+g).remove();$.each($(a).jstree("get_checked",!0),function(){this.children.length||(h+='<input type="hidden" name="'+c+'_altval[]" value="'+this.id+'" class="'+g+'" data-type="rfile_val">',d.push($.base64.decode(this.id).substr(2)))});d.length&&($(e+"_altval").html("<i>Server</i>: "+d),$(e+"_msg").html(""),$(e).val(""),$(f).append(h),ga.value.input[b]&&ga.value.input[b][c]&&ga.value.setInputfromRFile(d,
ga.value.input[b][c].mode,ga.value.input[b][c].ids))};ga.altfile.button.job=function(a,b,c){var d="";b="#"+b;var f="#"+c,e="",h="_hidden_job_sels_"+c;$(a).jstree(!0);$(b+" ."+h).remove();$.each($(a).jstree("get_checked",!0),function(){this.children.length||(e+='<input type="hidden" name="'+c+'_altval[]" value="'+this.id+'" class="'+h+'">',d+="<tr><td>"+this.parent+":"+this.text+"</td></tr>")});d.length&&($(f+"_altval").html("<table>"+d+"</table>"),$(f+"_msg").html(""),$(f).val(""),$(b).append(e))};
...
ga.airavata.select=function(a,b,c,d){var f=ga.airavata.data,e="",h=[],g,q,k;if("__resource__"==a&&!f.defaultresource||"airavata"!=a&&"__resource__"!=a)return"notused";if(!f.resources||!f.resources.length)return ga.msg.box({icon:"warning.png",text:"No resources currently enabled for Airavata submission"}),"abort";if(1==f.resources.length)return Object.keys(f.resources[k])[0];a="__airavataselect__"!=b?b:f.select.length?f.select:"random";switch(a){case "random":return k=Math.floor(f.resources.length*
Math.random()),Object.keys(f.resources[k])[0];case "choose":h.push({id:"submit_module",label:"Submit",data:[c,d,f.resources],cb:function(a){a[0](a[1],Object.keys(a[2][$("#airavata input[name=selectresource]:checked").val()])[0])}});e='<h3>Select a compute resource and press submit</h3><form id="airavata"><table>';for(g in f.resources)for(q in f.resources[g])e+='<tr><td><input type="radio" name="selectresource" id="airavata_'+g+'" value="'+g+'"'+(0==g?'checked="checked"':"")+'></td><td class="hoverhighlight" style="text-align:left"><label for="airavata_'+
g+'">'+f.resources[g][q]+"</label></td></tr>";ga.msg.box({icon:"question.png",text:e+"</table>",buttons:h});return"deferred";default:return ga.msg.box({icon:"toast.png",text:"ga.airavata.select, unknown selection type '"+a+"'"}),"abort"}};ga.resource={};ga.xsede={};ga.resource.xsedeprojecttokeys=function(){var a;ga.resource.xsedepkeys={};for(a in ga.resource.xsedeproject)ga.resource.xsedepkeys[ga.resource.xsedeproject[a]]=1};
ga.xsede.select=function(a,b,c){var d=ga.xsede.data;a="__resource__"==a?ga.resource.defaultval:a;var f="",e=[],h;delete ga.xsede.useproject;if(!(ga.resource.xsedeproject&&a in ga.resource.xsedepkeys))return b(c);if(!d||!d.length)return ga.msg.box({icon:"warning.png",text:"No XSEDE projects currently defined.  Create one under the user configuration button at the top right."}),"abort";if(1==d.length)return ga.xsede.useproject=d[0],b(c);e.push({id:"submit_xsedeproject",label:"Submit",data:[b,c,d],cb:function(a){ga.xsede.useproject=
a[2][$("#xsedeproject input[name=selectxsedeproject]:checked").val()];a[0](a[1])}});f='<h3>Select an XSEDE project and press submit</h3><form id="xsedeproject"><table>';for(h in d)f+='<tr><td><input type="radio" name="selectxsedeproject" id="xsedeproject_'+h+'" value="'+h+'"'+(0==h?'checked="checked"':"")+'></td><td class="hoverhighlight" style="text-align:left"><label for="xsedeproject_'+h+'">'+d[h]+"</label></td></tr>";ga.msg.box({icon:"question.png",text:f+"</table>",buttons:e});return"deferred"};
...
ga.value.modifyformdata=function(a,b,c,d,f){var e=[],h,g;console.log(d);$.each(a[b],function(a,b){console.log(a);-1!==jQuery.inArray(a,d)&&$.each(b,function(a,b){e.push(b)})});e=ga.repeat.map.convert(e);for(a=0;a<e.length;a++){if($("#"+e[a]).data("repeater"))for(h in g=ga.repeat.children(b,e[a]),g)g=e[a]+"-"+h,c["delete"](g);c["delete"](e[a])}c.append("currentstage",ga.stages[b][f])};
ga.value.processInputfromFiles=function(a,b,c,d){var f=a.trim().split(/[\r\n]+/g),e=[],h=0;c=ga.repeat.map.convert(c);for(var g=0;g<f.length;g++){var q=f[g].trim().split(/\s+/),h=h+q.length;e.push(h)}var h=[],k=[],r=[],n=[],z=[],u=[],q=[],x=0;switch(b){case "whitespace_formulchcompost":a=[];for(g=0;g<f.length;g++)e=f[g].split("#")[0],e=e.replace(/\s{2,}/g," "),e=e.replace(/\t/g," "),e=e.toString().trim().replace(/(\r\n|\n|\r)/g,""),a.push(e);for(var f=parseInt(a[1]),m,g=0;g<a.length;g++)if(2<g&&g<
3+f)for(m=a[g].trim().split(/\s+/),e=0;e<m.length;e++)k.push(m[e]);else h.push(a[g]);q.push(k);for(g=0;g<c.length;g++)switch($("#"+c[g]).attr("type")){case "text":k=new RegExp($("#"+c[g]).attr("pattern"));if(!k.test(h[g])){ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+x+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;case "number":k=[h[g]];
if(ga.value.checkFloatIntOK("#"+c[g],k))h[g]=k[0];else{ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+x+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;default:ga.msg.box({icon:"warning.png",text:"Selected input type is currently not supported. Contact the developer",buttons:[{id:"ok",label:"OK"}]});return}break;case "whitespace_formulchrg":case "whitespace_formulchcontrast":a=
[];for(g=0;g<f.length;g++)e=f[g].split("#")[0],e=e.replace(/\s{2,}/g," "),e=e.replace(/\t/g," "),e=e.toString().trim().replace(/(\r\n|\n|\r)/g,""),a.push(e);for(var f=parseInt(a[1]),v=parseInt(a[2+f]),y=parseInt(a[5+f+v]),B=parseInt(a[9+f+v+y]),g=0;g<a.length;g++)if(1<g&&g<2+f)for(m=a[g].trim().split(/\s+/),e=0;e<m.length;e++)k.push(m[e]);else g>2+f&&g<3+f+v?(m=a[g].trim().split(/\s+/),r.push(m[1]),r.push(m[2]),r.push(m[0]),r.push(m[3])):g>5+f+v&&g<6+f+v+y?(m=a[g].trim().split(/\s+/),n.push(m[1]),
n.push(m[2]),n.push(m[0]),n.push(m[3])):g>8+f+v+y&&g<9+f+v+y+B?(m=a[g].trim().split(/\s+/),z.push(m[1]),z.push(m[2]),z.push(m[0]),z.push(m[3])):h.push(a[g]);u.push(k);u.push(r);u.push(n);u.push(z);for(g=e=0;g<c.length;g++)if($("#"+c[g]).length)switch($("#"+c[g]).data("repeater")&&(q.push(u[e]),e++),$("#"+c[g]).attr("type")){case "text":k=new RegExp($("#"+c[g]).attr("pattern"));if(!k.test(h[g])){ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+x+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",
buttons:[{id:"ok",label:"OK"}]});return}break;case "number":k=[h[g]];if(ga.value.checkFloatIntOK("#"+c[g],k))h[g]=k[0];else{ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+x+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;default:ga.msg.box({icon:"warning.png",text:"Selected input type is currently not supported. Contact the developer",buttons:[{id:"ok",
label:"OK"}]});return}else e++;break;case "whitespaceseparated":case "whitespaceseparated_reverselogic":h=a.trim().split(/\s+/);if(h.length==c.length)for(g=0;g<h.length;g++){for(u=0;u<e.length;u++)if(g+1<=e[u]){x=u+1;break}switch($("#"+c[g]).attr("type")){case "checkbox":k=/^(0|1|n|y|true|false|t|f|yes|no)$/;if(!k.test(h[g].toLowerCase())){ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Checkbox input value on the line #"+x+" is not valid. Options are: [1 | 0 | yes | no | true | false | t | f | T | F | y | n | Y | N ]. Check your input file",
buttons:[{id:"ok",label:"OK"}]});return}break;case "number":k=[h[g]];if(ga.value.checkFloatIntOK("#"+c[g],k))h[g]=k[0];else{ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+x+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;case "text":k=new RegExp($("#"+c[g]).attr("pattern"));if(!k.test(h[g])){ga.msg.box({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+
...
ga.value.setInputfromFile=function(a,b,c,d){$(a).hide();var f=c.split(",");$(a).change(function(c){c=$(a)[0].files[0];var h=new FileReader;h.onload=function(a){ga.value.processInputfromFiles(a.target.result,b,f,d)};h.readAsText(c)})};
...
b);$("#global_data").data(a+":"+b+":last_value",[[]]);ga.value.clear.plot2d(b);c.plot([[]],ga.value.get.plot2d.plot_options(b));ga.showcollapse2d&&($(b+"_div").hide(),$(b+"_savetofile").length&&($(b+"_savetofile").hide(),$(b+"_savetofile_link").hide()),$(b+"_changescalex").length&&($(b+"_changescalex").hide(),$(b+"_changescalex_message").hide()),$(b+"_changescaley").length&&($(b+"_changescaley").hide(),$(b+"_changescaley_message").hide()),$(b+"_showcollapse").length&&$(b+"_showcollapse").addClass("hidden"));
break;case "bokeh":ga.bokeh.reset(a,b.replace(/^#/,""));break;case "plot3d":console.log("reset default value for plot3d: "+b);Plotly.purge(b.replace(/^#/,""));$(b+"_showcollapse").length&&$(b+"_showcollapse").addClass("hidden");break;case "image":c.html("");break;case "video":c.html("");break;case "atomicstructure":b.replace(/^#/,"");$("#global_data").data(a+":"+b+":last_value","");$(b).html("");break;default:c.val(c.attr("value"))}ga.value.saveLastValue(a,b);$(b+"_msg").html("")};
...
ga.pull.toPull=function(a,b,c,d,f){f&&"string"==typeof f&&0!=f.length&&"#__fields:repeat__"!=f||(f="");a=$("#global_data");var e=a.data("_pull_json"+f)||{},h=a.data("_pull_update"+f)||{};e[d]=0;"object"!=typeof h[d]&&(h[d]={});h[d][b]=c;a.data("_pull_json"+f,e);a.data("_pull_update"+f,h)};
...
ga.repeat.change=function(a,b,c){var d,f=[],e="#"+b;d=$(e);var h,g="",q="",k,r,n;if(!(ga.repeat.data[a]&&ga.repeat.data[a].repeater&&ga.repeat.data[a].repeater[b]&&d.length))return!1;switch(ga.repeat.data[a].repeater[b].type){case "checkbox":d=d.prop("checked")?1:0;break;case "integer":case "listbox":d=d.val();break;default:return console.warn("ga.repeat.change( "+a+" , "+b+" ) type "+ga.repeat.data[a].repeater[b].type+" not supported"),!1}if(!c&&ga.repeat.data[a].repeater[b].value===d)return!1;h=
ga.repeat.children(a,b);switch(ga.repeat.data[a].repeater[b].type){case "checkbox":if(d)for(k in h)n=b+"-"+k,ga.repeat.map[k]=n,g+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,n).replace("%%label%%",""),q+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,n),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[n]||(ga.repeat.data[a].repeater[n]=jQuery.extend({},ga.repeat.data[a].repeater[k])),f.push(n),ga.repeat.data[a].repeater[n].value&&delete ga.repeat.data[a].repeater[n].value);
break;case "integer":if(ga.repeat.data[a].repeater[b].tableize&&0<d)for(k in h)g+=ga.repeat.data[a].repeat[k].htmls;for(r=1;r<=d;++r)for(k in h)n=b+"-"+k+"-"+(r-1),ga.repeat.map[k]=n,g+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,n).replace("%%label%%","["+r+"]").replace(ga.repeat.data[a].repeater[b].tableize?/<td.*?><label.*?>.*?<\/label><\/td>/:"",""),q+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,n),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[n]||(ga.repeat.data[a].repeater[n]=
jQuery.extend({},ga.repeat.data[a].repeater[k])),f.push(n),ga.repeat.data[a].repeater[n].value&&delete ga.repeat.data[a].repeater[n].value);break;case "listbox":h=b.replace(/-[0-9]+$/,"").replace(/^(.*)-([A-ZA-z0-9_]*)$/,"$2")+"-"+d;r=b+"-"+d;h=ga.repeat.children(a,h);for(k in h)n=r+"-"+k,ga.repeat.map[k]=n,g+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,n).replace("%%label%%",""),q+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,n),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[n]||
(ga.repeat.data[a].repeater[n]=jQuery.extend({},ga.repeat.data[a].repeater[k])),f.push(n),ga.repeat.data[a].repeater[n].value&&delete ga.repeat.data[a].repeater[n].value);break;default:return console.warn("ga.repeat.change( "+a+" , "+b+" ) type "+ga.repeat.data[a].repeater[b].type+" not supported"),!1}!/^<tr>/.test(g)&&/<\/tr>$/.test(g)&&(g="<tr>"+g);$(e+"-repeater").html(g);eval(q);ga.repeat.data[a].repeater[b].value=d;for(k=0;k<f.length;++k)ga.repeat.change(a,f[k],c);$("#global_data").data("_pull_json#"+
...
ga.valuen.restore=function(a,b,c){var d="#"+a,f=$(d),e=f.find(":input").get(),h={},g,q;b=b||ga.valuen.data[a];c=c||ga.valuen.html[a];if(b){$(d+" .field_msg").html("");do g=!1,$.each(e,function(){var c,d,n=$(this),q,u;if(n.attr("data-repeater")&&!h[this.name]){h[this.name]=!0;if(this.name&&(b[this.name]||/checkbox|radio/i.test(this.type))){d=b[this.name];if(/checkbox|radio/i.test(this.type)){q=n.val();u=!1;if(d)for(c=0;c<d.length;c++)if(d[c]==q){u=!0;break}n.prop("checked",u)}else n.val(d[0]);g=!0;
ga.repeat.change(a,this.name,!0);e=f.find(":input").get();return!1}b[this.name]||/checkbox|radio/i.test(this.type)||console.warn("ga.valuen.restore() no data found for repeater setting value on "+this.name+" type "+this.type+" to "+d[0])}});while(g);$.each(e,function(){var c,d,e,f,g;if(this.name&&!h[this.name])if(e=$(this),(b[this.name]||/checkbox|radio/i.test(this.type))&&!/button/i.test(this.nodeName))if(d=b[this.name],/checkbox|radio/i.test(this.type)){f=e.val();g=!1;if(d)for(c=0;c<d.length;c++)if(d[c]==
...
ga.valuen.input=function(a,b){var c=$("#"+a),d=c.find(":input").get(),f={},e;if(b){do e=!1,$.each(d,function(){var h,g,q=$(this),k,r;if(q.attr("data-repeater")&&!f[this.name]){f[this.name]=!0;if(this.name&&(b[this.name]||/checkbox|radio/i.test(this.type))){g=b[this.name];"[object Array]"!==Object.prototype.toString.call(g)&&(g=[g]);if(/checkbox|radio/i.test(this.type)){k=q.val();r=!1;if(g)for(h=0;h<g.length;h++)if(g[h]==k){r=!0;break}q.prop("checked",r)}else q.val(g[0]);e=!0;ga.repeat.change(a,this.name,
!0);d=c.find(":input").get();return!1}b[this.name]||/checkbox|radio/i.test(this.type)||console.warn("ga.valuen.input() no data found for repeater setting value on "+this.name+" type "+this.type+" to "+g[0])}});while(e);$.each(d,function(){var c,d,e,k,r;if(this.name&&!f[this.name])if(e=$(this),(b[this.name]||/checkbox|radio/i.test(this.type))&&!/button/i.test(this.nodeName))if(d=b[this.name],"[object Array]"!==Object.prototype.toString.call(d)&&(d=[d]),/checkbox|radio/i.test(this.type)){k=e.val();
...
ga.calc.mktree=function(a,b){for(var c=[],d=null,f,e,h=0;h<a.length;++h){token=a[h];if(ga.calc.is_function_paren.test(token)){f=ga.calc.parensub(a.slice(h));if(f._error)return f;h+=f.newofs;token={op:token,args:[f=ga.calc.mktree(f.a)]};e=1;if(f._error)return f}else if(ga.calc.is_open_paren.test(token)){f=ga.calc.parensub(a.slice(h));if(f._error)return f;h+=f.newofs;token=ga.calc.mktree(f.a);if(token._error)return token;e=1}else e=0;e||ga.calc.is_atom.test(token)?(c.push(token),d&&(b?ga.calc.precedence[d]<
...
ga.bokeh.getdata=function(a,b){var c={},d=b.split(/\r?\n/),f=d.length,e,h=0;for(e=0;e<f;++e)if(h||-1!==d[e].indexOf("var docs_json ="))if(!h)h=1,c.docs_json=$.parseJSON(d[e].replace(/^\s*var\s+docs_json\s+=\s+/,"").replace(/(^'|('|);$)/g,""));else if(1==h&&-1!==d[e].indexOf("var render_items =")){c.render_items=$.parseJSON(d[e].replace(/^\s*var\s+render_items\s+=\s+/,"").replace(/;$/,""));break}return c};
...
ga.qr.question=function(a,b){var c="",d,f,e,h="",g="",q,k=[],r={},n;b._uuid||(h+="no _uuid in received data. ");b._msgid||(h+="no _msgid in received data. ");b._question||(h+="no _question in received data. ");b._question.fields||(h+="no _question:fields in received data. ");if(h.length)return ga.qr.rerror(b,h);b._question.title&&(c+="<h3>"+b._question.title+"</h3>");b._question.text&&(c+=b._question.text);q=b._uuid+"-"+b._msgid;c+='<form id="'+q+'"><table>';for(d=0;d<b._question.fields.length;++d)if(e=
b._question.fields[d],e.id||(h+="No id in field "+d+". "),e.type||(h+="No type in field "+d+". "),e.id&&e.type)switch(r[e.id]&&(h+="Duplicate id in _question fields:"+e.id+". "),r[e.id]=1,e.help?(f=' class="help_link"',n=' class="highlight help_link"',help_span='<span class="help">'+e.help+"</span>"):(f="",n=' class="highlight"',help_span=""),e.type){case "label":c+="<tr><td colspan=2><label"+f+">";e.label&&(c+=e.label);c+="</label>"+help_span+"</td></tr>";break;case "file":c+="<tr><td>";e.label&&
...
(c+=' accept="'+e.accept+'"');c+=">"+help_span+' or <button id="'+e.id+'_button" name="'+e.id+'_button" data-type="lrfile"'+f+'><span class="buttontext">Browse server</span></button>'+help_span+'</td><td><span id="'+e.id+'_altval"></td></span><input type="hidden" name="_selaltval_'+e.id+'" value="'+e.id+'_altval"</input></td></tr>';g+='ga.altfile("'+q+'","'+e.id+'","'+e.id+'_altval" );$( "#'+q+'" ).change( function(){ $( "#'+e.id+'_altval" ).html( "<i>Local</i>: " + $( "#'+e.id+'" ).val().replace(/^C:.fakepath./,""));});ga.altfile.button( "'+
q+'","'+e.id+'","'+e.label+'","rfile",function(v){ga.altfile.button.lrfile(v,"'+q+'","'+e.id+'")});';e.required&&(g+=',"lrfile"');g+='$("#'+e.id+'_button").on("click",function(){return ga.altfile.button.call("'+q+'","'+e.id+'");});';break;case "text":c+="<tr><td>";e.label&&(c+='<label for="'+e.id+'"'+f+">"+e.label+"</label>");c+='</td><td><input type="text" id="'+e.id+'" name="'+e.id+'"'+f;e.required&&(c+=" required");e.readonly&&(c+=" readonly");e["default"]&&(c+=' value="'+e["default"]+'"');e.pattern&&
(c+=' pattern="'+e.pattern+'"');e.maxlength&&(c+=' maxlength="'+e.maxlength+'"');e.size&&(c+=' size="'+e.size+'"');c+=">"+help_span+"</td></tr>";break;case "textarea":c+="<tr><td>";e.label&&(c+='<label for="'+e.id+'"'+f+">"+e.label+"</label>");c+='</td><td><textarea id="'+e.id+'" name="'+e.id+'"'+f;e.required&&(c+=" required");c+=" readonly";e.fontfamily&&(c+=' style="font-family: '+e.fontfamily+';"');e.cols&&(c+=" cols="+e.cols);e.rows&&(c+=" rows="+e.rows);c+=">";e["default"]&&(c+=e["default"]);
c+="</textarea>"+help_span+"</td></tr>";break;case "checkbox":c+="<tr><td>";e.label&&(c+='<label for="'+e.id+'"'+n+">"+e.label+"</label>");c+='</td><td><input type="checkbox" id="'+e.id+'" name="'+e.id+'"'+f;e.checked&&(c+=" checked");e.readonly&&(c+=" readonly");c+=">"+help_span+"</td></tr>";break;case "listbox":if(e.header){c+="<tr><td colspan=2";if(e.fontfamily||e.fontsize)c+=' style="',e.fontfamily&&(c+="font-family:"+e.fontfamily+";"),e.fontsize&&(c+="font-size:"+e.fontsize+";"),c+='"';e.width&&
(e.header=e.header.padEnd(e.width));e.header=e.header.replace(/ /g,"&nbsp;");console.log(e.header);c+=">"+e.header+"</td></tr>"}c+="<tr><td>";e.label&&(c+='<label for="'+e.id+'"'+n+">"+e.label+"</label>");c+="</td>";c=e.size&&1<e.size?c+"</tr><tr><td colspan=2>":c+"<td>";c+='<select id="'+e.id+'" name="'+e.id+'"'+f;e.fontfamily&&(c+=' style="font-family: '+e.fontfamily+';"');e.size&&(c+=" size="+e.size);c+=">";if(!e.values){h+="No values for listbox "+e.id+". ";break}if(e.returns){if(e.returns&&e.returns.length!=
e.values.length){h+="Listbox values length ("+e.values.length+") does not equal return length ("+e.returns.length+") for listbox "+e.id+". ";break}for(f=0;f<e.values.length;++f)c+='<option value="'+e.returns[f]+'">'+e.values[f].replace(/ /g,"&nbsp")+"</option>"}else for(f=0;f<e.values.length;++f)c+='<option value="'+f+'">'+e.values[f].replace(/ /g,"&nbsp")+"</option>";e["default"]&&(g+='$("#'+e.id+" option[value='"+e["default"]+'\']").attr("selected", "true");',console.log("Listbox qeval: "+g));c+=
"</select>"+help_span+"</td></tr>";break;default:h+="Unknown or currently unsupported field:type "+e.type+". "}c+="</table></form>";if(b._question.buttons&&b._question.buttons.length)for(d=0;d<b._question.buttons.length;++d)switch(e=b._question.buttons[d],typeof e){case "string":bid=e.replace(/\W/g,"").toLowerCase();r[bid]&&(h+="Duplicate id in _question fields & buttons:"+bid+". ");r[bid]=1;k.push({id:bid,label:e,cb:ga.qr.cb,adata:[b,bid]});break;case "object":!e.id&&e.label&&(e.id=e.label.replace(/\W/g,
"").toLowerCase());e.id?(r[e.id]&&(h+="Duplicate id in _question fields & buttons:"+e.id+". "),r[e.id]=1,e.id!=e.id.replace(/\W/g,"")&&(h+="Buttons array object entry "+(d+1)+" id is not pure alphanumeric. ")):h+="Buttons array object entry "+(d+1)+" does not have an id. ";e.label||(h+="Buttons array object entry "+(d+1)+" does not have a label. ");if(!h.length){k.push({id:e.id,label:e.label,cb:ga.qr.cb,adata:[b,e.id]});e.help&&(k[k.length-1].help=e.help);break}break;default:h+="Buttons array entry "+
(d+1)+" unknown type: "+typeof e+". "}else bid="ok",r[bid]&&(h+="Duplicate id in _question fields & buttons:"+bid+". "),r[bid]=1,bid="cancel",r[bid]&&(h+="Duplicate id in _question fields & buttons:"+bid+". "),r[bid]=1,k=[{id:"ok",label:"OK",cb:ga.qr.cb,adata:[b,"ok"]},{id:"cancel",label:"Cancel",cb:ga.qr.cb,adata:[b,"cancel"]}];if(h.length)return ga.qr.rerror(b,h);ga.qr.openq[q]="open";ga.msg.box({icon:"question.png",noclose:1,text:c+"<p></p>",eval:'$("#'+q+'").on("keyup keypress", function(e) { var code = e.keyCode || e.which;  if (code  == 13) { e.preventDefault(); return false; }});'+
g,buttons:k,ptext:"<p></p>"},0,2)};
...
ga.msg.close1=function(){ga.repeats.restore();$(".modalDialog").removeClass("modalDialog_on");setTimeout(function(){$("#configbody").html("")},400);ga.usesplash&&setTimeout(function(){splashlogin()},500)};ga.msg.close2=function(){ga.repeats.restore();$(".modalDialog2").removeClass("modalDialog2_on");$("#configbody2").html("");ga.usesplash&&setTimeout(function(){splashlogin()},500)};
