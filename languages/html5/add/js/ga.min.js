...
ga.browser=function(){var a=navigator.userAgent;ga.browser.type="unknown";ga.browser.prefix="";ga.browser.gradient=!1;ga.browser.clrpkr=!1;ga.browser.version="unknown";-1<a.indexOf("Version")&&(ga.browser.version=/Version\/([^ ]*)/.exec(a)[1],ga.browser.majversion=/Version\/([\d]*)/.exec(a)[1]);-1<a.indexOf("Chrome")?(ga.browser.type="chrome",ga.browser.prefix="-webkit-",ga.browser.gradient=!0,ga.browser.clrpkr=!0):-1<a.indexOf("Safari")?(ga.browser.type="safari",ga.browser.prefix="-webkit-",ga.browser.gradient=
!0):-1<a.indexOf("Opera")?(ga.browser.type="opera",ga.browser.prefix="-o-",ga.browser.gradient=!0,ga.browser.clrpkr=!0):-1<a.indexOf("Firefox")?(ga.browser.type="mozilla",ga.browser.prefix="-moz-",ga.browser.gradient=!0,ga.browser.clrpkr=!0):-1<a.indexOf("MSIE")&&(ga.browser.type="msie",ga.browser.prefix="",ga.browser.gradient=!0)};ga.cssrule={};
...
ga.data.textarea=function(a,b){console.log("ga.data.textarea hmod_out = "+a);var c=a+"_textarea",e=a.replace(/^#/,"")+"_textarea";console.log("ga.data.textarea hmod_out_textarea = "+c);c=$(c);var g=c[0].scrollHeight-c[0].scrollTop===c[0].clientHeight;b||(b="");c.is(":hidden")&&(document.getElementById(e).removeAttribute("hidden"),document.getElementById(e).removeAttribute("style"),e=document.getElementById(e+"_label"))&&(e.removeAttribute("hidden"),e.removeAttribute("style"));"__reset__\n"==b.substr(0,
10)?c.val(b.substr(10)):c.val(c.val()+b);ga.set("textarea:rows")?ga.data.textarea.h[a]?c.height(ga.data.textarea.h[a]):ga.data.textarea.h[a]=parseFloat(c.prop("clientHeight"))+parseFloat(c.css("borderTopWidth"))+parseFloat(c.css("borderBottomWidth")):c.height(parseFloat(c.prop("scrollHeight"))+parseFloat(c.css("borderTopWidth"))+parseFloat(c.css("borderBottomWidth")));g&&c.scrollTop(c[0].scrollHeight)};ga.data.textarea.h={};
ga.data.airavata=function(a,b){a=$(a+"_airavata");a.is(":hidden")&&a.show();a.html(b)};
...
d=!0;ga.repeat.change(a,this.name,!0);f=g.find(":input").get();return!1}b[this.name]||/checkbox|radio/i.test(this.type)||console.warn("ga.valuen.restore() no data found for repeater setting value on "+this.name+" type "+this.type+" to "+h[0])}})}while(d);$.each(f,function(){var c;if(this.name&&!k[this.name]){var d=$(this);if((b[this.name]||/checkbox|radio/i.test(this.type))&&!/button/i.test(this.nodeName)){var e=b[this.name];if(/checkbox|radio/i.test(this.type)){var f=d.val();var g=!1;if(e)for(c=
0;c<e.length;c++)if(e[c]==f){g=!0;break}d.prop("checked",g)}else"file"===this.type?e[0]?$("#"+this.id+"_msg").html(" "+e[0]+" please reload manually (programmatic setting of local files disallowed by browser security)"):d.val(""):d.val(e[0])}else if(/button/i.test(this.nodeName)&&(e=d.attr("data-type"))&&(d=ga.altfile.button.getnames(this.id,e)))for(c=0;c<d.length;++c)b[d[c]]&&ga.altfile.button.addhtml(a,this.id,e,b[d[c]])}});for(h in c)$("#"+h).html(c[h])}};
...
ga.layout.process=function(a){if(a&&a.resource)if(ga.layout.module&&ga.layout.module.name){var b=ga.layout.module.name;ga.layout.fields.b_submit?ga.layout.fields.b_submit.dhtml=ga.layout.fields.b_submit.dhtml.replace(/<button id/,'<button class="ga-button-submit" id'):(console.error("layout.js: ga.layout.fields[b_submit] should already be defined"),ga.layout.fields.b_submit={},ga.layout.fields.b_submit.lhtml='<label class=""></label>',ga.layout.fields.b_submit.dhtml='<button id="'+b+'_b_submit_button" class="ga-button-submit"><span class="buttontext">Submit</span></button><div id="b_submit_buttonval"></div>');
...
b+'_output", true );\n';ga.layout.fields.b_submit.eval=a&&a.captcha?ga.layout.fields.b_submit.eval+("\n   ga.captcha( do_"+b+"_submit, $(this) );\n   return false;\n"):ga.layout.fields.b_submit.eval+('\n   return ga.xsede.select( "'+a.resource+'", do_'+b+"_submit, $(this) );\n");ga.layout.fields.b_submit.eval+="\n});\n";ga.layout.fields.b_reset?ga.layout.fields.b_reset.dhtml=ga.layout.fields.b_reset.dhtml.replace(/<button id/,'<button class="ga-button-reset" id'):(console.error("layout.js: ga.layout.fields[b_reset] should already be defined"),
ga.layout.fields.b_reset={},ga.layout.fields.b_reset.lhtml='<label class=""></label>',ga.layout.fields.b_reset.lhtml+="",ga.layout.fields.b_reset.dhtml='<button id="'+b+'_b_reset_button" class="ga-button-reset"><span class="buttontext">Reset</span></button><div id="b_reset_buttonval"></div>',ga.layout.fields.b_reset.dhtml+="");ga.layout.fields.b_reset.eval='\n$("#'+b+'_b_reset_button" ).click(function(){\n    console.log("b_reset");\n    return '+b+"_reset();\n});";ga.layout.fields[b+"_progress"]||
(ga.layout.fields[b+"_progress"]={},ga.layout.fields[b+"_progress"].lhtml="",ga.layout.fields[b+"_progress"].dhtml='<span id="'+b+'_progress"></span>',ga.layout.fields[b+"_progress"].eval="");ga.layout.fields[b+"_output_airavata"]||(ga.layout.fields[b+"_output_airavata"]={},ga.layout.fields[b+"_output_airavata"].lhtml="",ga.layout.fields[b+"_output_airavata"].dhtml='<span id="'+b+'_output_airavata"></span>',ga.layout.fields[b+"_output_airavata"].eval="");ga.layout.fields[b+"_output_msgs"]||(ga.layout.fields[b+
"_output_msgs"]={},ga.layout.fields[b+"_output_msgs"].lhtml="",ga.layout.fields[b+"_output_msgs"].dhtml='<div id="'+b+'_output_msgs" class="warning" type="msgs"></div>',ga.layout.fields[b+"_output_msgs"].eval="");ga.layout.fields[b+"_output_textarea"]||(ga.layout.fields[b+"_output_textarea"]={},ga.layout.fields[b+"_output_textarea"].lhtml="",ga.layout.fields[b+"_output_textarea"].dhtml='<textarea class="ga-field-output-control" readonly hidden id="'+b+'_output_textarea"></textarea>',ga.layout.fields[b+
"_output_textarea"].eval="");ga.layout.panel.panels?ga.layout.setup():console.error("error JSON contains no panels")}else console.error("ga.layout.process() required ga.layout.module.name set");else console.error("ga.layout.process() required defaults object argument not specified")};
