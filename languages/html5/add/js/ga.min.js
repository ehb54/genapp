...
ga.qr.question=function(a,b){console.log("ga.qr.question( "+a+", q )");var c="",d,e,f="",h;b._uuid||(f+="no _uuid in received data. ");b._msgid||(f+="no _msgid in received data. ");b._question||(f+="no _question in received data. ");b._question.fields||(f+="no _question:fields in received data. ");if(f.length)return ga.qr.rerror(b,f);b._question.title&&(c+=b._question.title);h=b._uuid+"-"+b._msgid;c+='<form id="'+h+'"><table>';for(d=0;d<b._question.fields.length;++d)if(e=b._question.fields[d],e.id||
(f+="No id in field "+d+". "),e.type||(f+="No type in field "+d+". "),e.id&&e.type)switch(e.type){case "text":c+="<tr><td>";e.label&&(c+='<label for="'+e.id+'">'+e.label+"</label>");c+='</td><td><input type="text" id="'+e.id+'"';e.required&&(c+=" required");e.readonly&&(c+=" readonly");e["default"]&&(c+=' value="'+e["default"]+'"');e.pattern&&(c+=' pattern="'+e.pattern+'"');e.maxlength&&(c+=' maxlength="'+e.maxlength+'"');e.size&&(c+=' size="'+e.size+'"');c+="></td></tr>";break;case "checkbox":c+=
"<tr><td>";e.label&&(c+='<label for="'+e.id+'" class="highlight">'+e.label+"</label>");c+='</td><td><input type="checkbox" id="'+e.id+'"';e.checked&&(c+=" checked");e.readonly&&(c+=" readonly");c+="></td></tr>";break;default:f+="Unknown or currently unsupported field:type "+e.fype+". "}c+="</table></form>";if(f.length)return ga.qr.rerror(b,f);console.log("qtext:"+c);ga.qr.openq[h]="open";messagebox({icon:"question.png",text:c,eval:'$("#'+h+'").on("keyup keypress", function(e) { var code = e.keyCode || e.which;  if (code  == 13) { e.preventDefault(); return false; }});',
buttons:[{id:"ok",label:"OK",cb:ga.qr.cb,adata:[b,"ok"]},{id:"cancel",label:"Cancel",cb:ga.qr.cb,adata:[b,"cancel"]}]})};
