...
3+e)for(m=a[g].trim().split(/\s+/),f=0;f<m.length;f++)k.push(m[f]);else h.push(a[g]);p.push(k);for(g=0;g<c.length;g++)switch($("#"+c[g]).attr("type")){case "text":k=new RegExp($("#"+c[g]).attr("pattern"));if(!k.test(h[g])){messagebox({icon:"warning.png",text:"Wrong format of the input file! Input value on the line #"+w+" is not a valid number. Options are: [Integer | Float point number | Number with exponent]. Check your input file",buttons:[{id:"ok",label:"OK"}]});return}break;case "number":k=[h[g]];
...
break;case "integer":if(ga.repeat.data[a].repeater[b].tableize&&0<d)for(k in h)g+=ga.repeat.data[a].repeat[k].htmls;for(q=1;q<=d;++q)for(k in h)n=b+"-"+k+"-"+(q-1),ga.repeat.map[k]=n,g+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,n).replace("%%label%%","["+q+"]").replace(ga.repeat.data[a].repeater[b].tableize?/<td.*?><label.*?>.*?<\/label><\/td>/:"",""),p+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,n),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[n]||(ga.repeat.data[a].repeater[n]=
jQuery.extend({},ga.repeat.data[a].repeater[k])),e.push(n),ga.repeat.data[a].repeater[n].value&&delete ga.repeat.data[a].repeater[n].value);break;case "listbox":h=b.replace(/-[0-9]+$/,"").replace(/^(.*)-([A-ZA-z0-9_]*)$/,"$2")+"-"+d;q=b+"-"+d;h=ga.repeat.children(a,h);for(k in h)n=q+"-"+k,ga.repeat.map[k]=n,g+=ga.repeat.data[a].repeat[k].htmlr.replace(/%%id%%/g,n).replace("%%label%%",""),p+=ga.repeat.data[a].repeat[k].evalr.replace(/%%id%%/g,n),ga.repeat.data[a].repeater[k]&&(ga.repeat.data[a].repeater[n]||
...
b,l);break;case "atomicstructure":x=k+":#"+b+":last_value";B=l.file?l.file:l;_jmol_info[b].script="set background ["+ga.colors.background+"]; set zoomlarge false;set echo top center;echo loading "+B.split("/").pop()+';refresh;load "'+B+'";';ga.set(a+":jsmoladd")&&(_jmol_info[b].script+=ga.set(a+":jsmoladd"));l.script&&(_jmol_info[b].script+=";"+l.script);$("#global_data").data(x,_jmol_info[b].script);$("#"+b).html(Jmol.getAppletHtml("jmolApplet"+b,_jmol_info[b]));break;case "checkbox":case "radio":z.prop("checked",
...
l),h=1,"_status"==b&&(y.job_status=l),/^_getinput/.test(b)&&"_getinput"==b&&ga.valuen.input(a,l),"_textarea"==b&&ga.data.textarea(q,l),"_airavata"==b&&ga.data.airavata(q,l),"_loginverify"==b&&ga.login.verify(l),"_loginapprove"==b&&ga.login.approve(l)):"-close"==b?g=1:"-close2"==b?p=1:(f||(w.text(""),w.append("<p>Unexpected results:</p>"),e=f=1),w.append("<p>"+b+" => "+l+"</p>"))});ga.value.saveLastValues(k);ga.value.saveLastValue(k,r);$(q+"_progress").html("");h&&syncState();g&&closeModal();p&&closeModal2();
return y};
...
ga.bokeh.render=function(a,b,c){c=ga.bokeh.getdata(b,c);a+="_output";ga.bokeh.savedata(a,b,c);ga.bokeh.renderdata(a,b)};ga.bokeh.renderdata=function(a,b){ga.bokeh.data[a]&&ga.bokeh.data[a][b]&&ga.bokeh.data[a][b].docs_json&&ga.bokeh.data[a][b].render_items&&($("#"+b).html('<div class="bk-root"><div class="bk-plotdiv" id="'+ga.bokeh.data[a][b].render_items[0].elementid+'"></div></div>'),Bokeh.embed.embed_items(ga.bokeh.data[a][b].docs_json,ga.bokeh.data[a][b].render_items))};
ga.bokeh.savedata=function(a,b,c){ga.bokeh.data[a]=ga.bokeh.data[a]||{};ga.bokeh.data[a][b]=c};ga.bokeh.reset=function(a,b){ga.bokeh.data[a]=ga.bokeh.data[a]||{};ga.bokeh.data[a][b]={};$("#"+b).html("")};
