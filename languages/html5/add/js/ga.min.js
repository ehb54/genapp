...
parseFloat(A.css("borderTopWidth"))+parseFloat(A.css("borderBottomWidth"))))):A.val(l)}else c?"_"==p.charAt(0)?/^_fs_/.test(p)&&ga.data.nofcrefresh[a]||("_message"==p&&messagebox(l),"_question"==p&&ga.qr.question(a,b),/^_getinput/.test(p)&&"_getinput"==p&&ga.valuen.input(a,l),"_textarea"==p&&ga.data.textarea(r,l),"_airavata"==p&&ga.data.airavata(r,l),"_status"==p&&"complete"==l&&c(d,0,0)):(f||(x.append("<p>Unexpected results:</p>"),f=1),x.append("<p>"+p+" => "+l+"</p>")):"_"==p.charAt(0)?/^_fs_/.test(p)&&
ga.data.nofcrefresh[a]||($("#_state").data(p,l),h=1,"_status"==p&&(z.job_status=l),/^_getinput/.test(p)&&"_getinput"==p&&ga.valuen.input(a,l),"_textarea"==p&&ga.data.textarea(r,l),"_airavata"==p&&ga.data.airavata(r,l),"_loginverify"==p&&ga.login.verify(l),"_loginapprove"==p&&ga.login.approve(l)):"-close"==p?g=1:"-close2"==p?q=1:(f||(x.text(""),x.append("<p>Unexpected results:</p>"),e=f=1),x.append("<p>"+p+" => "+l+"</p>"))});ga.value.saveLastValues(k);ga.value.saveLastValue(k,u);$(r+"_progress").html("");
h&&syncState();g&&closeModal();q&&closeModal2();return z};
...
ga.bokeh.savedata=function(a,b,c){ga.bokeh.data[a]=ga.bokeh.data[a]||{};ga.bokeh.data[a][b]=c};ga.bokeh.reset=function(a,b){ga.bokeh.data[a]&&ga.bokeh.data[a][b]&&(ga.bokeh.data[a][b]={},$("#"+b).html(""))};ga.qr={};
ga.qr.question=function(a,b){console.log("ga.qr.question( "+a+", q )");var c="",d,e,f="";b._uuid||(f+="no _uuid in received data. ");b._msgid||(f+="no _msgid in received data. ");b._question||(f+="no _question in received data. ");b._question.fields||(f+="no _question:fields in received data. ");if(f.length)return ga.qr.rerror(b,f);b._question.title&&(c+=b._question.title);c+='<form id="'+(b._uuid+"-"+b._msgid)+'"><table>';for(d=0;d<b._question.fields.length;++d)if(e=b._question.fields[d],e.id||(f+=
"No id in field "+d+". "),e.type||(f+="No type in field "+d+". "),e.id&&e.type)switch(e.type){case "text":c+="<tr><td>";e.label&&(c+='<label for="'+e.id+'">'+e.label+"</label>");c+='</td><td><input type="text" id="'+e.id+'"';e.required&&(c+=" required");e.readonly&&(c+=" readonly");e["default"]&&(c+=' value="'+e["default"]+'"');e.pattern&&(c+=' pattern="'+e.pattern+'"');e.maxlength&&(c+=' maxlength="'+e.maxlength+'"');e.size&&(c+=' size="'+e.size+'"');c+="></td></tr>";break;case "checkbox":c+="<tr><td>";
e.label&&(c+='<label for="'+e.id+'" class="highlight">'+e.label+"</label>");c+='</td><td><input type="checkbox" id="'+e.id+'"';e.checked&&(c+=" checked");e.readonly&&(c+=" readonly");c+="></td></tr>";break;default:f+="Unknown or currently unsupported field:type "+e.fype+". "}c+="</table></form>";if(f.length)return ga.qr.rerror(b,f);console.log("qtext:"+c);messagebox({icon:"question.png",text:c,buttons:[{id:"ok",label:"OK",cb:ga.qr.cb,adata:[b,"ok"]},{id:"cancel",label:"Cancel",cb:ga.qr.cb,adata:[b,
"cancel"]}]})};ga.qr.cb=function(a,b){console.log("ga.qr.cb( data )");console.dir(a);console.dir(data);var c={},d=a._uuid+"-"+a._msgid;c._uuid=a._uuid;c._msgid=parseFloat(a._msgid);c._response={};c._response.button=b;a._question&&a._question.id&&(c._response.id=a._question.id);$("#"+d+" *").filter(":input").each(function(){console.dir(this);switch(this.type){case "text":c._response[this.id]=this.value;break;case "checkbox":this.checked&&(c._response[this.id]=!0)}});ga.qr.post(c)};
ga.qr.rerror=function(a,b){console.log("ga.qr.cb( data )");console.dir(a);console.dir(data);var c={};c._uuid=a._uuid;c._msgid=parseFloat(a._msgid);c._response={};c._response.error=b;a._question&&a._question.id&&(c._response.id=a._question.id);ga.qr.post(c);messagebox({icon:"toast.png",text:b})};
