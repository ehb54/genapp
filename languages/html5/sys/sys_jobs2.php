...
if ( !isset( $_REQUEST[ "_tree" ] ) &&
   ...
   foreach ( $GLOBALS[ 'jqgrid_jobs' ] as $job )
   {
       ...
       $locked = isprojectlocked( $job[ 'directory' ] );
       if ( isset( $job[ 'directory' ] ) && $locked ) {
          if ( !strlen( $project ) )
          {
              $project = 'no_project_specified';
          }
            ...
            cached_progress( $job["_id"] ) )
       {
           $endasprogress = 1;
           $remaining = "";
           if ( isset( $job["start"] ) && $GLOBALS[ 'cached_progress' ] > 0 ) {
               $secsrun = time() - ga_db_date_secs( $job["start"] );
               $remaining = ( 1 - $GLOBALS[ 'cached_progress' ] ) * ( $secsrun / $GLOBALS[ 'cached_progress' ] );
               $remaining_s = sprintf( $remaining > 5 * 60 ? "%.0f" : "%.2f", $remaining - 60 * intval( $remaining / 60 ) );
               $remaining_m = intval( $remaining / 60 ) % 60;
               $remaining_h = intval( $remaining /(60 * 60 ) ) % 24;
               $remaining_d = intval( $remaining /(24 * 60 * 60 ) );

               $remaining =
                   "( e.c. " .
                   ( $remaining_d > 0 ? $remaining_d . "d " : "" ) .
                   ( $remaining_h > 0 ? $remaining_h . "h " : "" ) .
                   ( $remaining_m > 0 ? $remaining_m . "m " : "" ) .
                   ( $remaining_s > 0 ? $remaining_s . "s" : "" ) .
                   " )"
                   ;
           }
           $job[ "end" ] = sprintf( "%.1f%% $remaining", 100.0 * $GLOBALS[ 'cached_progress' ] );
       }

       $jobactions = "<span class='ga-ja-g' onclick='ga.util.jaa(this)'>&rarr;</span> <span class='ga-ja-g' onclick='ga.util.jaa(this,1)'>&rArr;</span>";
       if ( $duration == 'active' ) {
           $jobactions .= " <span class='ga-ja-r' onclick='ga.util.jac(this)'>&otimes;</span>";
       } else {
       ...
       if ( $endasprogress )
       {
          ...
          $row[ "cells" ][] = array( "value" => intval( isset( $job["end"] ) ? ga_db_date_secs( $job["end"] ) : 0 ) );
           if ( isset( $job["end"] ) ) {
               $recenttime = time() - ga_db_date_secs( $job["end"] );
               if ( $recenttime <= 3600 ) {
                   $recent .= "H";
               }
               if ( $recenttime <= 3600 * 24 ) {
                   $recent .= "D";
               }
               if ( $recenttime <= 3600 * 24 * 7 ) {
                   $recent .= "W";
               }
               if ( $recenttime <= 3600 * 24 * 31 ) {
                   $recent .= "M";
               }
           }                   
       }
   ...
   $results["colModel"][] = array( "name" => "endnumeric", "index" => "endnumeric", "width" => 150, "align" => "left", "jsonmap" => "cells.$index.value", "hidden" => true ); $index++;
   $results["colModel"][] = array( "name" => "duration", "index" => "durationnumeric", "width" => 100, "align" => "left", "jsonmap" => "cells.$index.value" ); $index++;
   $results["colModel"][] = array( "name" => "durationnumeric", "index" => "durationnumeric", "width" => 100, "align" => "left", "jsonmap" => "cells.$index.value", "hidden" => true ); $index++;
   $results["colModel"][] = array( "name" => "remoteip", "index" => "remoteip", "width" => 125, "align" => "left", "jsonmap" => "cells.$index.value", "hidden" => true ); $index++;
   $results["colModel"][] = array( "name" => "resource", "index" => "resource", "width" => 125, "align" => "left", "jsonmap" => "cells.$index.value", "hidden" => true ); $index++;
   $results["colModel"][] = array( "name" => "recent", "index" => "recent", "width" => 125, "align" => "left", "jsonmap" => "cells.$index.value", "hidden" => true ); $index++;
   ...
   $results["colNames"] = [
       ...
       , "End numeric"
       , "<span class='ga-jqg-title'>Duration</span>"
       , "Duration numeric"
       , "Remote IP"
       , "Resource"
       , "Recent"
