...
function phpmailer_mail( $to, $subject, $body, $attach = NULL, $attachdata = NULL ) {
    ...
    try {
        ...
        $mail->setFrom( $emconfig->id . '@' . $json->mail->from );
        ...
        if ( isset( $attach ) && count( $attach ) ) {
            ...
            foreach ( $attach as $f ) {
                # $fn = str_replace( '__docroot:html5__/$emconfig->id/', '', $f );
                $fn = preg_replace( '/^.*\//', '', $f );
...
function mymail( $to, $subject, $body )
{
    ...
    $headers = array ('From' => $emconfig->id . '@' . $json->mail->from,
                      'To' => $to,
                      'Subject' => $subject );
...
function admin_mail( $subject, $body ) {
    ...
    $json = json_decode( file_get_contents( "appconfig.json" ) );
