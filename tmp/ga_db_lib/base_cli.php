...
__~debug:basemylog{error_log( "startup cli __application__ __menu:id__ __menu:modules:id__\n", 3, "/tmp/mylog" );}

$_REQUEST = json_decode( $argv, true );

if ( !sizeof( $_REQUEST ) )
{
    $msg = "[PHP code received no \$_REQUEST]\n";
...
$GLOBALS[ 'logon'     ] = isset( $_REQUEST[ '_logon' ] ) ? $_REQUEST[ 'logon' ] : 'not logged in';
$GLOBALS[ 'project'   ] = isset( $_REQUEST[ '_project' ] ) ? $_REQUEST[ '_project' ] : 'not in a project';
$GLOBALS[ 'command'   ] = $cmd;
$GLOBALS[ 'REMOTE_ADDR' ] = "not from an ip";
