...
| Job Manager filters | `Reference-Jobs.md`, legacy Job Manager code | Partial | Running-only, completed-window, project, and module filters should refresh the list canonically. | `t/unit_tests/ui2_runtime_logic.t`, `t/unit_tests/ui2_views.t` | Completed-window resolution, project, module, and no-running-jobs cases are covered. Server date display differs from legacy. Need a real running-job case; MMC or another long-running live job is required. | P0 |
...
| User settings/config | Legacy user configuration code, UI2 native theme tokens | Partial | Project/help settings should preserve legacy behavior; UI2 theme should offer native System/Slate/Light/Dark and Bootswatch-inspired review candidates. | Directive guardrails; `t/unit_tests/ui2_views.t`; `t/unit_tests/ui2_runtime_logic.t` | UI2 replaces legacy Bootswatch theme controls in Settings with a local native selector. Settings should behave as a utility splash/modal rather than replacing active module state. | P1 |
