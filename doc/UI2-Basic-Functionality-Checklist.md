...
| User settings/config | Legacy user configuration code, UI2 native theme tokens | Partial | Project/help settings should preserve legacy behavior; UI2 theme should offer native System/Light/Dark selection. | Directive guardrails; `t/unit_tests/ui2_views.t`; `t/unit_tests/ui2_runtime_logic.t` | UI2 replaces legacy Bootswatch theme controls in Settings with a local native selector. Settings should behave as a utility splash/modal rather than replacing active module state. | P1 |
