...
is( $generated->{status}, 0, 'unit_high_risk_features fixture generates before data-update runtime checks' )
...
my ( $status, $output, $quoted ) = run_command(
    cwd => $repo_root,
    env => {},
    cmd => [ $node, $harness, 'data-update', $ga_js ],
);

ok( $status == 0, 'generated ga.js data-update behavior works in DOM harness' )
    or diag("command failed ($status): $quoted\n$output");
like( $output, qr/ok - data-update/, 'DOM harness reports data-update scenario success' );
