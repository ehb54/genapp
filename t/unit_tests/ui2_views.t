...
like( $ui2_js, qr/function renderNglSelectionInspector\(output, component, structurePayload\).*?selection_visibility === true.*?Whole structure.*?Flexible regions only.*?Selected region only/s, 'NGL adds optional selection visibility controls only for opted-in inspectors' );
