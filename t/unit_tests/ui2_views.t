...
like( $ui2_js, qr/function renderNglComponentPlacement\(output, payload, plot, renderRevision\).*?Promise\.all\(specs\.map.*?stage\.loadFile.*?attachNglPlacementCoordinates.*?applyNglPlacementTransform.*?renderNglPlacementControls/s, 'opt-in NGL component placement loads independent topologies and optional coordinates into one shared stage' );
