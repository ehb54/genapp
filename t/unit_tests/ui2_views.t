...
like( $ui2_js, qr/function renderReactWorkbenchFieldGroup\(groupFields, role, presentation = \{\}\).*?planFields\(Array\.isArray\(groupFields\) \? groupFields : \[\]\).*?renderTableizedRepeater\(item, role, presentation\).*?renderField\(item\.field, role\)/s, 'React workbench field groups retain native repeater, matrix, and ordinary-field rendering' );
...
like( $ui2_react_source, qr/const requestResultFocus = React\.useCallback.*?resultGroups\.some\(\(group\) => group\.id === id\).*?setActiveResult\(id\).*?setPendingResultFocus\(id\).*?window\.addEventListener\("ui2-focus-result", focusResult\)/s, 'React workbenches accept action focus only for a declared result group' );
like( $ui2_react_source, qr/pendingResultFocus !== activeResult.*?visibleOutputGroups\.some.*?resultCardRef\.current.*?scrollIntoView\(\{ behavior: "smooth", block: "start", inline: "nearest" \}\)/s, 'an action-selected result scrolls its rendered result card into the visible workspace' );
like( $ui2_react_source, qr/const definition = actionReviews\[id\].*?requestResultFocus\(definition\.result\).*?setActionReview\(\{ definition, values: bridge\.syncValues\(\) \}\)/s, 'React workbenches create a review only for a view-declared result group' );
...
like( $ui2_react_css, qr/\.ui2-repeat-compact \.ui2-repeated-coupled-choice-cards\{(?=[^}]*display:grid)(?=[^}]*grid-template-columns:repeat\(2,minmax\(0,1fr\)\))(?=[^}]*min-width:0)[^}]*\}/, 'compact coupled choices remain visible side by side inside their table cell' );
like( $ui2_react_css, qr/\.ui2-repeat-compact \.ui2-repeated-coupled-choice-cards \.ui2-repeated-choice-card\{(?=[^}]*border:0)(?=[^}]*background:(?:transparent|0 0))[^}]*\}/, 'compact coupled choices use ordinary radio rows rather than inline cards' );
...
like( $ui2_react_source, qr/fieldPresentations\[field\.id \|\| ""\]\?\.control === "choice-cards" && repeatExpressionActive\(field\.repeat, liveValues\)/, 'choice cards honor the declared UI2 field visibility condition' );
...
like( $ui2_js, qr/function savedInputSummaryValues\(inputs\).*?fieldIsFileLike\(field\).*?state\.values\?\.\[field\.id\].*?summary\[field\.id\] = cloneUi2Value\(restoredValue\)/s, 'ui2 reattachment summaries recover display values for server-selected file fields without adding unrelated defaults' );
...
unlike( $ui2_js, qr/function renderUserConfigTool\(module, fields, options = \{\}\).*?renderUtilitySection\("Status".*?function requiredPasswordChangeFields/s, 'ui2 Settings uses its inline status without rendering a duplicate output section' );
like( $ui2_js, qr/function renderRegisterTool\(module, fields\).*?renderUtilitySection\("Status", outputFields, "output"\)/s, 'other utility tools retain their declared status output section' );
