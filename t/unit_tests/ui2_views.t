...
like( $ui2_js, qr/function attachNglPlacementCoordinates\(component, trajectoryPayload\).*?window\.NGL\.autoLoad.*?loaded\?\.frames.*?component\.addTrajectory.*?setFrame\?\.\(0\)/s, 'placement coordinate attachment uses a parsed PDB frame without changing PDB-only components' );
...
like( $ui2_js, qr/function attachNglFileTrajectory\(.*?window\.NGL\?\.autoLoad.*?nglTrajectoryParameters\(.*?loaded\?\.frames.*?component\.addTrajectory\(frames, trajectoryParams\)/s, 'ui2 NGL viewer parses file trajectories and unwraps PDB coordinate frames before attachment' );
