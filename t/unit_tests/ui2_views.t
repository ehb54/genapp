...
like( $ui2_js, qr/function renderNglOutput\(output, value\).*?new window\.NGL\.Stage\(plot\.id, nglViewerStageParams\(output\)\).*?const structureLoadParams = Object\.assign\(\{\}, structurePayload\.loadparams \|\| \{\}, \{ defaultRepresentation: false \}\).*?stage\.loadFile\(normalizeNglLoadName\(structurePayload\.loadname\), structureLoadParams/s, 'ui2 NGL outputs load rebased payloads without an unrequested duplicate NGL representation' );
