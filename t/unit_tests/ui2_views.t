...
like( $ui2_js, qr/output\._ui2_ngl_density_run !== event\.run[\s\S]+output\._ui2_ngl_density_run = event\.run;[\s\S]+output\._ui2_ngl_density_user_isovalue = null;[\s\S]+output\._ui2_ngl_density_user_opacity = null;[\s\S]+renderNglDensityUpdate/s, 'the first density event from a new job resets stale contour preferences while same-job updates retain them' );
